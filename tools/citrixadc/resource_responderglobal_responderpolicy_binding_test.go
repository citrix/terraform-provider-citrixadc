		...
		Steps: []resource.TestStep{
			resource.TestStep{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckResponderglobal_responderpolicy_bindingExist("citrixadc_responderglobal_responderpolicy_binding.foo", nil),
				),
			},
		},
	})
...
func testAccCheckResponderglobal_responderpolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		data, err := nsClient.FindResource(netscaler.Responderglobal_responderpolicy_binding.Type(), rs.Primary.ID)
...
func testAccCheckResponderglobal_responderpolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := nsClient.FindResource(netscaler.Responderglobal_responderpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
resource "citrixadc_responderglobal_responderpolicy_binding" "foo" {
