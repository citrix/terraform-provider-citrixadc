...
func createInatFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In createInatFunc")
...
func readInatFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] citrixadc-provider:  In readInatFunc")
	client := meta.(*NetScalerNitroClient).client
	inatName := d.Id()
	log.Printf("[DEBUG] citrixadc-provider: Reading inat state %s", inatName)
	data, err := client.FindResource(netscaler.Inat.Type(), inatName)
	if err != nil {
		log.Printf("[WARN] citrixadc-provider: Clearing inat state %s", inatName)
		d.SetId("")
		return nil
	}
...
func updateInatFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In updateInatFunc")
	client := meta.(*NetScalerNitroClient).client
	...
	hasChange := false
	if d.HasChange("ftp") {
		log.Printf("[DEBUG]  citrixadc-provider: Ftp has changed for inat %s, starting update", inatName)
		inat.Ftp = d.Get("ftp").(string)
		hasChange = true
	}
	if d.HasChange("mode") {
		log.Printf("[DEBUG]  citrixadc-provider: Mode has changed for inat %s, starting update", inatName)
		inat.Mode = d.Get("mode").(string)
		hasChange = true
	}
	if d.HasChange("name") {
		log.Printf("[DEBUG]  citrixadc-provider: Name has changed for inat %s, starting update", inatName)
		inat.Name = d.Get("name").(string)
		hasChange = true
	}
	if d.HasChange("privateip") {
		log.Printf("[DEBUG]  citrixadc-provider: Privateip has changed for inat %s, starting update", inatName)
		inat.Privateip = d.Get("privateip").(string)
		hasChange = true
	}
	if d.HasChange("proxyip") {
		log.Printf("[DEBUG]  citrixadc-provider: Proxyip has changed for inat %s, starting update", inatName)
		inat.Proxyip = d.Get("proxyip").(string)
		hasChange = true
	}
	if d.HasChange("publicip") {
		log.Printf("[DEBUG]  citrixadc-provider: Publicip has changed for inat %s, starting update", inatName)
		inat.Publicip = d.Get("publicip").(string)
		hasChange = true
	}
	if d.HasChange("tcpproxy") {
		log.Printf("[DEBUG]  citrixadc-provider: Tcpproxy has changed for inat %s, starting update", inatName)
		inat.Tcpproxy = d.Get("tcpproxy").(string)
		hasChange = true
	}
	if d.HasChange("td") {
		log.Printf("[DEBUG]  citrixadc-provider: Td has changed for inat %s, starting update", inatName)
		inat.Td = d.Get("td").(int)
		hasChange = true
	}
	if d.HasChange("tftp") {
		log.Printf("[DEBUG]  citrixadc-provider: Tftp has changed for inat %s, starting update", inatName)
		inat.Tftp = d.Get("tftp").(string)
		hasChange = true
	}
	if d.HasChange("useproxyport") {
		log.Printf("[DEBUG]  citrixadc-provider: Useproxyport has changed for inat %s, starting update", inatName)
		inat.Useproxyport = d.Get("useproxyport").(string)
		hasChange = true
	}
	if d.HasChange("usip") {
		log.Printf("[DEBUG]  citrixadc-provider: Usip has changed for inat %s, starting update", inatName)
		inat.Usip = d.Get("usip").(string)
		hasChange = true
	}
	if d.HasChange("usnip") {
		log.Printf("[DEBUG]  citrixadc-provider: Usnip has changed for inat %s, starting update", inatName)
...
func deleteInatFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In deleteInatFunc")
	client := meta.(*NetScalerNitroClient).client
	inatName := d.Id()
