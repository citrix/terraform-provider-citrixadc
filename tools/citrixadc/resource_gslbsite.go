...
func createGslbsiteFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In createGslbsiteFunc")
...
func readGslbsiteFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] citrixadc-provider:  In readGslbsiteFunc")
	client := meta.(*NetScalerNitroClient).client
	gslbsiteName := d.Id()
	log.Printf("[DEBUG] citrixadc-provider: Reading gslbsite state %s", gslbsiteName)
	data, err := client.FindResource(netscaler.Gslbsite.Type(), gslbsiteName)
	if err != nil {
		log.Printf("[WARN] citrixadc-provider: Clearing gslbsite state %s", gslbsiteName)
		d.SetId("")
		return nil
	}
...
func updateGslbsiteFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In updateGslbsiteFunc")
	client := meta.(*NetScalerNitroClient).client
	...
	hasChange := false
	if d.HasChange("clip") {
		log.Printf("[DEBUG]  citrixadc-provider: Clip has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Clip = d.Get("clip").(string)
		hasChange = true
	}
	if d.HasChange("metricexchange") {
		log.Printf("[DEBUG]  citrixadc-provider: Metricexchange has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Metricexchange = d.Get("metricexchange").(string)
		hasChange = true
	}
	if d.HasChange("naptrreplacementsuffix") {
		log.Printf("[DEBUG]  citrixadc-provider: Naptrreplacementsuffix has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Naptrreplacementsuffix = d.Get("naptrreplacementsuffix").(string)
		hasChange = true
	}
	if d.HasChange("nwmetricexchange") {
		log.Printf("[DEBUG]  citrixadc-provider: Nwmetricexchange has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Nwmetricexchange = d.Get("nwmetricexchange").(string)
		hasChange = true
	}
	if d.HasChange("parentsite") {
		log.Printf("[DEBUG]  citrixadc-provider: Parentsite has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Parentsite = d.Get("parentsite").(string)
		hasChange = true
	}
	if d.HasChange("publicclip") {
		log.Printf("[DEBUG]  citrixadc-provider: Publicclip has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Publicclip = d.Get("publicclip").(string)
		hasChange = true
	}
	if d.HasChange("publicip") {
		log.Printf("[DEBUG]  citrixadc-provider: Publicip has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Publicip = d.Get("publicip").(string)
		hasChange = true
	}
	if d.HasChange("sessionexchange") {
		log.Printf("[DEBUG]  citrixadc-provider: Sessionexchange has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Sessionexchange = d.Get("sessionexchange").(string)
		hasChange = true
	}
	if d.HasChange("siteipaddress") {
		log.Printf("[DEBUG]  citrixadc-provider: Siteipaddress has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Siteipaddress = d.Get("siteipaddress").(string)
		hasChange = true
	}
	if d.HasChange("sitename") {
		log.Printf("[DEBUG]  citrixadc-provider: Sitename has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Sitename = d.Get("sitename").(string)
		hasChange = true
	}
	if d.HasChange("sitetype") {
		log.Printf("[DEBUG]  citrixadc-provider: Sitetype has changed for gslbsite %s, starting update", gslbsiteName)
		gslbsite.Sitetype = d.Get("sitetype").(string)
		hasChange = true
	}
	if d.HasChange("triggermonitor") {
		log.Printf("[DEBUG]  citrixadc-provider: Triggermonitor has changed for gslbsite %s, starting update", gslbsiteName)
...
func deleteGslbsiteFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In deleteGslbsiteFunc")
	client := meta.(*NetScalerNitroClient).client
	gslbsiteName := d.Id()
