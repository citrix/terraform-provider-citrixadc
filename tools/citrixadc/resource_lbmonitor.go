...
func createLbmonitorFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In createLbmonitorFunc")
...
func readLbmonitorFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG] citrixadc-provider:  In readLbmonitorFunc")
	client := meta.(*NetScalerNitroClient).client
	lbmonitorName := d.Id()
	log.Printf("[DEBUG] citrixadc-provider: Reading lbmonitor state %s", lbmonitorName)
	data, err := client.FindResource(netscaler.Lbmonitor.Type(), lbmonitorName)
	if err != nil {
		log.Printf("[WARN] citrixadc-provider: Clearing lbmonitor state %s", lbmonitorName)
		d.SetId("")
		return nil
	}
...
func updateLbmonitorFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In updateLbmonitorFunc")
	client := meta.(*NetScalerNitroClient).client
	...
	if d.HasChange("action") {
		log.Printf("[DEBUG]  citrixadc-provider: Action has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Action = d.Get("action").(string)
		hasChange = true
	}
	if d.HasChange("alertretries") {
		log.Printf("[DEBUG]  citrixadc-provider: Alertretries has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Alertretries = d.Get("alertretries").(int)
		hasChange = true
	}
	if d.HasChange("application") {
		log.Printf("[DEBUG]  citrixadc-provider: Application has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Application = d.Get("application").(string)
		hasChange = true
	}
	if d.HasChange("attribute") {
		log.Printf("[DEBUG]  citrixadc-provider: Attribute has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Attribute = d.Get("attribute").(string)
		hasChange = true
	}
	if d.HasChange("basedn") {
		log.Printf("[DEBUG]  citrixadc-provider: Basedn has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Basedn = d.Get("basedn").(string)
		hasChange = true
	}
	if d.HasChange("binddn") {
		log.Printf("[DEBUG]  citrixadc-provider: Binddn has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Binddn = d.Get("binddn").(string)
		hasChange = true
	}
	if d.HasChange("customheaders") {
		log.Printf("[DEBUG]  citrixadc-provider: Customheaders has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Customheaders = d.Get("customheaders").(string)
		hasChange = true
	}
	if d.HasChange("database") {
		log.Printf("[DEBUG]  citrixadc-provider: Database has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Database = d.Get("database").(string)
		hasChange = true
	}
	if d.HasChange("destip") {
		log.Printf("[DEBUG]  citrixadc-provider: Destip has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Destip = d.Get("destip").(string)
		hasChange = true
	}
	if d.HasChange("destport") {
		log.Printf("[DEBUG]  citrixadc-provider: Destport has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Destport = d.Get("destport").(int)
		hasChange = true
	}
	if d.HasChange("deviation") {
		log.Printf("[DEBUG]  citrixadc-provider: Deviation has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Deviation = d.Get("deviation").(int)
		hasChange = true
	}
	if d.HasChange("dispatcherip") {
		log.Printf("[DEBUG]  citrixadc-provider: Dispatcherip has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Dispatcherip = d.Get("dispatcherip").(string)
		hasChange = true
	}
	if d.HasChange("dispatcherport") {
		log.Printf("[DEBUG]  citrixadc-provider: Dispatcherport has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Dispatcherport = d.Get("dispatcherport").(int)
		hasChange = true
	}
	if d.HasChange("domain") {
		log.Printf("[DEBUG]  citrixadc-provider: Domain has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Domain = d.Get("domain").(string)
		hasChange = true
	}
	if d.HasChange("downtime") {
		log.Printf("[DEBUG]  citrixadc-provider: Downtime has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Downtime = d.Get("downtime").(int)
		hasChange = true
	}
	if d.HasChange("evalrule") {
		log.Printf("[DEBUG]  citrixadc-provider: Evalrule has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Evalrule = d.Get("evalrule").(string)
		hasChange = true
	}
	if d.HasChange("failureretries") {
		log.Printf("[DEBUG]  citrixadc-provider: Failureretries has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Failureretries = d.Get("failureretries").(int)
		hasChange = true
	}
	if d.HasChange("filename") {
		log.Printf("[DEBUG]  citrixadc-provider: Filename has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Filename = d.Get("filename").(string)
		hasChange = true
	}
	if d.HasChange("filter") {
		log.Printf("[DEBUG]  citrixadc-provider: Filter has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Filter = d.Get("filter").(string)
		hasChange = true
	}
	if d.HasChange("firmwarerevision") {
		log.Printf("[DEBUG]  citrixadc-provider: Firmwarerevision has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Firmwarerevision = d.Get("firmwarerevision").(int)
		hasChange = true
	}
	if d.HasChange("group") {
		log.Printf("[DEBUG]  citrixadc-provider: Group has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Group = d.Get("group").(string)
		hasChange = true
	}
	if d.HasChange("hostipaddress") {
		log.Printf("[DEBUG]  citrixadc-provider: Hostipaddress has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Hostipaddress = d.Get("hostipaddress").(string)
		hasChange = true
	}
	if d.HasChange("hostname") {
		log.Printf("[DEBUG]  citrixadc-provider: Hostname has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Hostname = d.Get("hostname").(string)
		hasChange = true
	}
	if d.HasChange("httprequest") {
		log.Printf("[DEBUG]  citrixadc-provider: Httprequest has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Httprequest = d.Get("httprequest").(string)
		hasChange = true
	}
	if d.HasChange("inbandsecurityid") {
		log.Printf("[DEBUG]  citrixadc-provider: Inbandsecurityid has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Inbandsecurityid = d.Get("inbandsecurityid").(string)
		hasChange = true
	}
	if d.HasChange("interval") {
		log.Printf("[DEBUG]  citrixadc-provider: Interval has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Interval = d.Get("interval").(int)
		hasChange = true
	}
	if d.HasChange("iptunnel") {
		log.Printf("[DEBUG]  citrixadc-provider: Iptunnel has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Iptunnel = d.Get("iptunnel").(string)
		hasChange = true
	}
	if d.HasChange("kcdaccount") {
		log.Printf("[DEBUG]  citrixadc-provider: Kcdaccount has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Kcdaccount = d.Get("kcdaccount").(string)
		hasChange = true
	}
	if d.HasChange("lasversion") {
		log.Printf("[DEBUG]  citrixadc-provider: Lasversion has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Lasversion = d.Get("lasversion").(string)
		hasChange = true
	}
	if d.HasChange("logonpointname") {
		log.Printf("[DEBUG]  citrixadc-provider: Logonpointname has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Logonpointname = d.Get("logonpointname").(string)
		hasChange = true
	}
	if d.HasChange("lrtm") {
		log.Printf("[DEBUG]  citrixadc-provider: Lrtm has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Lrtm = d.Get("lrtm").(string)
		hasChange = true
	}
	if d.HasChange("maxforwards") {
		log.Printf("[DEBUG]  citrixadc-provider: Maxforwards has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Maxforwards = d.Get("maxforwards").(int)
		hasChange = true
	}
	if d.HasChange("metric") {
		log.Printf("[DEBUG]  citrixadc-provider: Metric has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Metric = d.Get("metric").(string)
		hasChange = true
	}
	if d.HasChange("metrictable") {
		log.Printf("[DEBUG]  citrixadc-provider: Metrictable has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Metrictable = d.Get("metrictable").(string)
		hasChange = true
	}
	if d.HasChange("metricthreshold") {
		log.Printf("[DEBUG]  citrixadc-provider: Metricthreshold has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Metricthreshold = d.Get("metricthreshold").(int)
		hasChange = true
	}
	if d.HasChange("metricweight") {
		log.Printf("[DEBUG]  citrixadc-provider: Metricweight has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Metricweight = d.Get("metricweight").(int)
		hasChange = true
	}
	if d.HasChange("monitorname") {
		log.Printf("[DEBUG]  citrixadc-provider: Monitorname has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Monitorname = d.Get("monitorname").(string)
		hasChange = true
	}
	if d.HasChange("mssqlprotocolversion") {
		log.Printf("[DEBUG]  citrixadc-provider: Mssqlprotocolversion has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Mssqlprotocolversion = d.Get("mssqlprotocolversion").(string)
		hasChange = true
	}
	if d.HasChange("netprofile") {
		log.Printf("[DEBUG]  citrixadc-provider: Netprofile has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Netprofile = d.Get("netprofile").(string)
		hasChange = true
	}
	if d.HasChange("oraclesid") {
		log.Printf("[DEBUG]  citrixadc-provider: Oraclesid has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Oraclesid = d.Get("oraclesid").(string)
		hasChange = true
	}
	if d.HasChange("originhost") {
		log.Printf("[DEBUG]  citrixadc-provider: Originhost has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Originhost = d.Get("originhost").(string)
		hasChange = true
	}
	if d.HasChange("originrealm") {
		log.Printf("[DEBUG]  citrixadc-provider: Originrealm has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Originrealm = d.Get("originrealm").(string)
		hasChange = true
	}
	if d.HasChange("password") {
		log.Printf("[DEBUG]  citrixadc-provider: Password has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Password = d.Get("password").(string)
		hasChange = true
	}
	if d.HasChange("productname") {
		log.Printf("[DEBUG]  citrixadc-provider: Productname has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Productname = d.Get("productname").(string)
		hasChange = true
	}
	if d.HasChange("query") {
		log.Printf("[DEBUG]  citrixadc-provider: Query has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Query = d.Get("query").(string)
		hasChange = true
	}
	if d.HasChange("querytype") {
		log.Printf("[DEBUG]  citrixadc-provider: Querytype has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Querytype = d.Get("querytype").(string)
		hasChange = true
	}
	if d.HasChange("radaccountsession") {
		log.Printf("[DEBUG]  citrixadc-provider: Radaccountsession has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radaccountsession = d.Get("radaccountsession").(string)
		hasChange = true
	}
	if d.HasChange("radaccounttype") {
		log.Printf("[DEBUG]  citrixadc-provider: Radaccounttype has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radaccounttype = d.Get("radaccounttype").(int)
		hasChange = true
	}
	if d.HasChange("radapn") {
		log.Printf("[DEBUG]  citrixadc-provider: Radapn has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radapn = d.Get("radapn").(string)
		hasChange = true
	}
	if d.HasChange("radframedip") {
		log.Printf("[DEBUG]  citrixadc-provider: Radframedip has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radframedip = d.Get("radframedip").(string)
		hasChange = true
	}
	if d.HasChange("radkey") {
		log.Printf("[DEBUG]  citrixadc-provider: Radkey has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radkey = d.Get("radkey").(string)
		hasChange = true
	}
	if d.HasChange("radmsisdn") {
		log.Printf("[DEBUG]  citrixadc-provider: Radmsisdn has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radmsisdn = d.Get("radmsisdn").(string)
		hasChange = true
	}
	if d.HasChange("radnasid") {
		log.Printf("[DEBUG]  citrixadc-provider: Radnasid has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radnasid = d.Get("radnasid").(string)
		hasChange = true
	}
	if d.HasChange("radnasip") {
		log.Printf("[DEBUG]  citrixadc-provider: Radnasip has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Radnasip = d.Get("radnasip").(string)
		hasChange = true
	}
	if d.HasChange("recv") {
		log.Printf("[DEBUG]  citrixadc-provider: Recv has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Recv = d.Get("recv").(string)
		hasChange = true
	}
	if d.HasChange("resptimeout") {
		log.Printf("[DEBUG]  citrixadc-provider: Resptimeout has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Resptimeout = d.Get("resptimeout").(int)
		hasChange = true
	}
	if d.HasChange("resptimeoutthresh") {
		log.Printf("[DEBUG]  citrixadc-provider: Resptimeoutthresh has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Resptimeoutthresh = d.Get("resptimeoutthresh").(int)
		hasChange = true
	}
	if d.HasChange("retries") {
		log.Printf("[DEBUG]  citrixadc-provider: Retries has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Retries = d.Get("retries").(int)
		hasChange = true
	}
	if d.HasChange("reverse") {
		log.Printf("[DEBUG]  citrixadc-provider: Reverse has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Reverse = d.Get("reverse").(string)
		hasChange = true
	}
	if d.HasChange("rtsprequest") {
		log.Printf("[DEBUG]  citrixadc-provider: Rtsprequest has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Rtsprequest = d.Get("rtsprequest").(string)
		hasChange = true
	}
	if d.HasChange("scriptargs") {
		log.Printf("[DEBUG]  citrixadc-provider: Scriptargs has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Scriptargs = d.Get("scriptargs").(string)
		hasChange = true
	}
	if d.HasChange("scriptname") {
		log.Printf("[DEBUG]  citrixadc-provider: Scriptname has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Scriptname = d.Get("scriptname").(string)
		hasChange = true
	}
	if d.HasChange("secondarypassword") {
		log.Printf("[DEBUG]  citrixadc-provider: Secondarypassword has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Secondarypassword = d.Get("secondarypassword").(string)
		hasChange = true
	}
	if d.HasChange("secure") {
		log.Printf("[DEBUG]  citrixadc-provider: Secure has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Secure = d.Get("secure").(string)
		hasChange = true
	}
	if d.HasChange("send") {
		log.Printf("[DEBUG]  citrixadc-provider: Send has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Send = d.Get("send").(string)
		hasChange = true
	}
	if d.HasChange("servicegroupname") {
		log.Printf("[DEBUG]  citrixadc-provider: Servicegroupname has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Servicegroupname = d.Get("servicegroupname").(string)
		hasChange = true
	}
	if d.HasChange("servicename") {
		log.Printf("[DEBUG]  citrixadc-provider: Servicename has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Servicename = d.Get("servicename").(string)
		hasChange = true
	}
	if d.HasChange("sipmethod") {
		log.Printf("[DEBUG]  citrixadc-provider: Sipmethod has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Sipmethod = d.Get("sipmethod").(string)
		hasChange = true
	}
	if d.HasChange("sipreguri") {
		log.Printf("[DEBUG]  citrixadc-provider: Sipreguri has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Sipreguri = d.Get("sipreguri").(string)
		hasChange = true
	}
	if d.HasChange("sipuri") {
		log.Printf("[DEBUG]  citrixadc-provider: Sipuri has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Sipuri = d.Get("sipuri").(string)
		hasChange = true
	}
	if d.HasChange("sitepath") {
		log.Printf("[DEBUG]  citrixadc-provider: Sitepath has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Sitepath = d.Get("sitepath").(string)
		hasChange = true
	}
	if d.HasChange("snmpcommunity") {
		log.Printf("[DEBUG]  citrixadc-provider: Snmpcommunity has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Snmpcommunity = d.Get("snmpcommunity").(string)
		hasChange = true
	}
	if d.HasChange("snmpoid") {
		log.Printf("[DEBUG]  citrixadc-provider: Snmpoid has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Snmpoid = d.Get("snmpoid").(string)
		hasChange = true
	}
	if d.HasChange("snmpthreshold") {
		log.Printf("[DEBUG]  citrixadc-provider: Snmpthreshold has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Snmpthreshold = d.Get("snmpthreshold").(string)
		hasChange = true
	}
	if d.HasChange("snmpversion") {
		log.Printf("[DEBUG]  citrixadc-provider: Snmpversion has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Snmpversion = d.Get("snmpversion").(string)
		hasChange = true
	}
	if d.HasChange("sqlquery") {
		log.Printf("[DEBUG]  citrixadc-provider: Sqlquery has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Sqlquery = d.Get("sqlquery").(string)
		hasChange = true
	}
	if d.HasChange("sslprofile") {
		log.Printf("[DEBUG]  citrixadc-provider: Sslprofile has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Sslprofile = d.Get("sslprofile").(string)
		hasChange = true
	}
	if d.HasChange("state") {
		log.Printf("[DEBUG]  citrixadc-provider: State has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.State = d.Get("state").(string)
		hasChange = true
	}
	if d.HasChange("storedb") {
		log.Printf("[DEBUG]  citrixadc-provider: Storedb has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Storedb = d.Get("storedb").(string)
		hasChange = true
	}
	if d.HasChange("storefrontacctservice") {
		log.Printf("[DEBUG]  citrixadc-provider: Storefrontacctservice has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Storefrontacctservice = d.Get("storefrontacctservice").(string)
		hasChange = true
	}
	if d.HasChange("storefrontcheckbackendservices") {
		log.Printf("[DEBUG]  citrixadc-provider: Storefrontcheckbackendservices has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Storefrontcheckbackendservices = d.Get("storefrontcheckbackendservices").(string)
		hasChange = true
	}
	if d.HasChange("storename") {
		log.Printf("[DEBUG]  citrixadc-provider: Storename has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Storename = d.Get("storename").(string)
		hasChange = true
	}
	if d.HasChange("successretries") {
		log.Printf("[DEBUG]  citrixadc-provider: Successretries has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Successretries = d.Get("successretries").(int)
		hasChange = true
	}
	if d.HasChange("tos") {
		log.Printf("[DEBUG]  citrixadc-provider: Tos has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Tos = d.Get("tos").(string)
		hasChange = true
	}
	if d.HasChange("tosid") {
		log.Printf("[DEBUG]  citrixadc-provider: Tosid has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Tosid = d.Get("tosid").(int)
		hasChange = true
	}
	if d.HasChange("transparent") {
		log.Printf("[DEBUG]  citrixadc-provider: Transparent has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Transparent = d.Get("transparent").(string)
		hasChange = true
	}
	if d.HasChange("trofscode") {
		log.Printf("[DEBUG]  citrixadc-provider: Trofscode has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Trofscode = d.Get("trofscode").(int)
		hasChange = true
	}
	if d.HasChange("trofsstring") {
		log.Printf("[DEBUG]  citrixadc-provider: Trofsstring has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Trofsstring = d.Get("trofsstring").(string)
		hasChange = true
	}
	if d.HasChange("type") {
		log.Printf("[DEBUG]  citrixadc-provider: Type has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Type = d.Get("type").(string)
		hasChange = true
	}
	if d.HasChange("units1") {
		log.Printf("[DEBUG]  citrixadc-provider: Units1 has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Units1 = d.Get("units1").(string)
		hasChange = true
	}
	if d.HasChange("units2") {
		log.Printf("[DEBUG]  citrixadc-provider: Units2 has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Units2 = d.Get("units2").(string)
		hasChange = true
	}
	if d.HasChange("units3") {
		log.Printf("[DEBUG]  citrixadc-provider: Units3 has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Units3 = d.Get("units3").(string)
		hasChange = true
	}
	if d.HasChange("units4") {
		log.Printf("[DEBUG]  citrixadc-provider: Units4 has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Units4 = d.Get("units4").(string)
		hasChange = true
	}
	if d.HasChange("username") {
		log.Printf("[DEBUG]  citrixadc-provider: Username has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Username = d.Get("username").(string)
		hasChange = true
	}
	if d.HasChange("validatecred") {
		log.Printf("[DEBUG]  citrixadc-provider: Validatecred has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Validatecred = d.Get("validatecred").(string)
		hasChange = true
	}
	if d.HasChange("vendorid") {
		log.Printf("[DEBUG]  citrixadc-provider: Vendorid has changed for lbmonitor %s, starting update", lbmonitorName)
		lbmonitor.Vendorid = d.Get("vendorid").(int)
		hasChange = true
	}
	if d.HasChange("vendorspecificvendorid") {
		log.Printf("[DEBUG]  citrixadc-provider: Vendorspecificvendorid has changed for lbmonitor %s, starting update", lbmonitorName)
...
func deleteLbmonitorFunc(d *schema.ResourceData, meta interface{}) error {
	log.Printf("[DEBUG]  citrixadc-provider: In deleteLbmonitorFunc")
	client := meta.(*NetScalerNitroClient).client
	lbmonitorName := d.Id()
