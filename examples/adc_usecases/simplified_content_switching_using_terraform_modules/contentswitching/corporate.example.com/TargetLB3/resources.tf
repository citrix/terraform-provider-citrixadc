...
resource "citrixadc_lbvserver" "tf_targetlb3" {
  name            = var.targetlb3_config["name"]
  ipv46           = var.targetlb3_config["vip"]
  port            = var.targetlb3_config["port"]
  servicetype     = var.targetlb3_config["servicetype"]
  lbmethod        = var.targetlb3_config["lbmethod"]
  persistencetype = var.targetlb3_config["persistencetype"]
  timeout = var.targetlb3_config["timeout"]
  //redirurl        = var.targetlb3_config["redirurl"]
  //httpprofilename = var.targetlb3_config["httpprofile"]
}
//output LBvservers "targetlb3"
output "targetlb3name" {
  value = citrixadc_lbvserver.tf_targetlb3.name
}
/*
...
resource "citrixadc_lbvserver_service_binding" "tf_bindinghttp" {
  for_each    = var.servers
  name        = citrixadc_lbvserver.tf_targetlb3.name
  servicename = "${each.key}-${var.service_config["service_port"]}"
  depends_on  = [module.loadbalancingservices]
}
