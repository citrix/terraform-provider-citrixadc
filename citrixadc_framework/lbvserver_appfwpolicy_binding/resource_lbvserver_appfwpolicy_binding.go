...
func (r *LbvserverAppfwpolicyBindingResource) readLbvserverAppfwpolicyBindingFromApi(ctx context.Context, data *LbvserverAppfwpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "lbvserver_appfwpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("lbvserver_appfwpolicy_binding not found with the provided ID attributes"))
		return
	}
