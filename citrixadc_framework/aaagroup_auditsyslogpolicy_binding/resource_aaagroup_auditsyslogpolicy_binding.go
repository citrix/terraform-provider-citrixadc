...
func (r *AaagroupAuditsyslogpolicyBindingResource) readAaagroupAuditsyslogpolicyBindingFromApi(ctx context.Context, data *AaagroupAuditsyslogpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "aaagroup_auditsyslogpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("aaagroup_auditsyslogpolicy_binding not found with the provided ID attributes"))
		return
	}
