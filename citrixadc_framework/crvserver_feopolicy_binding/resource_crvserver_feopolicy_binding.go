...
func (r *CrvserverFeopolicyBindingResource) readCrvserverFeopolicyBindingFromApi(ctx context.Context, data *CrvserverFeopolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "crvserver_feopolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("crvserver_feopolicy_binding not found with the provided ID attributes"))
		return
	}
