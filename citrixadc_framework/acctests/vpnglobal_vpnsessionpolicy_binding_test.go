...
func testAccCheckVpnglobal_vpnsessionpolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Vpnglobal_vpnsessionpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
const testAccVpnglobal_vpnsessionpolicy_bindingDataSource_basic = `
	...
	data "citrixadc_vpnglobal_vpnsessionpolicy_binding" "tf_bind" {
		policyname = citrixadc_vpnglobal_vpnsessionpolicy_binding.tf_bind.policyname
		depends_on = [citrixadc_vpnglobal_vpnsessionpolicy_binding.tf_bind]
	}
`

func TestAccVpnglobal_vpnsessionpolicy_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccVpnglobal_vpnsessionpolicy_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.citrixadc_vpnglobal_vpnsessionpolicy_binding.tf_bind", "policyname", "tf_vpnsessionpolicy"),
...
func TestAccVpnglobal_vpnsessionpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnglobal_vpnsessionpolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnglobal_vpnsessionpolicy_binding_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnglobal_vpnsessionpolicy_bindingExist(resAddr, nil),
				),
			},
			{
				PreConfig: func() {
					...
					if err := client.DeleteResourceWithArgs(service.Vpnglobal_vpnsessionpolicy_binding.Type(), "", []string{"policyname:tf_vpnsessionpolicy"}); err != nil {
