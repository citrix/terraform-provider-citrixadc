...
// sslcrlfile_import is an Import-as-create resource (POST ?action=Import), with
...
const testAccSslcrlfileImport_basic_step1 = `
resource "citrixadc_sslcrlfile_import" "tf_sslcrlfile" {
  name = "tf_sslcrlfile"
...
func TestAccSslcrlfileImport_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { doSslcrlfilePreChecks(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckSslcrlfileImportDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccSslcrlfileImport_basic_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSslcrlfileImportExist("citrixadc_sslcrlfile_import.tf_sslcrlfile", nil),
					resource.TestCheckResourceAttr("citrixadc_sslcrlfile_import.tf_sslcrlfile", "name", "tf_sslcrlfile"),
				),
			},
		},
	})
...
func TestAccSslcrlfileImport_import(t *testing.T) {
	const resAddr = "citrixadc_sslcrlfile_import.tf_sslcrlfile"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { doSslcrlfilePreChecks(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckSslcrlfileImportDestroy,
...
func testAccCheckSslcrlfileImportExist(n string, id *string) resource.TestCheckFunc {
...
func testAccCheckSslcrlfileImportDestroy(s *terraform.State) error {
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	}
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_sslcrlfile_import" {
			continue
		}
