...
// vpnpcoipconnection is an ACTION-ONLY resource: Create performs a POST ?action=kill
// to terminate active PCoIP connections. There is no GET-backed managed object, no
...
func TestAccVpnpcoipconnection_basic(t *testing.T) {
...
// testAccCheckVpnpcoipconnectionExist verifies the resource exists in Terraform state.
//
// IMPORTANT: This check intentionally does NOT call client.FindResource(...).
// vpnpcoipconnection is action-only (kill); the killed connection is not a persistent,
...
func testAccCheckVpnpcoipconnectionExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No vpnpcoipconnection ID is set")
		}
