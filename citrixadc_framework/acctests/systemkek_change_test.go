...
// applying the citrixadc_systemkek_change resource ROTATES the appliance Key
// Encryption Key (KEK) via POST /nitro/v1/config/systemkek?action=update.
//
...
// NOTE on the systemkek_change resource:
...
//     state with its synthetic ID ("systemkek_change"); it does NOT (and cannot)
...
const testAccSystemkekChange_basic_step1 = `
resource "citrixadc_systemkek_change" "tf_systemkek" {
  level = "basic"
}

`

func TestAccSystemkekChange_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSystemkekChange_basic_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSystemkekChangeExist("citrixadc_systemkek_change.tf_systemkek", nil),
					// "id" is the synthetic state handle "systemkek_change".
					resource.TestCheckResourceAttrSet("citrixadc_systemkek_change.tf_systemkek", "id"),
					// Assert the level actually set in HCL.
					resource.TestCheckResourceAttr("citrixadc_systemkek_change.tf_systemkek", "level", "basic"),
				),
			},
		},
	})
...
// testAccCheckSystemkekChangeExist is a state-only existence check.
//
// systemkek_change is an action-only resource: Read is a no-op and there is no
...
// (which equals the synthetic "systemkek_change" after a successful
// POST ?action=update). This mirrors testAccCheckAaasessionExist.
func testAccCheckSystemkekChangeExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No systemkek_change ID is set")
		}
