...
const testAccVpnglobal_domain_binding_basic_step2 = `
...
func TestAccVpnglobal_domain_binding_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccVpnglobal_domain_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnglobal_domain_bindingNotExist("citrixadc_vpnglobal_domain_binding.tf_bind", "http://www.example.com/"),
				),
			},
		},
	})
...
func testAccCheckVpnglobal_domain_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No vpnglobal_domain_binding id is set")
		}
		...
		if !found {
			return fmt.Errorf("vpnglobal_domain_binding %s not found", n)
		}
...
func testAccCheckVpnglobal_domain_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if found {
			return fmt.Errorf("vpnglobal_domain_binding %s was found, but it should have been destroyed", n)
		}
