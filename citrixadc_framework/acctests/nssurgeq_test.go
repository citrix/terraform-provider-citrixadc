...
//     state with its synthetic ID ("nssurgeq-config"); it does NOT (and cannot)
...
const testAccNssurgeq_basic = `
resource "citrixadc_nssurgeq" "tf_nssurgeq" {
}

`

func TestAccNssurgeq_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNssurgeq_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNssurgeqExist("citrixadc_nssurgeq.tf_nssurgeq", nil),
					// "id" is the synthetic state handle "nssurgeq-config".
					resource.TestCheckResourceAttrSet("citrixadc_nssurgeq.tf_nssurgeq", "id"),
				),
			},
		},
	})
...
// testAccCheckNssurgeqExist is a state-only existence check.
//
// nssurgeq is an action-only resource: Read is a no-op and there is no GET-by-id
// endpoint, so we CANNOT verify the flush via NITRO. We only assert that
// Terraform recorded the resource in state with a non-empty ID (which equals the
// synthetic "nssurgeq-config" after a successful POST ?action=flush).
func testAccCheckNssurgeqExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No nssurgeq ID is set")
		}
