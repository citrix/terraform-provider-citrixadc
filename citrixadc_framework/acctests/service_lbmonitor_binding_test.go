...
const testAccService_lbmonitor_binding_basic_step2 = `
...
func TestAccService_lbmonitor_binding_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccService_lbmonitor_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckService_lbmonitor_bindingNotExist("citrixadc_service_lbmonitor_binding.tf_binding", "tf_service,tf_monitor"),
				),
			},
		},
	})
...
func testAccCheckService_lbmonitor_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No service_lbmonitor_binding id is set")
		}
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "monitor_name"}, nil)
		if err != nil {
		...
		if !found {
			return fmt.Errorf("service_lbmonitor_binding %s not found", n)
		}
...
func testAccCheckService_lbmonitor_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		if found {
			return fmt.Errorf("service_lbmonitor_binding %s was found, but it should have been destroyed", n)
		}
...
func TestAccService_lbmonitor_binding_import(t *testing.T) {
	...
	// order: name,monitor_name) so it matches exactly what SDK v2 wrote.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccService_lbmonitor_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func testAccCheckService_lbmonitor_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_service_lbmonitor_binding" {
			continue
		}
		...
		_, err := client.FindResource(service.Service_lbmonitor_binding.Type(), rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("service_lbmonitor_binding %s still exists", rs.Primary.ID)
		}
