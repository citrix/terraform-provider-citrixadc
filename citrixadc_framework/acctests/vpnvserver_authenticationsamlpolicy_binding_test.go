		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("data.citrixadc_vpnvserver_authenticationsamlpolicy_binding.tf_bind", "id"),
				),
			},
		},
	})
...
func TestAccVpnvserver_authenticationsamlpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVpnvserver_authenticationsamlpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"bindpoint", "priority"}},
		},
	})
}
...
func TestAccVpnvserver_authenticationsamlpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_authenticationsamlpolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_authenticationsamlpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckVpnvserver_authenticationsamlpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgs(service.Vpnvserver_authenticationsamlpolicy_binding.Type(), "tf_vserver_examples", []string{"policy:tf_samlpolicy", "bindpoint:RESPONSE"}); err != nil {
