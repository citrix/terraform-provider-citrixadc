...
// NOTE on the lsnsession_flush resource:
//   - Models the NITRO POST /lsnsession?action=flush endpoint.
//   - This is an ACTION-ONLY (one-shot side-effect) flush filter resource:
//     Create performs the flush action with the optional filter selectors that
//     are set; Read is a no-op (preserves state), Update is a no-op (all
//     attributes are RequiresReplace), and Delete is a state-only removal.
//     There is no GET-by-id endpoint that can re-resolve "the flushed
//     sessions", so the resource CANNOT be verified by reading it back from the
//     ADC.
...
//     "255.255.255.255".
//   - The Create uses a CONSTANT synthetic ID ("lsnsession_flush") because
//     there is no get-by-name key on NITRO for this action-only resource.
...
//     state with its synthetic ID ("lsnsession_flush"); it does NOT (and
...
//   - There is NO datasource for lsnsession_flush (no NITRO GET endpoint), so
//     no datasource test is generated.
//
...
const testAccLsnsessionFlush_basic = `
resource "citrixadc_lsnsession_flush" "tf_lsnsession_flush" {
  nattype = "NAT44"
  network = "100.64.0.0"
  netmask = "255.192.0.0"
}
...
func TestAccLsnsessionFlush_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccLsnsessionFlush_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLsnsessionFlushExist("citrixadc_lsnsession_flush.tf_lsnsession_flush", nil),
					// "id" is the synthetic state handle "lsnsession_flush".
					resource.TestCheckResourceAttrSet("citrixadc_lsnsession_flush.tf_lsnsession_flush", "id"),
					// Assert only the filters actually set in HCL.
					resource.TestCheckResourceAttr("citrixadc_lsnsession_flush.tf_lsnsession_flush", "nattype", "NAT44"),
					resource.TestCheckResourceAttr("citrixadc_lsnsession_flush.tf_lsnsession_flush", "network", "100.64.0.0"),
					resource.TestCheckResourceAttr("citrixadc_lsnsession_flush.tf_lsnsession_flush", "netmask", "255.192.0.0"),
				),
			},
		},
	})
...
// testAccCheckLsnsessionFlushExist is a state-only existence check.
//
// lsnsession_flush is an action-only flush resource: Read is a no-op and there
// is no GET-by-id endpoint, so we CANNOT verify the flush via NITRO. We only
// assert that Terraform recorded the resource in state with a non-empty ID
// (which equals the synthetic "lsnsession_flush" after a successful POST
// ?action=flush). This mirrors testAccCheckAaasessionExist.
func testAccCheckLsnsessionFlushExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No lsnsession_flush ID is set")
		}
