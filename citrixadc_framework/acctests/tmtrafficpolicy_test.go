...
func testAccCheckTmtrafficpolicyExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		data, err := client.FindResource(service.Tmtrafficpolicy.Type(), rs.Primary.ID)
...
func testAccCheckTmtrafficpolicyDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Tmtrafficpolicy.Type(), rs.Primary.ID)
		if err == nil {
...
data "citrixadc_tmtrafficpolicy" "tf_tmtrafficpolicy" {
    name = citrixadc_tmtrafficpolicy.tf_tmtrafficpolicy.name
}
`

func TestAccTmtrafficpolicyDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccTmtrafficpolicyDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
