...
//     state with its synthetic ID ("nsstats-config"); it does NOT (and cannot)
...
const testAccNsstats_basic = `
resource "citrixadc_nsstats" "tf_nsstats" {
  cleanuplevel = "global"
}

`

func TestAccNsstats_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNsstats_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNsstatsExist("citrixadc_nsstats.tf_nsstats", nil),
					resource.TestCheckResourceAttr("citrixadc_nsstats.tf_nsstats", "cleanuplevel", "global"),
					// "id" is the synthetic state handle "nsstats-config".
					resource.TestCheckResourceAttrSet("citrixadc_nsstats.tf_nsstats", "id"),
				),
			},
		},
	})
...
// testAccCheckNsstatsExist is a state-only existence check.
//
// nsstats is an action-only resource: Read is a no-op and there is no GET-by-id
// endpoint, so we CANNOT verify the clear via NITRO. We only assert that
// Terraform recorded the resource in state with a non-empty ID (which equals the
// synthetic "nsstats-config" after a successful POST ?action=clear).
func testAccCheckNsstatsExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No nsstats ID is set")
		}
