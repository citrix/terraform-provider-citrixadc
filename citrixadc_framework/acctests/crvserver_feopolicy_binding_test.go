...
func TestAccCrvserver_feopolicy_binding_selfHealing(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCrvserver_feopolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckCrvserver_feopolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					...
					if err := client.DeleteResourceWithArgs(service.Crvserver_feopolicy_binding.Type(), "my_vserver", []string{"policyname:tf_feopolicy", "priority:10"}); err != nil {
...
func TestAccCrvserver_feopolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccCrvserver_feopolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func testAccCheckCrvserver_feopolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Crvserver_feopolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
const testAccCrvserver_feopolicy_bindingDataSource_basic = `
	...
	data "citrixadc_crvserver_feopolicy_binding" "crvserver_feopolicy_binding" {
		name       = citrixadc_crvserver_feopolicy_binding.crvserver_feopolicy_binding.name
		policyname = citrixadc_crvserver_feopolicy_binding.crvserver_feopolicy_binding.policyname
		depends_on = [citrixadc_crvserver_feopolicy_binding.crvserver_feopolicy_binding]
	}
`

func TestAcccrvserver_feopolicy_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             nil,
		Steps: []resource.TestStep{
			{
				Config: testAccCrvserver_feopolicy_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
