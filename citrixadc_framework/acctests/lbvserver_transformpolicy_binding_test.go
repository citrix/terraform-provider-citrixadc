...
func TestAccLbvserver_transformpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccLbvserver_transformpolicy_binding_basic_step1, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
// TestAccLbvserver_transformpolicy_binding_selfHealing verifies drift recovery: after the
// binding is deleted out-of-band, re-applying the same config recreates it.
func TestAccLbvserver_transformpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_lbvserver_transformpolicy_binding.tf_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccLbvserver_transformpolicy_binding_basic_step1,
				Check:  resource.ComposeTestCheckFunc(testAccCheckLbvserver_transformpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Lbvserver_transformpolicy_binding.Type(), "tf_lbvserver", map[string]string{"policyname": "tf_trans_policy", "bindpoint": "REQUEST", "priority": "100"}); err != nil {
