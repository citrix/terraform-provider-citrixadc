...
const testAccAppfwprofile_xmlwsiurl_binding_upgrade_basic = `
	resource "citrixadc_appfwprofile" "tf_appfwprofile" {
		name = "tf_appfwprofile"
		type = ["HTML"]
	}
...
// TestAccAppfwprofile_xmlwsiurl_binding_sdkv2StateUpgrade verifies that state
...
// legacy id "tf_appfwprofile,.*"). Step 2 refreshes/plans/applies the same config
...
// new "key:value" form.
func TestAccAppfwprofile_xmlwsiurl_binding_import(t *testing.T) {
	const resAddr = "citrixadc_appfwprofile_xmlwsiurl_binding.tf_binding"

	// Backward-compat: import via the LEGACY SDK v2 id. Rebuild the legacy positional id from
	// the current canonical key:value id (raw values, only the keys actually set, in legacy
	// order: name,xmlwsiurl) so it matches exactly what SDK v2 wrote.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{Config: testAccAppfwprofile_xmlwsiurl_binding_basic},
			{Config: testAccAppfwprofile_xmlwsiurl_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"alertonly", "isautodeployed", "xmlwsichecks"}},
			{Config: testAccAppfwprofile_xmlwsiurl_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"alertonly", "isautodeployed", "xmlwsichecks"}},
		},
	})
}

func TestAccAppfwprofile_xmlwsiurl_binding_sdkv2StateUpgrade(t *testing.T) {
	resourceAddr := "citrixadc_appfwprofile_xmlwsiurl_binding.tf_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		CheckDestroy: testAccCheckAppfwprofile_xmlwsiurl_bindingDestroy,
		Steps: []resource.TestStep{
			// Step 1: create with the last SDK v2 release -> state carries the legacy id.
			{
				...
				Config: testAccAppfwprofile_xmlwsiurl_binding_upgrade_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAppfwprofile_xmlwsiurl_bindingExist(resourceAddr, nil),
			...
			// recomputed to the new key:value format.
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAppfwprofile_xmlwsiurl_bindingExist(resourceAddr, nil),
					resource.TestCheckResourceAttr(resourceAddr, "id", "name:tf_appfwprofile,xmlwsiurl:.%2A"),
				),
			},
		},
	})
...
func TestAccAppfwprofile_xmlwsiurl_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_appfwprofile_xmlwsiurl_binding.tf_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAppfwprofile_xmlwsiurl_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckAppfwprofile_xmlwsiurl_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgs(service.Appfwprofile_xmlwsiurl_binding.Type(), "tf_appfwprofile", []string{"xmlwsiurl:.%2A"}); err != nil {
