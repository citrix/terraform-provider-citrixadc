...
func TestAccGslbservice_lbmonitor_binding_import(t *testing.T) {
	...
	// order: servicename,monitor_name) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"servicename", "monitor_name"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccGslbservice_lbmonitor_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func testAccCheckGslbservice_lbmonitor_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"servicename", "monitor_name"}, nil)
		if err != nil {
...
func TestAccGslbservice_lbmonitor_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_gslbservice_lbmonitor_binding.tf_gslbservice_lbmonitor_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGslbservice_lbmonitor_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckGslbservice_lbmonitor_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Gslbservice_lbmonitor_binding.Type(), "tf_gslb1vservice", map[string]string{"monitor_name": "tf_monitor"}); err != nil {
