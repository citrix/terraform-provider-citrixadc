...
// NOTE on the clusterpropstatus resource:
...
//     state with its synthetic ID ("clusterpropstatus"); it does NOT (and cannot)
...
const testAccClusterpropstatus_basic = `
resource "citrixadc_clusterpropstatus" "tf_clusterpropstatus" {
}

`

func TestAccClusterpropstatus_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccClusterpropstatus_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckClusterpropstatusExist("citrixadc_clusterpropstatus.tf_clusterpropstatus", nil),
					// "id" is the synthetic state handle "clusterpropstatus".
					resource.TestCheckResourceAttrSet("citrixadc_clusterpropstatus.tf_clusterpropstatus", "id"),
				),
			},
		},
	})
...
// testAccCheckClusterpropstatusExist is a state-only existence check.
//
// clusterpropstatus is an action-only resource: Read is a no-op and there is no
...
// equals the synthetic "clusterpropstatus" after a successful POST ?action=clear).
// This mirrors testAccCheckAaasessionExist / testAccCheckClusterfilesActionExist.
func testAccCheckClusterpropstatusExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No clusterpropstatus ID is set")
		}
