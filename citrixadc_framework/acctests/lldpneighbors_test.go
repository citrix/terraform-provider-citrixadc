...
// NOTE on the lldpneighbors resource:
...
//   - The RESOURCE therefore fires ?action=clear on Create with an EMPTY payload
//     ({"lldpneighbors":{}}) and NO args, and treats Read/Update/Delete as
//     no-ops. It records a FIXED synthetic Terraform state ID "lldpneighbors".
//     Because there is no GET-by-id endpoint, the resource CANNOT be verified by
//     reading it back; the Exist check below only asserts the synthetic state ID
//     is set (mirrors ipsecalgsession_test.go / cacheobject_test.go).
...
const testAccLldpneighbors_basic_step1 = `
resource "citrixadc_lldpneighbors" "tf_lldpneighbors" {
}

`

func TestAccLldpneighbors_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		// No CheckDestroy: lldpneighbors has no delete/GET-by-id endpoint (action-only).
		Steps: []resource.TestStep{
			{
				Config: testAccLldpneighbors_basic_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLldpneighborsExist("citrixadc_lldpneighbors.tf_lldpneighbors", nil),
					// Fixed synthetic ID recorded after a successful ?action=clear.
					resource.TestCheckResourceAttrSet("citrixadc_lldpneighbors.tf_lldpneighbors", "id"),
					resource.TestCheckResourceAttr("citrixadc_lldpneighbors.tf_lldpneighbors", "id", "lldpneighbors"),
				),
			},
		},
	})
...
// testAccCheckLldpneighborsExist is a state-only existence check.
//
// lldpneighbors is an action-only resource: Read is a no-op and there is no
...
// (the fixed synthetic "lldpneighbors" after a successful POST ?action=clear).
// This mirrors testAccCheckIpsecalgsessionExist.
func testAccCheckLldpneighborsExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No lldpneighbors ID is set")
		}
