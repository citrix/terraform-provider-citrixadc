...
func testAccCheckVpnvserver_appflowpolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "policy", "bindpoint"}, nil)
		if err != nil {
...
func testAccCheckVpnvserver_appflowpolicy_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(id, []string{"name", "policy", "bindpoint"}, nil)
		if err != nil {
...
func testAccCheckVpnvserver_appflowpolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Vpnvserver_appflowpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
func TestAccVpnvserver_appflowpolicy_binding_import(t *testing.T) {
	...
	// order: name,policy,bindpoint) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"name", "policy", "bindpoint"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVpnvserver_appflowpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}

func TestAccVpnvserver_appflowpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_appflowpolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_appflowpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckVpnvserver_appflowpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgs(service.Vpnvserver_appflowpolicy_binding.Type(), "tf_vpnvserver", []string{"policy:tf_appflowpolicy", "bindpoint:ICA_REQUEST"}); err != nil {
