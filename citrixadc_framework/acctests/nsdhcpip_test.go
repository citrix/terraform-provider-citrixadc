...
//     with its synthetic id ("nsdhcpip-config"); it cannot verify the side
//     effect via NITRO.
//
...
const testAccNsdhcpip_basic = `
resource "citrixadc_nsdhcpip" "tf_nsdhcpip" {
}
`

func TestAccNsdhcpip_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNsdhcpip_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNsdhcpipExist("citrixadc_nsdhcpip.tf_nsdhcpip", nil),
					// "id" is the synthetic state handle "nsdhcpip-config".
					resource.TestCheckResourceAttrSet("citrixadc_nsdhcpip.tf_nsdhcpip", "id"),
				),
			},
		},
	})
...
// testAccCheckNsdhcpipExist is a state-only existence check. nsdhcpip is an
...
func testAccCheckNsdhcpipExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No nsdhcpip ID is set")
		}
