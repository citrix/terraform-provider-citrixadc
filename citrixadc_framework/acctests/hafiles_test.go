...
//     state with its synthetic ID ("hafiles"); it does NOT (and cannot) verify
...
//
// ENVIRONMENT CAVEAT: hafiles sync only works on a node that is part of an HA
// pair. On a standalone testbed the sync action errors out. Following the
// repo's HA-test convention (see hafailover_test.go and hanode_test.go), this
// test is gated on ADC_TESTBED == "HA_PAIR" and skipped otherwise.
...
const testAccHafiles_basic = `
resource "citrixadc_hafiles" "tf_hafiles" {
  mode = ["all"]
}

`

func TestAccHafiles_basic(t *testing.T) {
	if adcTestbed != "HA_PAIR" {
		t.Skipf("ADC testbed is %s. Expected HA_PAIR (hafiles sync requires an HA pair).", adcTestbed)
	}
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccHafiles_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckHafilesExist("citrixadc_hafiles.tf_hafiles", nil),
					// "id" is the synthetic state handle "hafiles".
					resource.TestCheckResourceAttrSet("citrixadc_hafiles.tf_hafiles", "id"),
					// Assert the mode list actually set in HCL.
					resource.TestCheckResourceAttr("citrixadc_hafiles.tf_hafiles", "mode.#", "1"),
					resource.TestCheckResourceAttr("citrixadc_hafiles.tf_hafiles", "mode.0", "all"),
				),
			},
		},
	})
...
// testAccCheckHafilesExist is a state-only existence check.
//
// hafiles is an action-only resource: Read is a no-op and there is no GET-by-id
// endpoint, so we CANNOT verify the sync via NITRO. We only assert that
// Terraform recorded the resource in state with a non-empty ID (which equals
// the synthetic "hafiles" after a successful POST ?action=sync). This mirrors
// testAccCheckClusterfilesActionExist and testAccCheckAaasessionExist.
func testAccCheckHafilesExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No hafiles ID is set")
		}
