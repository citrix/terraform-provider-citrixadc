...
func TestAccVpnglobal_authenticationldappolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnglobal_authenticationldappolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnglobal_authenticationldappolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckVpnglobal_authenticationldappolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgs(service.Vpnglobal_authenticationldappolicy_binding.Type(), "", []string{"policyname:tf_ldappolicy"}); err != nil {
