...
func testAccCheckGslbvserver_spilloverpolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Gslbvserver_spilloverpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
const testAccGslbvserver_spilloverpolicy_bindingDataSource_basic = `
...
data "citrixadc_gslbvserver_spilloverpolicy_binding" "tf_gslbvserver_spilloverpolicy_binding" {
	name       = citrixadc_gslbvserver_spilloverpolicy_binding.tf_gslbvserver_spilloverpolicy_binding.name
	policyname = citrixadc_gslbvserver_spilloverpolicy_binding.tf_gslbvserver_spilloverpolicy_binding.policyname
}
`

func TestAccGslbvserver_spilloverpolicy_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccGslbvserver_spilloverpolicy_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.citrixadc_gslbvserver_spilloverpolicy_binding.tf_gslbvserver_spilloverpolicy_binding", "name", "gslb_vserver"),
					resource.TestCheckResourceAttr("data.citrixadc_gslbvserver_spilloverpolicy_binding.tf_gslbvserver_spilloverpolicy_binding", "policyname", "tf_spilloverpolicy"),
					resource.TestCheckResourceAttr("data.citrixadc_gslbvserver_spilloverpolicy_binding.tf_gslbvserver_spilloverpolicy_binding", "priority", "100"),
				),
			},
		},
	})
...
func TestAccGslbvserver_spilloverpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccGslbvserver_spilloverpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}

// TestAccGslbvserver_spilloverpolicy_binding_selfHealing verifies drift recovery: after the
...
func TestAccGslbvserver_spilloverpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_gslbvserver_spilloverpolicy_binding.tf_gslbvserver_spilloverpolicy_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGslbvserver_spilloverpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckGslbvserver_spilloverpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Gslbvserver_spilloverpolicy_binding.Type(), "gslb_vserver", map[string]string{"policyname": "tf_spilloverpolicy"}); err != nil {
