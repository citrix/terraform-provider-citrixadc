...
const testAccSystemgroup_nspartition_binding_basic_step2 = `
...
func TestAccSystemgroup_nspartition_binding_import(t *testing.T) {
	...
	// order: groupname,partitionname) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"groupname", "partitionname"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccSystemgroup_nspartition_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSystemgroup_nspartition_bindingExist("citrixadc_systemgroup_nspartition_binding.tf_systemgroup_nspartition_binding", nil),
				),
			},
			{
				Config: testAccSystemgroup_nspartition_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSystemgroup_nspartition_bindingNotExist("citrixadc_systemgroup_nspartition_binding.tf_systemgroup_nspartition_binding", "tf_systemgroup,tf_nspartition"),
				),
			},
		},
	})
...
func testAccCheckSystemgroup_nspartition_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No systemgroup_nspartition_binding id is set")
		}
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"groupname", "partitionname"}, nil)
		if err != nil {
		...
		if !found {
			return fmt.Errorf("systemgroup_nspartition_binding %s not found", n)
		}
...
func testAccCheckSystemgroup_nspartition_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		idMap, _, err := utils.ParseIdString(id, []string{"groupname", "partitionname"}, nil)
		if err != nil {
		...
		if found {
			return fmt.Errorf("systemgroup_nspartition_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckSystemgroup_nspartition_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_systemgroup_nspartition_binding" {
			continue
		}
		...
		_, err := client.FindResource("systemgroup_nspartition_binding", rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("systemgroup_nspartition_binding %s still exists", rs.Primary.ID)
		}
