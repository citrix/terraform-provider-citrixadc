...
func TestAccClusternodegroup_lbvserver_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccClusternodegroup_lbvserver_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func TestAccClusternodegroup_lbvserver_binding_selfHealing(t *testing.T) {
	if adcTestbed != "CLUSTER" {
		t.Skipf("ADC testbed is %s. Expected CLUSTER.", adcTestbed)
	}
	const resAddr = "citrixadc_clusternodegroup_lbvserver_binding.tf_clusternodegroup_lbvserver_binding"
		...
		Steps: []resource.TestStep{
			{
				Config: testAccClusternodegroup_lbvserver_binding_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckClusternodegroup_lbvserver_bindingExist(resAddr, nil),
				),
			},
			{
				PreConfig: func() {
					...
					if err := client.DeleteResourceWithArgsMap(service.Clusternodegroup_lbvserver_binding.Type(), "my_tf_group", map[string]string{"vserver": "my_lb_vserver_ds"}); err != nil {
