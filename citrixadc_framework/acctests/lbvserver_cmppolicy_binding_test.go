...
func TestAccLbvserver_cmppolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccLbvserver_cmppolicy_binding_basic_step1, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func TestAccLbvserver_cmppolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_lbvserver_cmppolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccLbvserver_cmppolicy_binding_basic_step1,
				Check:  resource.ComposeTestCheckFunc(testAccCheckLbvserver_cmppolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Lbvserver_cmppolicy_binding.Type(), "tf_lbvserver", map[string]string{"policyname": "tf_cmppolicy", "bindpoint": "RESPONSE", "priority": "100"}); err != nil {
