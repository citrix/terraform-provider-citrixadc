...
// ssldhfile_import is an Import-as-create resource (POST ?action=Import), with no
...
const testAccSsldhfileImport_basic_step1 = `
resource "citrixadc_ssldhfile_import" "tf_ssldhfile" {
  name = "tf_ssldhfile"
...
func TestAccSsldhfileImport_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { doSsldhfilePreChecks(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckSsldhfileImportDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccSsldhfileImport_basic_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSsldhfileImportExist("citrixadc_ssldhfile_import.tf_ssldhfile", nil),
					resource.TestCheckResourceAttr("citrixadc_ssldhfile_import.tf_ssldhfile", "name", "tf_ssldhfile"),
				),
			},
		},
	})
...
func testAccCheckSsldhfileImportExist(n string, id *string) resource.TestCheckFunc {
...
func testAccCheckSsldhfileImportDestroy(s *terraform.State) error {
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	}
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_ssldhfile_import" {
			continue
		}
...
func TestAccSsldhfileImport_import(t *testing.T) {
	const resAddr = "citrixadc_ssldhfile_import.tf_ssldhfile"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { doSsldhfilePreChecks(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckSsldhfileImportDestroy,
