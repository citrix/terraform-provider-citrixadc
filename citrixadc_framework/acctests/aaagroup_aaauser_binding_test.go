...
func TestAccAaagroup_aaauser_binding_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccAaagroup_aaauser_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAaagroup_aaauser_bindingNotExist("citrixadc_aaagroup_aaauser_binding.tf_aaagroup_aaauser_binding", "my_group,user1"),
				),
			},
		},
	})
...
func testAccCheckAaagroup_aaauser_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No aaagroup_aaauser_binding id is set")
		}
		...
		// Iterate through results to find the one with the matching username
		found := false
		...
		if !found {
			return fmt.Errorf("aaagroup_aaauser_binding %s not found", n)
		}
...
func testAccCheckAaagroup_aaauser_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		// Iterate through results to hopefully not find the one with the matching username
		found := false
		for _, v := range dataArr {
			if v["username"].(string) == username {
				found = true
		...
		if found {
			return fmt.Errorf("aaagroup_aaauser_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckAaagroup_aaauser_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_aaagroup_aaauser_binding" {
			continue
		}
		...
		_, err := client.FindResource(service.Aaagroup_aaauser_binding.Type(), rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("aaagroup_aaauser_binding %s still exists", rs.Primary.ID)
		}
...
func TestAccAaagroup_aaauser_binding_import(t *testing.T) {
	...
	// order: groupname,username) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"groupname", "username"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccAaagroup_aaauser_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
