...
// NOTE on the ssldefaultprofile resource:
...
//     with its synthetic id ("ssldefaultprofile-config"); it cannot verify the side
//     effect via NITRO.
//
...
const testAccSsldefaultprofile_basic = `
resource "citrixadc_ssldefaultprofile" "tf_ssldefaultprofile" {
}
`

func TestAccSsldefaultprofile_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSsldefaultprofile_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSsldefaultprofileExist("citrixadc_ssldefaultprofile.tf_ssldefaultprofile", nil),
					// "id" is the synthetic state handle "ssldefaultprofile-config".
					resource.TestCheckResourceAttrSet("citrixadc_ssldefaultprofile.tf_ssldefaultprofile", "id"),
				),
			},
		},
	})
...
// testAccCheckSsldefaultprofileExist is a state-only existence check.
// ssldefaultprofile is an action-only resource with no GET-by-id endpoint, so we
// only assert Terraform recorded the resource in state with a non-empty ID.
func testAccCheckSsldefaultprofileExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No ssldefaultprofile ID is set")
		}
