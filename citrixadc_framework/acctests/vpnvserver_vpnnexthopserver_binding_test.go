...
const testAccVpnvserver_vpnnexthopserver_binding_upgrade_basic = `
	resource "citrixadc_vpnvserver" "tf_vpnvserver" {
		name        = "tf_exampleserver"
		servicetype = "SSL"
		ipv46       = "3.3.3.3"
...
// TestAccVpnvserver_vpnnexthopserver_binding_sdkv2StateUpgrade verifies that a
...
func TestAccVpnvserver_vpnnexthopserver_binding_sdkv2StateUpgrade(t *testing.T) {
	resourceAddr := "citrixadc_vpnvserver_vpnnexthopserver_binding.tf_bind"
	legacyId := "tf_exampleserver,tf_vpnnexthopserver"
	newId := "name:tf_exampleserver,nexthopserver:tf_vpnnexthopserver"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			// Step 1: create with the last SDK v2 release, writing legacy-id state.
			{
				...
				Config: testAccVpnvserver_vpnnexthopserver_binding_upgrade_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnvserver_vpnnexthopserver_bindingExist(resourceAddr, nil),
...
func testAccCheckVpnvserver_vpnnexthopserver_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "nexthopserver"}, nil)
		if err != nil {
			return err
		}
		name := idMap["name"]
		nexthopserver := idMap["nexthopserver"]
...
func testAccCheckVpnvserver_vpnnexthopserver_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(id, []string{"name", "nexthopserver"}, nil)
		if err != nil {
			return err
		}
		name := idMap["name"]
...
func testAccCheckVpnvserver_vpnnexthopserver_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Vpnvserver_vpnnexthopserver_binding.Type(), rs.Primary.ID)
		if err == nil {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("data.citrixadc_vpnvserver_vpnnexthopserver_binding.tf_bind", "id"),
				),
			},
		},
	})
...
func TestAccVpnvserver_vpnnexthopserver_binding_import(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_vpnnexthopserver_binding.tf_bind"

	// Backward-compat: import via the LEGACY SDK v2 id. Rebuild the legacy positional id from
	// the current canonical key:value id (raw values, only the keys actually set, in legacy
	// order: name,nexthopserver) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"name", "nexthopserver"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{Config: testAccVpnvserver_vpnnexthopserver_binding_basic},
			{Config: testAccVpnvserver_vpnnexthopserver_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
			{Config: testAccVpnvserver_vpnnexthopserver_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}

func TestAccVpnvserver_vpnnexthopserver_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_vpnnexthopserver_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_vpnnexthopserver_binding_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnvserver_vpnnexthopserver_bindingExist(resAddr, nil),
				),
			},
			{
				PreConfig: func() {
					...
					if err := client.DeleteResourceWithArgsMap(service.Vpnvserver_vpnnexthopserver_binding.Type(), "tf_exampleserver", map[string]string{"nexthopserver": "tf_vpnnexthopserver"}); err != nil {
