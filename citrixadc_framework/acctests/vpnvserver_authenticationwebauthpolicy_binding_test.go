...
const testAccVpnvserver_authenticationwebauthpolicy_binding_basic_step2 = `
...
func testAccCheckVpnvserver_authenticationwebauthpolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No vpnvserver_authenticationwebauthpolicy_binding id is set")
		}
		...
		if !found {
			return fmt.Errorf("vpnvserver_authenticationwebauthpolicy_binding %s not found", n)
		}
...
func testAccCheckVpnvserver_authenticationwebauthpolicy_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		if found {
			return fmt.Errorf("vpnvserver_authenticationwebauthpolicy_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckVpnvserver_authenticationwebauthpolicy_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_vpnvserver_authenticationwebauthpolicy_binding" {
			continue
		}
		...
		_, err := client.FindResource(service.Vpnvserver_authenticationwebauthpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("vpnvserver_authenticationwebauthpolicy_binding %s still exists", rs.Primary.ID)
		}
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttrSet("data.citrixadc_vpnvserver_authenticationwebauthpolicy_binding.tf_bind", "id"),
				),
			},
		},
	})
...
func TestAccVpnvserver_authenticationwebauthpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVpnvserver_authenticationwebauthpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"bindpoint"}},
		},
	})
}

func TestAccVpnvserver_authenticationwebauthpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_authenticationwebauthpolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_authenticationwebauthpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckVpnvserver_authenticationwebauthpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgs(service.Vpnvserver_authenticationwebauthpolicy_binding.Type(), "tf_examplevserver", []string{"policy:tf_webauthpolicy", "bindpoint:OTHERTCP_REQUEST"}); err != nil {
