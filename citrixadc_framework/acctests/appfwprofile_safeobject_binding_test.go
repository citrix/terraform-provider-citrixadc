...
func testAccCheckAppfwprofile_safeobject_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "safeobject"}, nil)
		if err != nil {
			return fmt.Errorf("Error parsing ID %s: %v", bindingId, err)
		}
		name := idMap["name"]
		safeobject := idMap["safeobject"]
...
func testAccCheckAppfwprofile_safeobject_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Appfwprofile_safeobject_binding.Type(), rs.Primary.ID)
		if err == nil {
...
func TestAccAppfwprofile_safeobject_binding_import(t *testing.T) {
	...
	// order: name,safeobject) so it matches exactly what SDK v2 wrote.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccAppfwprofile_safeobject_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"alertonly", "isautodeployed"}},
		},
	})
}
