...
// sslpkcs12_convert is an ACTION-ONLY resource: NITRO exposes only ?action=convert
...
const testAccSslpkcs12Convert_basic = `
	...
	resource "citrixadc_sslpkcs12_convert" "tf_sslpkcs12" {
		export = true
...
func TestAccSslpkcs12Convert_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSslpkcs12Convert_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSslpkcs12ConvertExist("citrixadc_sslpkcs12_convert.tf_sslpkcs12", nil),
					// Only assert HCL-set / echoed non-secret attributes.
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12", "export", "true"),
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12", "keyfile", "/nsconfig/ssl/servercert1.key"),
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12", "outfile", "tf_sslpkcs12_out.pem"),
				),
			},
		},
	})
...
const testAccSslpkcs12Convert_password_basic = `
	...
	resource "citrixadc_sslpkcs12_convert" "tf_sslpkcs12_pw" {
		export = true
...
func TestAccSslpkcs12Convert_password(t *testing.T) {
	t.Skip("Requires clean up of pkcs12file/outfile from ADC file system or unique pkcs12file/outfile has to be provided.")
	t.Setenv("TF_VAR_sslpkcs12_password", "p12password")
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSslpkcs12Convert_password_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSslpkcs12ConvertExist("citrixadc_sslpkcs12_convert.tf_sslpkcs12_pw", nil),
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12_pw", "keyfile", "/nsconfig/ssl/servercert3.key"),
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12_pw", "outfile", "tf_sslpkcs12_pw_out.pem"),
				),
			},
		},
	})
...
// TestAccSslpkcs8_wo. Every sslpkcs12_convert attribute is RequiresReplace
// (action-only convert), so each step re-runs the convert. Step 1 uses
// servercert2.key (pass phrase "123456"); step 2 rotates to servercert3.key
// (pass phrase "1234567"). The write-only secrets are never stored in state or
// asserted.
const testAccSslpkcs12Convert_wo_step1 = `
	...
	resource "citrixadc_sslpkcs12_convert" "tf_sslpkcs12_wo" {
		export = true
...
func TestAccSslpkcs12Convert_wo(t *testing.T) {
	t.Skip("Requires clean up of pkcs12file/outfile from ADC file system or unique pkcs12file/outfile has to be provided.")
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSslpkcs12Convert_wo_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSslpkcs12ConvertExist("citrixadc_sslpkcs12_convert.tf_sslpkcs12_wo", nil),
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12_wo", "keyfile", "/nsconfig/ssl/servercert2.key"),
					resource.TestCheckResourceAttr("citrixadc_sslpkcs12_convert.tf_sslpkcs12_wo", "pempassphrase_wo_version", "1"),
				),
			},
		},
	})
...
// testAccCheckSslpkcs12ConvertExist is STATE-ONLY: sslpkcs12_convert has no NITRO
// GET endpoint, so there is no FindResource call. It only verifies the Terraform
// ID is set (the convert action ran and produced state).
func testAccCheckSslpkcs12ConvertExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No sslpkcs12_convert ID is set")
		}
		...
		// No FindResource call: sslpkcs12_convert is action-only (?action=convert)
