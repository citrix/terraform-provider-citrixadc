...
func TestAccBotprofile_blacklist_binding_import(t *testing.T) {
	...
	// order: name,bot_blacklist_value) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"name", "bot_blacklist_value"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccBotprofile_blacklist_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
const testAccBotprofile_blacklist_bindingDataSource_basic = `
	resource "citrixadc_botprofile" "tf_botprofile" {
		name                     = "tf_botprofile"
		errorurl                 = "http://www.citrix.com"
		trapurl                  = "/http://www.citrix.com"
	...
	data "citrixadc_botprofile_blacklist_binding" "tf_binding" {
		name                = citrixadc_botprofile_blacklist_binding.tf_binding.name
		bot_blacklist_value = citrixadc_botprofile_blacklist_binding.tf_binding.bot_blacklist_value
	}
`

func TestAccBotprofileBlacklistBindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccBotprofile_blacklist_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.citrixadc_botprofile_blacklist_binding.tf_binding", "name", "tf_botprofile"),
					resource.TestCheckResourceAttr("data.citrixadc_botprofile_blacklist_binding.tf_binding", "bot_blacklist", "true"),
					resource.TestCheckResourceAttr("data.citrixadc_botprofile_blacklist_binding.tf_binding", "bot_blacklist_type", "IPv4"),
					resource.TestCheckResourceAttr("data.citrixadc_botprofile_blacklist_binding.tf_binding", "bot_blacklist_value", "1.3.5.7"),
