...
// NOTE on the dnssubnetcache_flush resource:
...
//     state with its synthetic ID ("dnssubnetcache_flush"); it does NOT (and
...
// required).
const testAccDnssubnetcacheFlush_basic = `
resource "citrixadc_dnssubnetcache_flush" "tf_dnssubnetcache_flush" {
  all = true
}

`

func TestAccDnssubnetcacheFlush_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDnssubnetcacheFlush_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDnssubnetcacheFlushExist("citrixadc_dnssubnetcache_flush.tf_dnssubnetcache_flush", nil),
					resource.TestCheckResourceAttr("citrixadc_dnssubnetcache_flush.tf_dnssubnetcache_flush", "all", "true"),
					// "id" is the synthetic state handle "dnssubnetcache_flush".
					resource.TestCheckResourceAttrSet("citrixadc_dnssubnetcache_flush.tf_dnssubnetcache_flush", "id"),
				),
			},
		},
	})
...
// testAccCheckDnssubnetcacheFlushExist is a state-only existence check.
//
// dnssubnetcache_flush is an action-only resource: Read is a no-op and there is
// no GET-by-id endpoint, so we CANNOT verify the flush via NITRO. We only assert
// that Terraform recorded the resource in state with a non-empty ID (which
// equals the synthetic "dnssubnetcache_flush" after a successful POST
// ?action=flush). This mirrors testAccCheckDnsproxyrecordsExist.
func testAccCheckDnssubnetcacheFlushExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No dnssubnetcache_flush ID is set")
		}
