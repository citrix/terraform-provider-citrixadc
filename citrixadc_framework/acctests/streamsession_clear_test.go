...
// NOTE on the streamsession_clear resource:
...
//     state with its synthetic ID ("streamsession_clear"); it does NOT (and cannot)
...
// and the streamsession_clear clears that identifier's session by name.
const testAccStreamsessionClear_basic = `
...
resource "citrixadc_streamsession_clear" "tf_streamsession" {
...
func TestAccStreamsessionClear_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccStreamsessionClear_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckStreamsessionClearExist("citrixadc_streamsession_clear.tf_streamsession", nil),
					resource.TestCheckResourceAttr("citrixadc_streamsession_clear.tf_streamsession", "name", "my_streamidentifier"),
					// "id" is the synthetic state handle "streamsession_clear".
					resource.TestCheckResourceAttrSet("citrixadc_streamsession_clear.tf_streamsession", "id"),
				),
			},
		},
	})
...
// testAccCheckStreamsessionClearExist is a state-only existence check.
//
// streamsession_clear is an action-only resource: Read is a no-op and there is no
...
// synthetic "streamsession_clear" after a successful POST ?action=clear). This
// mirrors testAccCheckRnatsessionFlushExist.
func testAccCheckStreamsessionClearExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No streamsession_clear ID is set")
		}
