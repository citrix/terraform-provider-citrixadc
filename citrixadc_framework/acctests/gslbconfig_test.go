...
// NOTE on the gslbconfig resource:
...
//     datasource was REMOVED (no NITRO GET endpoint exists). gslbconfig is the
...
//     (`resource "citrixadc_gslbconfig" "tf_gslbconfig" {}`) running
//     "sync gslb config" with no args is equally valid.
//   - The Exist check below only verifies that the resource landed in Terraform
//     state with its synthetic ID ("gslbconfig"); it does NOT (and cannot) verify
...
const testAccGslbconfig_basic = `
resource "citrixadc_gslbconfig" "tf_gslbconfig" {
  debug = true
}

`

func TestAccGslbconfig_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccGslbconfig_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckGslbconfigExist("citrixadc_gslbconfig.tf_gslbconfig", nil),
					// "id" is the synthetic state handle "gslbconfig".
					resource.TestCheckResourceAttrSet("citrixadc_gslbconfig.tf_gslbconfig", "id"),
					// Assert the flag actually set in HCL.
					resource.TestCheckResourceAttr("citrixadc_gslbconfig.tf_gslbconfig", "debug", "true"),
				),
			},
		},
	})
...
// testAccCheckGslbconfigExist is a state-only existence check.
//
// gslbconfig is an action-only resource: Read is a no-op and there is no
...
// synthetic "gslbconfig" after a successful POST ?action=sync). This mirrors
// testAccCheckHafilesExist and testAccCheckClusterfilesActionExist.
func testAccCheckGslbconfigExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No gslbconfig ID is set")
		}
