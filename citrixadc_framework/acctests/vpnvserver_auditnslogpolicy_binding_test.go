...
func testAccCheckVpnvserver_auditnslogpolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Vpnvserver_auditnslogpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
const testAccVpnvserver_auditnslogpolicy_bindingDataSource_basic = `
	...
	data "citrixadc_vpnvserver_auditnslogpolicy_binding" "tf_bind" {
		name     = citrixadc_vpnvserver.tf_vpnvserver.name
		policy   = citrixadc_auditnslogpolicy.tf_auditnslogpolicy.name
		depends_on = [citrixadc_vpnvserver_auditnslogpolicy_binding.tf_bind]
	}
`

func TestAccVpnvserver_auditnslogpolicy_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_auditnslogpolicy_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
