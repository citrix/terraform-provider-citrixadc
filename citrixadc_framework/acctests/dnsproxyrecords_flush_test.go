...
// NOTE on the dnsproxyrecords_flush resource:
...
//     state with its synthetic ID ("dnsproxyrecords_flush"); it does NOT (and
...
const testAccDnsproxyrecordsFlush_basic = `
resource "citrixadc_dnsproxyrecords_flush" "tf_dnsproxyrecords" {
  type = "A"
}

`

func TestAccDnsproxyrecordsFlush_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccDnsproxyrecordsFlush_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckDnsproxyrecordsFlushExist("citrixadc_dnsproxyrecords_flush.tf_dnsproxyrecords", nil),
					resource.TestCheckResourceAttr("citrixadc_dnsproxyrecords_flush.tf_dnsproxyrecords", "type", "A"),
					// "id" is the synthetic state handle "dnsproxyrecords_flush".
					resource.TestCheckResourceAttrSet("citrixadc_dnsproxyrecords_flush.tf_dnsproxyrecords", "id"),
				),
			},
		},
	})
...
// testAccCheckDnsproxyrecordsFlushExist is a state-only existence check.
//
// dnsproxyrecords_flush is an action-only resource: Read is a no-op and there is
...
// equals the synthetic "dnsproxyrecords_flush" after a successful POST
// ?action=flush). This mirrors testAccCheckLbpersistentsessionsExist.
func testAccCheckDnsproxyrecordsFlushExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No dnsproxyrecords_flush ID is set")
		}
