...
func TestAccAppfwprofile_csrftag_binding_import(t *testing.T) {
	...
	// order: name,csrftag,csrfformactionurl) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"name", "csrftag", "csrfformactionurl"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccAppfwprofile_csrftag_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func testAccCheckAppfwprofile_csrftag_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "csrftag", "csrfformactionurl"}, nil)
		if err != nil {
			return err
		}
		name := idMap["name"]
		csrftag := idMap["csrftag"]
		csrfformactionurl := idMap["csrfformactionurl"]
...
func testAccCheckAppfwprofile_csrftag_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(id, []string{"name", "csrftag", "csrfformactionurl"}, nil)
		if err != nil {
			return err
		}
		name := idMap["name"]
...
func testAccCheckAppfwprofile_csrftag_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Appfwprofile_csrftag_binding.Type(), rs.Primary.ID)
		if err == nil {
...
func TestAccAppfwprofile_csrftag_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_appfwprofile_csrftag_binding.tf_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAppfwprofile_csrftag_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckAppfwprofile_csrftag_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgs(service.Appfwprofile_csrftag_binding.Type(), "tf_appfwprofile", []string{"csrftag:" + utils.UrlEncode("www.source.com"), "csrfformactionurl:" + utils.UrlEncode("^https://sd2\\-zgw\\.test\\.ctxns\\.com/api/document/content$")}); err != nil {
