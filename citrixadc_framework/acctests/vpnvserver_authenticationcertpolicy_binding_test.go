...
const testAccVpnvserver_authenticationcertpolicy_binding_basic_step2 = `
...
func TestAccVpnvserver_authenticationcertpolicy_binding_basic(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnvserver_authenticationcertpolicy_bindingExist("citrixadc_vpnvserver_authenticationcertpolicy_binding.tf_bind", nil),
				),
			},
			{
				Config: testAccVpnvserver_authenticationcertpolicy_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnvserver_authenticationcertpolicy_bindingNotExist("citrixadc_vpnvserver_authenticationcertpolicy_binding.tf_bind", "tf_vserver,tf_certpolicy"),
				),
			},
		},
	})
...
func TestAccVpnvserver_authenticationcertpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVpnvserver_authenticationcertpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"bindpoint"}},
		},
	})
}
...
func testAccCheckVpnvserver_authenticationcertpolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No vpnvserver_authenticationcertpolicy_binding id is set")
		}
		...
		findParams := service.FindParams{
			ResourceType:             "vpnvserver_authenticationcertpolicy_binding",
			ResourceName:             name,
		...
		if !found {
			return fmt.Errorf("vpnvserver_authenticationcertpolicy_binding %s not found", n)
		}
...
func testAccCheckVpnvserver_authenticationcertpolicy_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		if found {
			return fmt.Errorf("vpnvserver_authenticationcertpolicy_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckVpnvserver_authenticationcertpolicy_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_vpnvserver_authenticationcertpolicy_binding" {
			continue
		}
		...
		_, err := client.FindResource(service.Vpnvserver_authenticationcertpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("vpnvserver_authenticationcertpolicy_binding %s still exists", rs.Primary.ID)
		}
...
func TestAccVpnvserver_authenticationcertpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_authenticationcertpolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_authenticationcertpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckVpnvserver_authenticationcertpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Vpnvserver_authenticationcertpolicy_binding.Type(), "tf_vserver", map[string]string{"policy": "tf_certpolicy", "bindpoint": "REQUEST"}); err != nil {
