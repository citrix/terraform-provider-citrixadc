...
//
// nslaslicense is an APPLY-ONLY resource whose Create performs the NITRO
...
const testAccNslaslicense_basic_step1 = `
resource "citrixadc_nslaslicense" "tf_nslaslicense" {
...
func TestAccNslaslicense_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccNslaslicense_basic_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckNslaslicenseExist("citrixadc_nslaslicense.tf_nslaslicense", nil),
					resource.TestCheckResourceAttrSet("citrixadc_nslaslicense.tf_nslaslicense", "id"),
					resource.TestCheckResourceAttr("citrixadc_nslaslicense.tf_nslaslicense", "filename", "offline_token_10.101.132.151_activation.blob.tgz"),
					resource.TestCheckResourceAttr("citrixadc_nslaslicense.tf_nslaslicense", "filelocation", "/nsconfig/license"),
					resource.TestCheckResourceAttr("citrixadc_nslaslicense.tf_nslaslicense", "fixedbandwidth", "true"),
				),
			},
		},
	})
...
// testAccCheckNslaslicenseExist is a STATE-ONLY check. nslaslicense is
...
func testAccCheckNslaslicenseExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No nslaslicense ID is set")
		}
		...
		// No NITRO FindResource call: nslaslicense exposes no GET endpoint.
		return nil
	}
}
