...
// applying the citrixadc_systemsession_kill resource KILLS administrative
// sessions via POST /nitro/v1/config/systemsession?action=kill.
//
...
// NOTE on the systemsession_kill resource:
...
//     both are RequiresReplace. The synthetic ID is "systemsession_kill".
...
// sid value below before running this manually -- there is no generic valid
// sid. Using `all = true` instead is FORBIDDEN: it would kill the provider's
// own NITRO session (and everyone else's) and is never permitted in automation.
const testAccSystemsessionKill_basic_step1 = `
resource "citrixadc_systemsession_kill" "tf_systemsession_kill" {
  sid = 13555
}

`

func TestAccSystemsessionKill_basic(t *testing.T) {
	t.Skip("TODO: Requires review - the kill action terminates live admin sessions and can disconnect the test's own management/NITRO session; unsafe on a shared testbed")
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSystemsessionKill_basic_step1,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSystemsessionKillExist("citrixadc_systemsession_kill.tf_systemsession_kill", nil),
					// "id" is the synthetic state handle ("systemsession_kill").
					resource.TestCheckResourceAttrSet("citrixadc_systemsession_kill.tf_systemsession_kill", "id"),
				),
			},
		},
	})
...
// testAccCheckSystemsessionKillExist is a state-only existence check.
//
// systemsession_kill is an action-only resource: Read is a no-op and there is no
...
// with a non-empty ID (the synthetic "systemsession_kill" handle after a
// successful POST ?action=kill). This mirrors testAccCheckAaasessionExist.
func testAccCheckSystemsessionKillExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No systemsession_kill ID is set")
		}
...
//
// NOTE: the datasource type is the UNCHANGED citrixadc_systemsession (the
// refactor only split out the kill action into citrixadc_systemsession_kill).
