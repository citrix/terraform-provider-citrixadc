...
func TestAccAaagroup_vpnurlpolicy_binding_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccAaagroup_vpnurlpolicy_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAaagroup_vpnurlpolicy_bindingNotExist("citrixadc_aaagroup_vpnurlpolicy_binding.tf_aaagroup_vpnurlpolicy_binding", "my_group,new_policy"),
				),
			},
		},
	})
...
func testAccCheckAaagroup_vpnurlpolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No aaagroup_vpnurlpolicy_binding id is set")
		}
		...
		if !found {
			return fmt.Errorf("aaagroup_vpnurlpolicy_binding %s not found", n)
		}
...
func testAccCheckAaagroup_vpnurlpolicy_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		if found {
			return fmt.Errorf("aaagroup_vpnurlpolicy_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckAaagroup_vpnurlpolicy_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_aaagroup_vpnurlpolicy_binding" {
			continue
		}
		...
		_, err := client.FindResource("aaagroup_vpnurlpolicy_binding", rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("aaagroup_vpnurlpolicy_binding %s still exists", rs.Primary.ID)
		}
...
const testAccAaagroup_vpnurlpolicy_bindingDataSource_basic = `
	...
	data "citrixadc_aaagroup_vpnurlpolicy_binding" "tf_aaagroup_vpnurlpolicy_binding" {
		groupname = citrixadc_aaagroup_vpnurlpolicy_binding.tf_aaagroup_vpnurlpolicy_binding.groupname
		policy    = citrixadc_aaagroup_vpnurlpolicy_binding.tf_aaagroup_vpnurlpolicy_binding.policy
		depends_on = [citrixadc_aaagroup_vpnurlpolicy_binding.tf_aaagroup_vpnurlpolicy_binding]
	}
  
`

func TestAccAaagroup_vpnurlpolicy_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		Steps: []resource.TestStep{
			{
				Config: testAccAaagroup_vpnurlpolicy_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
