...
func TestAccVpnvserver_responderpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVpnvserver_responderpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func testAccCheckVpnvserver_responderpolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Vpnvserver_responderpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
const testAccVpnvserver_responderpolicy_bindingDataSource_basic = `
	resource "citrixadc_vpnvserver" "tf_vpnvserver" {
		name        = "tf_examplevserver"
		servicetype = "SSL"
		ipv46       = "3.3.3.3"
	...
	data "citrixadc_vpnvserver_responderpolicy_binding" "tf_bind" {
		name       = citrixadc_vpnvserver_responderpolicy_binding.tf_bind.name
		policy     = citrixadc_vpnvserver_responderpolicy_binding.tf_bind.policy
		depends_on = [citrixadc_vpnvserver_responderpolicy_binding.tf_bind]
	}
`

func TestAccVpnvserver_responderpolicy_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             nil,
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_responderpolicy_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
...
func TestAccVpnvserver_responderpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_vpnvserver_responderpolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccVpnvserver_responderpolicy_binding_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnvserver_responderpolicy_bindingExist(resAddr, nil),
				),
			},
			{
				PreConfig: func() {
					...
					if err := client.DeleteResourceWithArgs(service.Vpnvserver_responderpolicy_binding.Type(), "tf_examplevserver", []string{"policy:tf_responder_policy"}); err != nil {
