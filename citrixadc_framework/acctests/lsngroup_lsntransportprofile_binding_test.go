...
func testAccCheckLsngroup_lsntransportprofile_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"groupname", "transportprofilename"}, nil)
		if err != nil {
			return fmt.Errorf("Error parsing ID: %v", err)
		}
		groupname := idMap["groupname"]
		transportprofilename := idMap["transportprofilename"]
...
func TestAccLsngroup_lsntransportprofile_binding_import(t *testing.T) {
	...
	// order: groupname,transportprofilename) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"groupname", "transportprofilename"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccLsngroup_lsntransportprofile_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}

func TestAccLsngroup_lsntransportprofile_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_lsngroup_lsntransportprofile_binding.tf_lsngroup_lsntransportprofile_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccLsngroup_lsntransportprofile_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckLsngroup_lsntransportprofile_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Lsngroup_lsntransportprofile_binding.Type(), "my_lsngroup", map[string]string{"transportprofilename": "my_lsntransportprofile"}); err != nil {
