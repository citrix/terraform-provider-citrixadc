...
// NOTE on the rnatsession_flush resource:
...
//     state with its synthetic ID ("rnatsession_flush"); it does NOT (and
...
const testAccRnatsessionFlush_basic = `
...
resource "citrixadc_rnatsession_flush" "tf_rnatsession_flush" {
  network = "192.0.2.0"
  netmask = "255.255.255.0"
...
func TestAccRnatsessionFlush_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccRnatsessionFlush_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckRnatsessionFlushExist("citrixadc_rnatsession_flush.tf_rnatsession_flush", nil),
					resource.TestCheckResourceAttr("citrixadc_rnatsession_flush.tf_rnatsession_flush", "network", "192.0.2.0"),
					resource.TestCheckResourceAttr("citrixadc_rnatsession_flush.tf_rnatsession_flush", "netmask", "255.255.255.0"),
					// "id" is the synthetic state handle "rnatsession_flush".
					resource.TestCheckResourceAttrSet("citrixadc_rnatsession_flush.tf_rnatsession_flush", "id"),
				),
			},
		},
	})
...
// testAccCheckRnatsessionFlushExist is a state-only existence check.
//
// rnatsession_flush is an action-only resource: Read is a no-op and there is no
...
// equals the synthetic "rnatsession_flush" after a successful POST
// ?action=flush). This mirrors testAccCheckDnsproxyrecordsExist.
func testAccCheckRnatsessionFlushExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No rnatsession_flush ID is set")
		}
