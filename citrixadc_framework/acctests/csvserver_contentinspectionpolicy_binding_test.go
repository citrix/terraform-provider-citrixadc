...
func TestAccCsvserver_contentinspectionpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccCsvserver_contentinspectionpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}

func TestAccCsvserver_contentinspectionpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_csvserver_contentinspectionpolicy_binding.tf_csvserver_contentinspectionpolicy_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccCsvserver_contentinspectionpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckCsvserver_contentinspectionpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Csvserver_contentinspectionpolicy_binding.Type(), "tf_csvserver", map[string]string{"policyname": "tf_contentinspectionpolicy", "priority": "1"}); err != nil {
