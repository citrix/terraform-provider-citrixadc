...
const testAccSslcertbundle_basic_step1 = `
resource "citrixadc_sslcertbundle" "tf_sslcertbundle" {
...
func TestAccSslcertbundle_basic(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSslcertbundleExist("citrixadc_sslcertbundle.tf_sslcertbundle", nil),
					resource.TestCheckResourceAttr("citrixadc_sslcertbundle.tf_sslcertbundle", "name", "tf_sslcertbundle"),
...
func testAccCheckSslcertbundleDestroy(s *terraform.State) error {
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	}
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_sslcertbundle" {
			continue
		}
...
const testAccSslcertbundleDataSource_basic = `
...
data "citrixadc_sslcertbundle" "tf_sslcertbundle" {
  name       = citrixadc_sslcertbundle.tf_sslcertbundle.name
  depends_on = [citrixadc_sslcertbundle.tf_sslcertbundle]
}
`

func TestAccSslcertbundleDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { doSslcertbundlePreChecks(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             testAccCheckSslcertbundleDestroy,
		Steps: []resource.TestStep{
			{
				Config: testAccSslcertbundleDataSource_basic,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("data.citrixadc_sslcertbundle.tf_sslcertbundle", "name", "tf_sslcertbundle"),
				),
			},
		},
	})
