...
func testAccCheckAppfwprofile_excluderescontenttype_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "excluderescontenttype"}, nil)
		if err != nil {
			return err
		}
		name := idMap["name"]
		excluderescontenttype := idMap["excluderescontenttype"]
...
const testAccAppfwprofile_excluderescontenttype_binding_upgrade_basic = `
	resource "citrixadc_appfwprofile" "tf_appfwprofile" {
		name                     = "tf_appfwprofile"
		type                     = ["HTML"]
	}
...
func TestAccAppfwprofile_excluderescontenttype_binding_sdkv2StateUpgrade(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		CheckDestroy: testAccCheckAppfwprofile_excluderescontenttype_bindingDestroy,
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAppfwprofile_excluderescontenttype_bindingExist("citrixadc_appfwprofile_excluderescontenttype_binding.tf_binding1", nil),
					resource.TestCheckResourceAttr("citrixadc_appfwprofile_excluderescontenttype_binding.tf_binding1", "id", "tf_appfwprofile,expressionexample"),
				),
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAppfwprofile_excluderescontenttype_bindingExist("citrixadc_appfwprofile_excluderescontenttype_binding.tf_binding1", nil),
					resource.TestCheckResourceAttr("citrixadc_appfwprofile_excluderescontenttype_binding.tf_binding1", "id", "excluderescontenttype:expressionexample,name:tf_appfwprofile"),
				),
			},
		},
	})
...
func TestAccAppfwprofile_excluderescontenttype_binding_import(t *testing.T) {
	const resAddr = "citrixadc_appfwprofile_excluderescontenttype_binding.tf_binding1"

	// Backward-compat: import via the LEGACY SDK v2 id. Rebuild the legacy positional id from
	// the current canonical key:value id (raw values, only the keys actually set, in legacy
	// order: name,excluderescontenttype) so it matches exactly what SDK v2 wrote.
	legacyID := func(s *terraform.State) (string, error) {
		...
		ordr := []string{"name", "excluderescontenttype"}
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{Config: testAccAppfwprofile_excluderescontenttype_binding_basic},
			{Config: testAccAppfwprofile_excluderescontenttype_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"alertonly", "isautodeployed"}},
			{Config: testAccAppfwprofile_excluderescontenttype_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"alertonly", "isautodeployed"}},
		},
	})
}
