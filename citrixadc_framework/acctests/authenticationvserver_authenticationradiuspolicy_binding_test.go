...
func testAccCheckAuthenticationvserver_authenticationradiuspolicy_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource(service.Authenticationvserver_authenticationradiuspolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
...
// testAccAuthenticationvserver_authenticationradiuspolicy_binding_upgrade_basic reuses the
...
// attribute names.
const testAccAuthenticationvserver_authenticationradiuspolicy_binding_upgrade_basic = `
	resource "citrixadc_authenticationvserver" "tf_authenticationvserver" {
		name           = "tf_authenticationvserver"
		servicetype    = "SSL"
		comment        = "new"
...
// TestAccAuthenticationvserver_authenticationradiuspolicy_binding_sdkv2StateUpgrade verifies that
...
// binding with citrix/citrixadc 2.2.0 (writes the legacy id "tf_authenticationvserver,tf_radiuspolicy").
...
func TestAccAuthenticationvserver_authenticationradiuspolicy_binding_sdkv2StateUpgrade(t *testing.T) {
	resourceAddr := "citrixadc_authenticationvserver_authenticationradiuspolicy_binding.tf_bind"
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		CheckDestroy: testAccCheckAuthenticationvserver_authenticationradiuspolicy_bindingDestroy,
		Steps: []resource.TestStep{
			// Step 1: create with the last SDK v2 release -> state carries the legacy id.
			{
				...
				Config: testAccAuthenticationvserver_authenticationradiuspolicy_binding_upgrade_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAuthenticationvserver_authenticationradiuspolicy_bindingExist(resourceAddr, nil),
					resource.TestCheckResourceAttr(resourceAddr, "id", "tf_authenticationvserver,tf_radiuspolicy"),
				),
			},
			...
			// key:value format.
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAuthenticationvserver_authenticationradiuspolicy_bindingExist(resourceAddr, nil),
					resource.TestCheckResourceAttr(resourceAddr, "id", "name:tf_authenticationvserver,policy:tf_radiuspolicy"),
				),
			},
		},
	})
...
func TestAccAuthenticationvserver_authenticationradiuspolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{Config: testAccAuthenticationvserver_authenticationradiuspolicy_binding_basic},
			{Config: testAccAuthenticationvserver_authenticationradiuspolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"bindpoint"}},
			{Config: testAccAuthenticationvserver_authenticationradiuspolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"bindpoint"}},
		},
	})
}
