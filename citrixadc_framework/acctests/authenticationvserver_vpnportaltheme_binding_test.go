...
const testAccAuthenticationvserver_vpnportaltheme_binding_basic_step2 = `
...
func TestAccAuthenticationvserver_vpnportaltheme_binding_basic(t *testing.T) {
		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAuthenticationvserver_vpnportaltheme_bindingExist("citrixadc_authenticationvserver_vpnportaltheme_binding.tf_bind", nil),
				),
			},
			{
				Config: testAccAuthenticationvserver_vpnportaltheme_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAuthenticationvserver_vpnportaltheme_bindingNotExist("citrixadc_authenticationvserver_vpnportaltheme_binding.tf_bind", "tf_authenticationvserver,tf_vpnportaltheme"),
				),
			},
		},
	})
...
func testAccCheckAuthenticationvserver_vpnportaltheme_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No authenticationvserver_vpnportaltheme_binding id is set")
		}
		...
		if !found {
			return fmt.Errorf("authenticationvserver_vpnportaltheme_binding %s not found", n)
		}
...
func testAccCheckAuthenticationvserver_vpnportaltheme_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		if found {
			return fmt.Errorf("authenticationvserver_vpnportaltheme_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckAuthenticationvserver_vpnportaltheme_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_authenticationvserver_vpnportaltheme_binding" {
			continue
		}
		...
		_, err := client.FindResource("authenticationvserver_vpnportaltheme_binding", rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("authenticationvserver_vpnportaltheme_binding %s still exists", rs.Primary.ID)
		}
...
func TestAccAuthenticationvserver_vpnportaltheme_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccAuthenticationvserver_vpnportaltheme_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
