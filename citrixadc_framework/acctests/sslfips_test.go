...
// Datasource KEPT for sslfips (datasource files present). Also FIPS-only and
// therefore skip-gated -- the underlying GET requires the HSM to be initialized.
const testAccSslfipsDataSource_basic = `
variable "sslfips_sopassword_wo" {
  type      = string
  sensitive = true
}
...
data "citrixadc_sslfips" "tf_sslfips" {
  depends_on = [citrixadc_sslfips.tf_sslfips]
}
`
...
func TestAccSslfipsDataSource_basic(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSslfipsDataSource_basic,
				Check: resource.ComposeAggregateTestCheckFunc(
					resource.TestCheckResourceAttr("data.citrixadc_sslfips.tf_sslfips", "inithsm", "Level-2"),
					resource.TestCheckResourceAttr("data.citrixadc_sslfips.tf_sslfips", "hsmlabel", "test_hsm_label"),
				),
			},
		},
	})
