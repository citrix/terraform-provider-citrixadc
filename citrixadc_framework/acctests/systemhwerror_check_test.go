...
// NOTE on the systemhwerror_check resource:
...
//     state with its synthetic ID ("systemhwerror_check"); it does NOT (and
...
const testAccSystemhwerrorCheck_basic = `
resource "citrixadc_systemhwerror_check" "tf_systemhwerror" {
  diskcheck = true
}

`

func TestAccSystemhwerrorCheck_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccSystemhwerrorCheck_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckSystemhwerrorCheckExist("citrixadc_systemhwerror_check.tf_systemhwerror", nil),
					// "id" is the synthetic state handle "systemhwerror_check".
					resource.TestCheckResourceAttrSet("citrixadc_systemhwerror_check.tf_systemhwerror", "id"),
					// Assert the flag actually set in HCL.
					resource.TestCheckResourceAttr("citrixadc_systemhwerror_check.tf_systemhwerror", "diskcheck", "true"),
				),
			},
		},
	})
...
// testAccCheckSystemhwerrorCheckExist is a state-only existence check.
//
// systemhwerror_check is an action-only resource: Read is a no-op and there is no
...
// equals the synthetic "systemhwerror_check" after a successful POST
// ?action=check). This mirrors testAccCheckGslbconfigExist.
func testAccCheckSystemhwerrorCheckExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No systemhwerror_check ID is set")
		}
