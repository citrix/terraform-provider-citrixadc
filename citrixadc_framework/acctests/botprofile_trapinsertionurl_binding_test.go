...
func testAccCheckBotprofile_trapinsertionurl_bindingDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := client.FindResource("botprofile_trapinsertionurl_binding", rs.Primary.ID)
		if err == nil {
...
const testAccbotprofile_trapinsertionurl_bindingDataSource_basic = `
	resource "citrixadc_botprofile" "tf_botprofile" {
		name                     = "tf_botprofile"
		errorurl                 = "http://www.citrix.com"
		trapurl                  = "/http://www.citrix.com"
	...
	data "citrixadc_botprofile_trapinsertionurl_binding" "tf_binding" {
		name             = citrixadc_botprofile_trapinsertionurl_binding.tf_binding.name
		bot_trap_url     = citrixadc_botprofile_trapinsertionurl_binding.tf_binding.bot_trap_url
		depends_on       = [citrixadc_botprofile_trapinsertionurl_binding.tf_binding]
	}
`

func TestAccbotprofile_trapinsertionurl_bindingDataSource_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		CheckDestroy:             nil,
		Steps: []resource.TestStep{
			{
				Config: testAccbotprofile_trapinsertionurl_bindingDataSource_basic,
				Check: resource.ComposeTestCheckFunc(
					resource.TestCheckResourceAttr("data.citrixadc_botprofile_trapinsertionurl_binding.tf_binding", "name", "tf_botprofile"),
					resource.TestCheckResourceAttr("data.citrixadc_botprofile_trapinsertionurl_binding.tf_binding", "trapinsertionurl", "true"),
