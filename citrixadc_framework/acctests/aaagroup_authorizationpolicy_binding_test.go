...
func TestAccAaagroup_authorizationpolicy_binding_import(t *testing.T) {
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccAaagroup_authorizationpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"type"}},
		},
	})
}
...
func TestAccAaagroup_authorizationpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_aaagroup_authorizationpolicy_binding.tf_aaagroup_authorizationpolicy_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAaagroup_authorizationpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckAaagroup_authorizationpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Aaagroup_authorizationpolicy_binding.Type(), "my_group", map[string]string{"policy": "tp-authorize-1", "type": "REQUEST"}); err != nil {
