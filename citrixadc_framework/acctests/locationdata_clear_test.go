...
// NOTE on the locationdata_clear resource:
...
//     with its synthetic id ("locationdata_clear"); it cannot verify the side
//     effect via NITRO.
//
...
const testAccLocationdataClear_basic = `
resource "citrixadc_locationdata_clear" "tf_locationdata" {
}
`

func TestAccLocationdataClear_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccLocationdataClear_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckLocationdataClearExist("citrixadc_locationdata_clear.tf_locationdata", nil),
					// "id" is the synthetic state handle "locationdata_clear".
					resource.TestCheckResourceAttrSet("citrixadc_locationdata_clear.tf_locationdata", "id"),
				),
			},
		},
	})
...
// testAccCheckLocationdataClearExist is a state-only existence check. locationdata_clear
...
func testAccCheckLocationdataClearExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No locationdata_clear ID is set")
		}
