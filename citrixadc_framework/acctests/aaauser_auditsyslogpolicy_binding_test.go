...
func TestAccAaauser_auditsyslogpolicy_binding_basic(t *testing.T) {
...
func testAccCheckAaauser_auditsyslogpolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No aaauser_auditsyslogpolicy_binding id is set")
		}
		...
		findParams := service.FindParams{
			ResourceType:             "aaauser_auditsyslogpolicy_binding",
		...
		if !found {
			return fmt.Errorf("aaauser_auditsyslogpolicy_binding %s not found", n)
		}
...
func testAccCheckAaauser_auditsyslogpolicy_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		if found {
			return fmt.Errorf("aaauser_auditsyslogpolicy_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckAaauser_auditsyslogpolicy_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_aaauser_auditsyslogpolicy_binding" {
			continue
		}
		...
		_, err := client.FindResource(service.Aaauser_auditsyslogpolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("aaauser_auditsyslogpolicy_binding %s still exists", rs.Primary.ID)
		}
...
// testAccAaauser_auditsyslogpolicy_binding_upgrade_basic reuses the _basic config
...
// attribute names.
const testAccAaauser_auditsyslogpolicy_binding_upgrade_basic = `
...
// TestAccAaauser_auditsyslogpolicy_binding_sdkv2StateUpgrade verifies that state
...
// legacy id "user1,tf_auditsyslogpolicy"). Step 2 refreshes/plans/applies the same
...
func TestAccAaauser_auditsyslogpolicy_binding_sdkv2StateUpgrade(t *testing.T) {
	resourceAddr := "citrixadc_aaauser_auditsyslogpolicy_binding.tf_aaauser_auditsyslogpolicy_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:     func() { testAccPreCheck(t) },
		CheckDestroy: testAccCheckAaauser_auditsyslogpolicy_bindingDestroy,
		Steps: []resource.TestStep{
			// Step 1: create with the last SDK v2 release -> state carries the legacy id.
			{
				...
				Config: testAccAaauser_auditsyslogpolicy_binding_upgrade_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAaauser_auditsyslogpolicy_bindingExist(resourceAddr, nil),
					resource.TestCheckResourceAttr(resourceAddr, "id", "user1,tf_auditsyslogpolicy"),
				),
			},
			...
			// recomputed to the new key:value format.
			{
				...
				Check: resource.ComposeTestCheckFunc(
					testAccCheckAaauser_auditsyslogpolicy_bindingExist(resourceAddr, nil),
					resource.TestCheckResourceAttr(resourceAddr, "id", "policy:tf_auditsyslogpolicy,username:user1"),
				),
			},
		},
	})
...
func TestAccAaauser_auditsyslogpolicy_binding_import(t *testing.T) {
	const resAddr = "citrixadc_aaauser_auditsyslogpolicy_binding.tf_aaauser_auditsyslogpolicy_binding"
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{Config: testAccAaauser_auditsyslogpolicy_binding_basic},
			{Config: testAccAaauser_auditsyslogpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"type"}},
			{Config: testAccAaauser_auditsyslogpolicy_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{"type"}},
		},
	})
}

func TestAccAaauser_auditsyslogpolicy_binding_selfHealing(t *testing.T) {
	const resAddr = "citrixadc_aaauser_auditsyslogpolicy_binding.tf_aaauser_auditsyslogpolicy_binding"
	resource.Test(t, resource.TestCase{
		PreCheck:                 func() { testAccPreCheck(t) },
		ProtoV6ProviderFactories: testAccProtoV6ProviderFactories,
		...
		Steps: []resource.TestStep{
			{
				Config: testAccAaauser_auditsyslogpolicy_binding_basic,
				Check:  resource.ComposeTestCheckFunc(testAccCheckAaauser_auditsyslogpolicy_bindingExist(resAddr, nil)),
			},
			{
				PreConfig: func() {
					client, err := testAccGetFrameworkClient()
					...
					if err := client.DeleteResourceWithArgsMap(service.Aaauser_auditsyslogpolicy_binding.Type(), "user1", map[string]string{"policy": "tf_auditsyslogpolicy"}); err != nil {
