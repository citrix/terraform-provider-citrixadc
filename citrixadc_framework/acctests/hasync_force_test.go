...
// NOTE on the hasync_force resource:
...
//     state with its synthetic ID ("hasync_force"); it does NOT (and cannot)
...
const testAccHasyncForce_basic = `
resource "citrixadc_hasync_force" "tf_hasync" {
  force = true
  save  = "YES"
}
...
func TestAccHasyncForce_basic(t *testing.T) {
	if adcTestbed != "HA_PAIR" {
		t.Skipf("ADC testbed is %s. Expected HA_PAIR (hasync_force Force requires an HA pair).", adcTestbed)
	}
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			{
				Config: testAccHasyncForce_basic,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckHasyncForceExist("citrixadc_hasync_force.tf_hasync", nil),
					// "id" is the synthetic state handle "hasync_force".
					resource.TestCheckResourceAttrSet("citrixadc_hasync_force.tf_hasync", "id"),
					// Assert the config attributes actually set in HCL; both are
					// stored in state even though they are not Computed.
					resource.TestCheckResourceAttr("citrixadc_hasync_force.tf_hasync", "force", "true"),
					resource.TestCheckResourceAttr("citrixadc_hasync_force.tf_hasync", "save", "YES"),
				),
			},
		},
	})
...
// testAccCheckHasyncForceExist is a state-only existence check.
//
// hasync_force is an action-only resource: Read is a no-op and there is no
// GET-by-id endpoint, so we CANNOT verify the Force sync via NITRO. We only
// assert that Terraform recorded the resource in state with a non-empty ID
// (which equals the synthetic "hasync_force" after a successful POST
// ?action=Force). This mirrors testAccCheckHafilesExist,
// testAccCheckClusterfilesActionExist and testAccCheckAaasessionExist.
func testAccCheckHasyncForceExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No hasync_force ID is set")
		}
