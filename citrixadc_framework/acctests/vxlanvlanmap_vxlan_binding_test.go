...
const testAccVxlanvlanmap_vxlan_binding_basic_step2 = `
...
func TestAccVxlanvlanmap_vxlan_binding_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccVxlanvlanmap_vxlan_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVxlanvlanmap_vxlan_bindingNotExist("citrixadc_vxlanvlanmap_vxlan_binding.tf_binding", "tf_vxlanvlanmp,123"),
				),
			},
		},
	})
...
func TestAccVxlanvlanmap_vxlan_binding_import(t *testing.T) {
	...
	// order: name,vxlan) so it matches exactly what SDK v2 wrote.
	...
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{Config: testAccVxlanvlanmap_vxlan_binding_basic, ResourceName: resAddr, ImportState: true, ImportStateIdFunc: legacyID, ImportStateVerify: true, ImportStateVerifyIgnore: []string{}},
		},
	})
}
...
func testAccCheckVxlanvlanmap_vxlan_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No vxlanvlanmap_vxlan_binding id is set")
		}
		...
		idMap, _, err := utils.ParseIdString(bindingId, []string{"name", "vxlan"}, nil)
		if err != nil {
		...
		if !found {
			return fmt.Errorf("vxlanvlanmap_vxlan_binding %s not found", n)
		}
...
func testAccCheckVxlanvlanmap_vxlan_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		// Use the shared utility function to get a configured client
		client, err := testAccGetFrameworkClient()
		if err != nil {
		...
		idMap, _, err := utils.ParseIdString(id, []string{"name", "vxlan"}, nil)
		if err != nil {
			return fmt.Errorf("Error parsing ID: %v", err)
		}
		name := idMap["name"]
		...
		if found {
			return fmt.Errorf("vxlanvlanmap_vxlan_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckVxlanvlanmap_vxlan_bindingDestroy(s *terraform.State) error {
	// Use the shared utility function to get a configured client
	client, err := testAccGetFrameworkClient()
	if err != nil {
		return fmt.Errorf("Failed to get test client: %v", err)
	...
	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_vxlanvlanmap_vxlan_binding" {
			continue
		}
		...
		_, err := client.FindResource("vxlanvlanmap_vxlan_binding", rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("vxlanvlanmap_vxlan_binding %s still exists", rs.Primary.ID)
		}
