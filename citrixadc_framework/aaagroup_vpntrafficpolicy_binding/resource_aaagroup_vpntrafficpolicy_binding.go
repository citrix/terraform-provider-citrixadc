...
func (r *AaagroupVpntrafficpolicyBindingResource) readAaagroupVpntrafficpolicyBindingFromApi(ctx context.Context, data *AaagroupVpntrafficpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "aaagroup_vpntrafficpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("aaagroup_vpntrafficpolicy_binding not found with the provided ID attributes"))
		return
	}
