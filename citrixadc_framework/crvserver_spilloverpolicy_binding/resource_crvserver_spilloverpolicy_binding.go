...
func (r *CrvserverSpilloverpolicyBindingResource) readCrvserverSpilloverpolicyBindingFromApi(ctx context.Context, data *CrvserverSpilloverpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "crvserver_spilloverpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("crvserver_spilloverpolicy_binding not found with the provided ID attributes"))
		return
	}
