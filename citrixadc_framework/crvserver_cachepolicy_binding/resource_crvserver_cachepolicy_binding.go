...
func (r *CrvserverCachepolicyBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	_, err := r.client.AddResource(service.Crvserver_cachepolicy_binding.Type(), "", &crvserver_cachepolicy_binding)
	if err != nil {
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "crvserver_cachepolicy_binding not found on the ADC immediately after create")
		return
	}
...
func (r *CrvserverCachepolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "crvserver_cachepolicy_binding not found on the ADC immediately after update")
		return
	}
