...
func (r *AppfwprofileDenyurlBindingResource) readAppfwprofileDenyurlBindingFromApi(ctx context.Context, data *AppfwprofileDenyurlBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "appfwprofile_denyurl_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("appfwprofile_denyurl_binding not found with the provided ID attributes"))
		return
	}
