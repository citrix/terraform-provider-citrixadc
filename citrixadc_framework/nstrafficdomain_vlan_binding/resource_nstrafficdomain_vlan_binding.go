...
func (r *NstrafficdomainVlanBindingResource) readNstrafficdomainVlanBindingFromApi(ctx context.Context, data *NstrafficdomainVlanBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "nstrafficdomain_vlan_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("nstrafficdomain_vlan_binding not found with the provided ID attributes"))
		return
	}
