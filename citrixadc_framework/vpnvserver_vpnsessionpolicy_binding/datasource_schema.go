...
// VpnvserverVpnsessionpolicyBindingDataSourceModel is the data-source-specific
...
type VpnvserverVpnsessionpolicyBindingDataSourceModel struct {
	...
	// (zion73x_readonly/vpnvserver_vpnsessionpolicy_binding.json). Never
	// settable; populated from GET.
	Acttype types.Int64 `tfsdk:"acttype"`
}

func VpnvserverVpnsessionpolicyBindingDataSourceSchema() schema.Schema {
...
// vpnvserver_vpnsessionpolicy_bindingDataSourceSetAttrFromGet projects a NITRO
// vpnvserver_vpnsessionpolicy_binding GET response onto the data-source model.
...
func vpnvserver_vpnsessionpolicy_bindingDataSourceSetAttrFromGet(ctx context.Context, data *VpnvserverVpnsessionpolicyBindingDataSourceModel, g map[string]interface{}) {
	tflog.Debug(ctx, "In vpnvserver_vpnsessionpolicy_bindingDataSourceSetAttrFromGet Function")
	...
	idParts := []string{}
	idParts = append(idParts, fmt.Sprintf("bindpoint:%s", utils.UrlEncode(utils.AnyToString(data.Bindpoint.ValueString()))))
