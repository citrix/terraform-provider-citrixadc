...
func (r *SystemgroupNspartitionBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	systemgroup_nspartition_binding := systemgroup_nspartition_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "systemgroup_nspartition_binding not found on the ADC immediately after create")
		return
	}
...
func (r *SystemgroupNspartitionBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state SystemgroupNspartitionBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for systemgroup_nspartition_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "systemgroup_nspartition_binding not found on the ADC immediately after update")
		return
	}
...
func (r *SystemgroupNspartitionBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	idMap, _, err := utils.ParseIdString(data.Id.ValueString(), []string{"groupname", "partitionname"}, nil)
	if err != nil {
	...
	tflog.Trace(ctx, "Deleted systemgroup_nspartition_binding binding")
}
