...
func (r *SystemgroupNspartitionBindingResource) readSystemgroupNspartitionBindingFromApi(ctx context.Context, data *SystemgroupNspartitionBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "systemgroup_nspartition_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("systemgroup_nspartition_binding not found with the provided ID attributes"))
		return
	}
