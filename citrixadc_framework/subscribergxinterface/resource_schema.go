...
func subscribergxinterfaceGetThePayloadFromtheConfig(ctx context.Context, data *SubscribergxinterfaceResourceModel) subscriber.Subscribergxinterface {
	...
	if !data.Cerrequesttimeout.IsNull() && !data.Cerrequesttimeout.IsUnknown() {
	...
	if !data.Healthcheckttl.IsNull() && !data.Healthcheckttl.IsUnknown() {
		subscribergxinterface.Healthcheckttl = utils.IntPtr(int(data.Healthcheckttl.ValueInt64()))
	}
	if !data.Holdonsubscriberabsence.IsNull() && !data.Holdonsubscriberabsence.IsUnknown() {
	...
	if !data.Negativettl.IsNull() && !data.Negativettl.IsUnknown() {
		subscribergxinterface.Negativettl = utils.IntPtr(int(data.Negativettl.ValueInt64()))
	}
	if !data.Negativettllimitedsuccess.IsNull() && !data.Negativettllimitedsuccess.IsUnknown() {
	...
	if !data.Pcrfrealm.IsNull() && !data.Pcrfrealm.IsUnknown() {
		subscribergxinterface.Pcrfrealm = data.Pcrfrealm.ValueString()
	}
	if !data.Purgesdbongxfailure.IsNull() && !data.Purgesdbongxfailure.IsUnknown() {
		subscribergxinterface.Purgesdbongxfailure = data.Purgesdbongxfailure.ValueString()
	}
	if !data.Requestretryattempts.IsNull() && !data.Requestretryattempts.IsUnknown() {
	...
	if !data.Revalidationtimeout.IsNull() && !data.Revalidationtimeout.IsUnknown() {
	...
	if !data.Servicepathavp.IsNull() && !data.Servicepathavp.IsUnknown() {
		var servicepathavpList []int64
		data.Servicepathavp.ElementsAs(ctx, &servicepathavpList, false)
		intList := make([]int, len(servicepathavpList))
		for i, v := range servicepathavpList {
			intList[i] = int(v)
		}
		subscribergxinterface.Servicepathavp = intList
	}
	if !data.Servicepathvendorid.IsNull() && !data.Servicepathvendorid.IsUnknown() {
...
func subscribergxinterfaceSetAttrFromGet(ctx context.Context, data *SubscribergxinterfaceResourceModel, getResponseData map[string]interface{}) *SubscribergxinterfaceResourceModel {
	...
	} else if data.Cerrequesttimeout.IsUnknown() {
	...
	} else if data.Healthcheck.IsUnknown() {
	...
	} else if data.Healthcheckttl.IsUnknown() {
	...
	} else if data.Holdonsubscriberabsence.IsUnknown() {
	...
	} else if data.Negativettl.IsUnknown() {
	...
	} else if data.Negativettllimitedsuccess.IsUnknown() {
	...
	} else if data.Pcrfrealm.IsUnknown() {
	...
	} else if data.Purgesdbongxfailure.IsUnknown() {
	...
	} else if data.Requestretryattempts.IsUnknown() {
	...
	} else if data.Requesttimeout.IsUnknown() {
	...
	} else if data.Revalidationtimeout.IsUnknown() {
	...
	if val, ok := getResponseData["servicepathavp"]; ok && val != nil {
		if sliceVal, ok := val.([]interface{}); ok {
			// NITRO may return the AVP codes as strings or as JSON numbers;
		...
		} else if data.Servicepathavp.IsUnknown() {
	...
	} else if data.Servicepathvendorid.IsUnknown() {
