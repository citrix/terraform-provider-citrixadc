...
func (d *ClusternodegroupCsvserverBindingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Resource is missing
	if foundIndex == -1 {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("clusternodegroup_csvserver_binding with name %s not found", name_Name))
		return
	}

	clusternodegroup_csvserver_bindingSetAttrFromGetForDatasource(ctx, &data, dataArr[foundIndex])
