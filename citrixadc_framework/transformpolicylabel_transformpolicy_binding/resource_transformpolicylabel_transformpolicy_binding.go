...
func (r *TransformpolicylabelTransformpolicyBindingResource) readTransformpolicylabelTransformpolicyBindingFromApi(ctx context.Context, data *TransformpolicylabelTransformpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "transformpolicylabel_transformpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("transformpolicylabel_transformpolicy_binding not found with the provided ID attributes"))
		return
	}
