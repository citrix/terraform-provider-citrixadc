...
func (r *LsngroupLsnpoolBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	lsngroup_lsnpool_binding := lsngroup_lsnpool_bindingGetThePayloadFromthePlan(ctx, &data)
...
func (r *LsngroupLsnpoolBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state LsngroupLsnpoolBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for lsngroup_lsnpool_binding resource, skipping update")
	}
...
func (r *LsngroupLsnpoolBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	idMap, _, err := utils.ParseIdString(data.Id.ValueString(), []string{"groupname", "poolname"}, nil)
	if err != nil {
	...
	if val, ok := idMap["poolname"]; ok && val != "" {
		argsMap["poolname"] = val
	}

	err = r.client.DeleteResourceWithArgsMap(service.Lsngroup_lsnpool_binding.Type(), groupname_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete lsngroup_lsnpool_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted lsngroup_lsnpool_binding binding")
}
...
func (r *LsngroupLsnpoolBindingResource) readLsngroupLsnpoolBindingFromApi(ctx context.Context, data *LsngroupLsnpoolBindingResourceModel, diags *diag.Diagnostics) {
	...
	for i, v := range dataArr {
		...
		} else if _, ok := v["poolname"].(string); ok {
			match = false
			continue
		}
		if match {
