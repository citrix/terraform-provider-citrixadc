...
func (r *ServiceLbmonitorBindingResource) readServiceLbmonitorBindingFromApi(ctx context.Context, data *ServiceLbmonitorBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "service_lbmonitor_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("service_lbmonitor_binding not found with the provided ID attributes"))
		return
	}
