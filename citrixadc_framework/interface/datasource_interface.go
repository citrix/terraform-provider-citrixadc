...
var _ datasource.DataSource = (*InterfaceDataSource)(nil)

func INterfaceDataSource() datasource.DataSource {
...
func (d *InterfaceDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	interfaceid_Name := data.Interfaceid.ValueString()

	var dataArr []map[string]interface{}
	var err error

	findParams := service.FindParams{
		ResourceType:             service.Interface.Type(),
		ResourceMissingErrorCode: 258,
	}
	dataArr, err = d.client.FindResourceArrayWithParams(findParams)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read interface, got error: %s", err))
		return
	}

	// Resource is missing
	if len(dataArr) == 0 {
		resp.Diagnostics.AddError("Client Error", "interface returned empty array")
		return
	}
	...
	for i, v := range dataArr {
		...
		if v["id"].(string) != interfaceid_Name {
			match = false
		}
	...
	// Resource is missing
	if foundIndex == -1 {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("interface with id %s not found", interfaceid_Name))
		return
	}
