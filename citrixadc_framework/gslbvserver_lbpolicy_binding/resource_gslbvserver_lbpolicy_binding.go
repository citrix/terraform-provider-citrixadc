...
func (r *GslbvserverLbpolicyBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	gslbvserver_lbpolicy_binding := gslbvserver_lbpolicy_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "gslbvserver_lbpolicy_binding not found on the ADC immediately after create")
		return
	}
...
func (r *GslbvserverLbpolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state GslbvserverLbpolicyBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for gslbvserver_lbpolicy_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "gslbvserver_lbpolicy_binding not found on the ADC immediately after update")
		return
	}
...
func (r *GslbvserverLbpolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Gslbvserver_lbpolicy_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete gslbvserver_lbpolicy_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted gslbvserver_lbpolicy_binding binding")
}
