...
func (r *GslbvserverLbpolicyBindingResource) readGslbvserverLbpolicyBindingFromApi(ctx context.Context, data *GslbvserverLbpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "gslbvserver_lbpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("gslbvserver_lbpolicy_binding not found with the provided ID attributes"))
		return
	}
