...
func (r *NspartitionVxlanBindingResource) readNspartitionVxlanBindingFromApi(ctx context.Context, data *NspartitionVxlanBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "nspartition_vxlan_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("nspartition_vxlan_binding not found with the provided ID attributes"))
		return
	}
