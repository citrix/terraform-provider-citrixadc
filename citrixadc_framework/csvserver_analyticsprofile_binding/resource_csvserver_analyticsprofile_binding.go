...
func (r *CsvserverAnalyticsprofileBindingResource) readCsvserverAnalyticsprofileBindingFromApi(ctx context.Context, data *CsvserverAnalyticsprofileBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "csvserver_analyticsprofile_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("csvserver_analyticsprofile_binding not found with the provided ID attributes"))
		return
	}
