...
// HanodeRoutemonitor6BindingDataSourceModel is the data-source-specific model,
...
type HanodeRoutemonitor6BindingDataSourceModel struct {
	...
	// (zion73x_readonly/hanode_routemonitor6_binding.json). Never settable;
...
// hanode_routemonitor6_bindingDataSourceSetAttrFromGet projects a NITRO GET
// response onto the data-source model. Because a data source has no plan/apply
// reconciliation, attributes are simply filled from the GET (or left Null when
// the GET omits them) via the shared utils.MapGet* helpers. The composite ID is
// built exactly as the resource Create emits it (legacy order: hanode_id,routemonitor).
func hanode_routemonitor6_bindingDataSourceSetAttrFromGet(ctx context.Context, data *HanodeRoutemonitor6BindingDataSourceModel, g map[string]interface{}) {
	tflog.Debug(ctx, "In hanode_routemonitor6_bindingDataSourceSetAttrFromGet Function")
	...
	// Composite ID (legacy order: hanode_id,routemonitor), key:UrlEncode(value) pairs.
	idParts := []string{}
