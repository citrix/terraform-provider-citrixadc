...
func lbvserver_spilloverpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *LbvserverSpilloverpolicyBindingResourceModel) lb.Lbvserverspilloverpolicybinding {
	tflog.Debug(ctx, "In lbvserver_spilloverpolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Order.IsNull() {
		lbvserver_spilloverpolicy_binding.Order = utils.IntPtr(int(data.Order.ValueInt64()))
	}
