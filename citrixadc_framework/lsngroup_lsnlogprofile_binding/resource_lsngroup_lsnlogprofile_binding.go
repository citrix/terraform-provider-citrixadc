...
func (r *LsngroupLsnlogprofileBindingResource) readLsngroupLsnlogprofileBindingFromApi(ctx context.Context, data *LsngroupLsnlogprofileBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "lsngroup_lsnlogprofile_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("lsngroup_lsnlogprofile_binding not found with the provided ID attributes"))
		return
	}
