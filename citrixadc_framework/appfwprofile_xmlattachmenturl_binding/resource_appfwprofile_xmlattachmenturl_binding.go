...
func (r *AppfwprofileXmlattachmenturlBindingResource) readAppfwprofileXmlattachmenturlBindingFromApi(ctx context.Context, data *AppfwprofileXmlattachmenturlBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "appfwprofile_xmlattachmenturl_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("appfwprofile_xmlattachmenturl_binding not found with the provided ID attributes"))
		return
	}
