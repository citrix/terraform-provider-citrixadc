...
func (r *VpnglobalVpnurlBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	vpnglobal_vpnurl_binding := vpnglobal_vpnurl_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	data.Id = types.StringValue(fmt.Sprintf("%v", data.Urlname.ValueString()))
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "vpnglobal_vpnurl_binding not found on the ADC immediately after create")
		return
	}
...
func (r *VpnglobalVpnurlBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state VpnglobalVpnurlBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for vpnglobal_vpnurl_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "vpnglobal_vpnurl_binding not found on the ADC immediately after update")
		return
	}
...
func (r *VpnglobalVpnurlBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// urlname is an intranet URL that may contain slashes/special characters; the
	// NITRO delete arg value must be URL-encoded (the client does not encode arg
	// values).
	urlname_value := data.Id.ValueString()
	args := []string{
		fmt.Sprintf("urlname:%s", url.QueryEscape(urlname_value)),
	}

	err := r.client.DeleteResourceWithArgs(service.Vpnglobal_vpnurl_binding.Type(), "", args)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete vpnglobal_vpnurl_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted vpnglobal_vpnurl_binding binding")
}
...
func (r *VpnglobalVpnurlBindingResource) readVpnglobalVpnurlBindingFromApi(ctx context.Context, data *VpnglobalVpnurlBindingResourceModel, diags *diag.Diagnostics) {
	...
	idMap, _, err := utils.ParseIdString(data.Id.ValueString(), []string{"urlname"}, nil)
	if err != nil {
