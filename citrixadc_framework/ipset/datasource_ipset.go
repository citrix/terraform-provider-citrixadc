...
var _ datasource.DataSource = (*IpsetDataSource)(nil)

func IPsetDataSource() datasource.DataSource {
...
func (d *IpsetDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = IpsetDataSourceSchema()
}
...
func (d *IpsetDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	getResponseData, err = d.client.FindResource(service.Ipset.Type(), name_Name)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read ipset, got error: %s", err))
		return
	}
