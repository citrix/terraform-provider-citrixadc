...
func (r *LbmetrictableMetricBindingResource) readLbmetrictableMetricBindingFromApi(ctx context.Context, data *LbmetrictableMetricBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "lbmetrictable_metric_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("lbmetrictable_metric_binding not found with the provided ID attributes"))
		return
	}
