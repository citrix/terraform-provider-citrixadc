...
func (r *VxlanSrcipBindingResource) readVxlanSrcipBindingFromApi(ctx context.Context, data *VxlanSrcipBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vxlan_srcip_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vxlan_srcip_binding not found with the provided ID attributes"))
		return
	}
