...
func (r *SslservicegroupSslcacertbundleBindingResource) readSslservicegroupSslcacertbundleBindingFromApi(ctx context.Context, data *SslservicegroupSslcacertbundleBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "sslservicegroup_sslcacertbundle_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("sslservicegroup_sslcacertbundle_binding not found with the provided ID attributes"))
		return
	}
