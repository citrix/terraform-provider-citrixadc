...
func (r *LbvserverVideooptimizationpacingpolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state LbvserverVideooptimizationpacingpolicyBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for lbvserver_videooptimizationpacingpolicy_binding resource, skipping update")
	}
...
func (r *LbvserverVideooptimizationpacingpolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Lbvserver_videooptimizationpacingpolicy_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete lbvserver_videooptimizationpacingpolicy_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted lbvserver_videooptimizationpacingpolicy_binding binding")
}
