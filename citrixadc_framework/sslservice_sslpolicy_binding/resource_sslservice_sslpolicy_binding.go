...
func (r *SslserviceSslpolicyBindingResource) readSslserviceSslpolicyBindingFromApi(ctx context.Context, data *SslserviceSslpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "sslservice_sslpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("sslservice_sslpolicy_binding not found with the provided ID attributes"))
		return
	}
