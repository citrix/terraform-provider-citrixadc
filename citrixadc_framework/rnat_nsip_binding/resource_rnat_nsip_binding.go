...
func (r *RnatNsipBindingResource) readRnatNsipBindingFromApi(ctx context.Context, data *RnatNsipBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "rnat_nsip_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("rnat_nsip_binding not found with the provided ID attributes"))
		return
	}
