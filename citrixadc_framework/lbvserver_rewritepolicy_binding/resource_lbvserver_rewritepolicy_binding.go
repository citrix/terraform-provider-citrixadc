...
func (r *LbvserverRewritepolicyBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	lbvserver_rewritepolicy_binding := lbvserver_rewritepolicy_bindingGetThePayloadFromthePlan(ctx, &data)

	// Make API call
	// Binding resource - NITRO `add` is POST (matches the SDK v2 AddResource contract)
	_, err := r.client.AddResource(service.Lbvserver_rewritepolicy_binding.Type(), data.Name.ValueString(), &lbvserver_rewritepolicy_binding)
	if err != nil {
...
func (r *LbvserverRewritepolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state LbvserverRewritepolicyBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for lbvserver_rewritepolicy_binding resource, skipping update")
	}
...
func (r *LbvserverRewritepolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Lbvserver_rewritepolicy_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete lbvserver_rewritepolicy_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted lbvserver_rewritepolicy_binding binding")
}
...
func (r *LbvserverRewritepolicyBindingResource) readLbvserverRewritepolicyBindingFromApi(ctx context.Context, data *LbvserverRewritepolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "lbvserver_rewritepolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("lbvserver_rewritepolicy_binding not found with the provided ID attributes"))
		return
	}
