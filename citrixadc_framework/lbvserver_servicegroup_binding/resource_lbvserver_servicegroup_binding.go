...
func (r *LbvserverServicegroupBindingResource) readLbvserverServicegroupBindingFromApi(ctx context.Context, data *LbvserverServicegroupBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "lbvserver_servicegroup_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("lbvserver_servicegroup_binding not found with the provided ID attributes"))
		return
	}
