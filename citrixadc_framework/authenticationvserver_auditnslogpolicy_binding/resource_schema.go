...
func (r *AuthenticationvserverAuditnslogpolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
...
func authenticationvserver_auditnslogpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *AuthenticationvserverAuditnslogpolicyBindingResourceModel) authentication.Authenticationvserverauditnslogpolicybinding {
	tflog.Debug(ctx, "In authenticationvserver_auditnslogpolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Groupextraction.IsNull() {
		authenticationvserver_auditnslogpolicy_binding.Groupextraction = data.Groupextraction.ValueBool()
	}
	if !data.Name.IsNull() {
		authenticationvserver_auditnslogpolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Nextfactor.IsNull() {
		authenticationvserver_auditnslogpolicy_binding.Nextfactor = data.Nextfactor.ValueString()
	}
	if !data.Policy.IsNull() {
		authenticationvserver_auditnslogpolicy_binding.Policy = data.Policy.ValueString()
	}
	if !data.Priority.IsNull() {
		authenticationvserver_auditnslogpolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
	if !data.Secondary.IsNull() {
		authenticationvserver_auditnslogpolicy_binding.Secondary = data.Secondary.ValueBool()
	}
