...
func (r *CsvserverVpnvserverBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "csvserver_vpnvserver_binding not found on the ADC immediately after create")
		return
	}
...
func (r *CsvserverVpnvserverBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state CsvserverVpnvserverBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for csvserver_vpnvserver_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "csvserver_vpnvserver_binding not found on the ADC immediately after update")
		return
	}
...
func (r *CsvserverVpnvserverBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Csvserver_vpnvserver_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete csvserver_vpnvserver_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted csvserver_vpnvserver_binding binding")
}
