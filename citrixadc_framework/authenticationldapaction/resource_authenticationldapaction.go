...
var _ resource.ResourceWithUpgradeState = &AuthenticationldapactionResource{}
...
func (r *AuthenticationldapactionResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	...
	return utils.WoVersionUpgradeState(schemaResp.Schema, func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
		...
		if data.LdapbinddnpasswordWoVersion.IsNull() {
			data.LdapbinddnpasswordWoVersion = types.Int64Value(1)
		}
		resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
	})
}
