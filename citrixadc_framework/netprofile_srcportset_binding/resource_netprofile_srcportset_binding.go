...
func (r *NetprofileSrcportsetBindingResource) readNetprofileSrcportsetBindingFromApi(ctx context.Context, data *NetprofileSrcportsetBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "netprofile_srcportset_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("netprofile_srcportset_binding not found with the provided ID attributes"))
		return
	}
