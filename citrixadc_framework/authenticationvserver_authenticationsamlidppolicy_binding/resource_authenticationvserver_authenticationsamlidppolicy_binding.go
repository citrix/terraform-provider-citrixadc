...
func (r *AuthenticationvserverAuthenticationsamlidppolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Authenticationvserver_authenticationsamlidppolicy_binding.Type(), name_value, argsMap)
	if err != nil {
...
func (r *AuthenticationvserverAuthenticationsamlidppolicyBindingResource) readAuthenticationvserverAuthenticationsamlidppolicyBindingFromApi(ctx context.Context, data *AuthenticationvserverAuthenticationsamlidppolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "authenticationvserver_authenticationsamlidppolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("authenticationvserver_authenticationsamlidppolicy_binding not found with the provided ID attributes"))
		return
	}
