...
// AuthenticationvserverAuthenticationtacacspolicyBindingDataSourceModel is the
// data-source-specific model, decoupled from the resource model. A data source
// is a pure read surface, so it exposes the full GET projection: the read/write
...
type AuthenticationvserverAuthenticationtacacspolicyBindingDataSourceModel struct {
	...
	// (zion73x_readonly/authenticationvserver_authenticationtacacspolicy_binding.json).
	Acttype types.Int64 `tfsdk:"acttype"`
}

func AuthenticationvserverAuthenticationtacacspolicyBindingDataSourceSchema() schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"acttype": schema.Int64Attribute{
				Computed:    true,
				Description: "Type of the bound authentication policy action, as reported by the appliance.",
			},
		},
	}
}

// authenticationvserver_authenticationtacacspolicy_bindingDataSourceSetAttrFromGet
// projects a NITRO GET response onto the data-source model. Attributes are
// simply filled from the GET (or left Null when the GET omits them) via the
// shared utils.MapGet* helpers.
func authenticationvserver_authenticationtacacspolicy_bindingDataSourceSetAttrFromGet(ctx context.Context, data *AuthenticationvserverAuthenticationtacacspolicyBindingDataSourceModel, g map[string]interface{}) {
	tflog.Debug(ctx, "In authenticationvserver_authenticationtacacspolicy_bindingDataSourceSetAttrFromGet Function")
	...
	// Compose the ID for the datasource (no Create step).
	// ID = groupextraction,name,policy,secondary
	idParts := []string{}
