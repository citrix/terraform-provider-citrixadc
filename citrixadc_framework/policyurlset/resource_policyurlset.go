...
var _ resource.ResourceWithUpgradeState = &PolicyurlsetResource{}
...
func (r *PolicyurlsetResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	...
	return utils.WoVersionUpgradeState(schemaResp.Schema, func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
		...
		if data.UrlWoVersion.IsNull() {
			data.UrlWoVersion = types.Int64Value(1)
		}
		resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
	})
}
