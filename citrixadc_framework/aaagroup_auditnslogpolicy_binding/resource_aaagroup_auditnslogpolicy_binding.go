...
func (r *AaagroupAuditnslogpolicyBindingResource) readAaagroupAuditnslogpolicyBindingFromApi(ctx context.Context, data *AaagroupAuditnslogpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "aaagroup_auditnslogpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("aaagroup_auditnslogpolicy_binding not found with the provided ID attributes"))
		return
	}
