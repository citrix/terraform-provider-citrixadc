...
func (r *VpneulaResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// vpneula := vpneulaGetThePayloadFromtheConfig(ctx, &data)

	// Make API call
	// err := r.client.UpdateUnnamedResource(service.Vpneula.Type(), &vpneula)
	// if err != nil {
	...
	data.Id = types.StringValue("vpneula-config")

	tflog.Trace(ctx, "Created vpneula resource")

	// Read the updated state back
	r.readVpneulaFromApi(ctx, &data, &resp.Diagnostics)
...
func (r *VpneulaResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// err := r.client.UpdateUnnamedResource(service.Vpneula.Type(), &vpneula)
	// if err != nil {
	// 	 resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to update vpneula, got error: %s", err))
	//	 return
	// }

	tflog.Trace(ctx, "Updated vpneula resource")
...
func (r *VpneulaResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// For vpneula, we don't actually delete the resource as it's a global configuration
	// We just remove it from state
	tflog.Trace(ctx, "Deleted vpneula resource from state")
}

// Helper function to read vpneula data from API
func (r *VpneulaResource) readVpneulaFromApi(ctx context.Context, data *VpneulaResourceModel, diags *diag.Diagnostics) {
	getResponseData, err := r.client.FindResource(service.Vpneula.Type(), "")
	if err != nil {
