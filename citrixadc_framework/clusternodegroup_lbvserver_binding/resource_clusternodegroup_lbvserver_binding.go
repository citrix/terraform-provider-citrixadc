...
func (r *ClusternodegroupLbvserverBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "clusternodegroup_lbvserver_binding not found on the ADC immediately after create")
		return
	}
...
func (r *ClusternodegroupLbvserverBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// All attributes are identity attributes (ForceNew in SDK v2 -> RequiresReplace).
	// There are no updateable attributes, so this is a no-op update (matches SDK v2 which had no Update).
	tflog.Debug(ctx, "No updateable attributes for clusternodegroup_lbvserver_binding resource, skipping update")
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "clusternodegroup_lbvserver_binding not found on the ADC immediately after update")
		return
	}
...
func (r *ClusternodegroupLbvserverBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Clusternodegroup_lbvserver_binding.Type(), name_value, argsMap)
	if err != nil {
...
func (r *ClusternodegroupLbvserverBindingResource) readClusternodegroupLbvserverBindingFromApi(ctx context.Context, data *ClusternodegroupLbvserverBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Only match on keys actually present in the parsed id (backward compat with legacy ids).
	foundIndex := -1
