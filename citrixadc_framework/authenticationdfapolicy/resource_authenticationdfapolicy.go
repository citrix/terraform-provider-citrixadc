...
func (r *AuthenticationdfapolicyResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	authenticationdfapolicy := authenticationdfapolicyGetThePayloadFromthePlan(ctx, &data)

	// Make API call
	// Named resource - use AddResource
	name_value := data.Name.ValueString()
	_, err := r.client.AddResource(service.Authenticationdfapolicy.Type(), name_value, &authenticationdfapolicy)
	if err != nil {
	...
	if !r.readAuthenticationdfapolicyFromApi(ctx, &data, &resp.Diagnostics) {
		if !resp.Diagnostics.HasError() {
			resp.Diagnostics.AddError("Client Error", "authenticationdfapolicy not found immediately after create")
		}
		return
	}
...
func (r *AuthenticationdfapolicyResource) Read(ctx context.Context, req resource.ReadRequest, resp *resource.ReadResponse) {
	...
	found := r.readAuthenticationdfapolicyFromApi(ctx, &data, &resp.Diagnostics)
	if resp.Diagnostics.HasError() {
		return
	}
	if !found {
...
func (r *AuthenticationdfapolicyResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state AuthenticationdfapolicyResourceModel
	...
	if !data.Action.Equal(state.Action) {
		tflog.Debug(ctx, "action has changed for authenticationdfapolicy")
		hasChange = true
	}
	if !data.Rule.Equal(state.Rule) {
		tflog.Debug(ctx, "rule has changed for authenticationdfapolicy")
		hasChange = true
	}

	if hasChange {
		// Create API request body from the model
		authenticationdfapolicy := authenticationdfapolicyGetTheUpdatablePayloadFromThePlan(ctx, &data)
		// Make API call
		// Named resource - use UpdateResource
		name_value := data.Name.ValueString()
		_, err := r.client.UpdateResource(service.Authenticationdfapolicy.Type(), name_value, &authenticationdfapolicy)
		if err != nil {
		...
		tflog.Debug(ctx, "No changes detected for authenticationdfapolicy resource, skipping update")
	}

	// Read the updated state back
	if !r.readAuthenticationdfapolicyFromApi(ctx, &data, &resp.Diagnostics) {
		if !resp.Diagnostics.HasError() {
			resp.Diagnostics.AddError("Client Error", "authenticationdfapolicy not found immediately after update")
		}
		return
	}
...
func (r *AuthenticationdfapolicyResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	name_value := data.Name.ValueString()
	err := r.client.DeleteResource(service.Authenticationdfapolicy.Type(), name_value)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete authenticationdfapolicy, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted authenticationdfapolicy resource")
}

// Helper function to read authenticationdfapolicy data from API
func (r *AuthenticationdfapolicyResource) readAuthenticationdfapolicyFromApi(ctx context.Context, data *AuthenticationdfapolicyResourceModel, diags *diag.Diagnostics) bool {
	...
	getResponseData, err = r.client.FindResource(service.Authenticationdfapolicy.Type(), name_Name)
	if err != nil {
