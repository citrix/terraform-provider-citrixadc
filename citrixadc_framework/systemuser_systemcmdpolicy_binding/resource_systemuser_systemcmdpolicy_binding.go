...
func (r *SystemuserSystemcmdpolicyBindingResource) readSystemuserSystemcmdpolicyBindingFromApi(ctx context.Context, data *SystemuserSystemcmdpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "systemuser_systemcmdpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("systemuser_systemcmdpolicy_binding not found with the provided ID attributes"))
		return
	}
