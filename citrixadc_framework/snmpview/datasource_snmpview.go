...
func (d *SnmpviewDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data SnmpviewDataSourceModel
	...
	subtree_Name := data.Subtree.ValueString()

	var dataArr []map[string]interface{}
	var err error

	findParams := service.FindParams{
		ResourceType:             "snmpview",
		ResourceMissingErrorCode: 258,
	}
	dataArr, err = d.client.FindResourceArrayWithParams(findParams)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read snmpview, got error: %s", err))
		return
	}

	// Resource is missing
	if len(dataArr) == 0 {
		resp.Diagnostics.AddError("Client Error", "snmpview returned empty array")
		return
	}
	...
	for i, v := range dataArr {
		...
		if v["subtree"].(string) != subtree_Name {
			match = false
		}
	...
	// Resource is missing
	if foundIndex == -1 {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("snmpview with name %s not found", name_Name))
		return
	}

	snmpviewDataSourceSetAttrFromGet(ctx, &data, dataArr[foundIndex])
