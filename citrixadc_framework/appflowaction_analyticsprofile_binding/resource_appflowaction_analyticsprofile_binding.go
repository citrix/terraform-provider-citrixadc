...
func (r *AppflowactionAnalyticsprofileBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	appflowaction_analyticsprofile_binding := appflowaction_analyticsprofile_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "appflowaction_analyticsprofile_binding not found on the ADC immediately after create")
		return
	}
...
func (r *AppflowactionAnalyticsprofileBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state AppflowactionAnalyticsprofileBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for appflowaction_analyticsprofile_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "appflowaction_analyticsprofile_binding not found on the ADC immediately after update")
		return
	}
...
func (r *AppflowactionAnalyticsprofileBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Appflowaction_analyticsprofile_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete appflowaction_analyticsprofile_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted appflowaction_analyticsprofile_binding binding")
}
