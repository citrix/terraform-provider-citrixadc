...
func (r *AaauserIntranetip6BindingResource) readAaauserIntranetip6BindingFromApi(ctx context.Context, data *AaauserIntranetip6BindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "aaauser_intranetip6_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("aaauser_intranetip6_binding not found with the provided ID attributes"))
		return
	}
