...
func (r *AaauserAuditnslogpolicyBindingResource) readAaauserAuditnslogpolicyBindingFromApi(ctx context.Context, data *AaauserAuditnslogpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "aaauser_auditnslogpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("aaauser_auditnslogpolicy_binding not found with the provided ID attributes"))
		return
	}
