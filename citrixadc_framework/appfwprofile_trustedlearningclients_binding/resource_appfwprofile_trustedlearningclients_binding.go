...
func (r *AppfwprofileTrustedlearningclientsBindingResource) readAppfwprofileTrustedlearningclientsBindingFromApi(ctx context.Context, data *AppfwprofileTrustedlearningclientsBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "appfwprofile_trustedlearningclients_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("appfwprofile_trustedlearningclients_binding not found with the provided ID attributes"))
		return
	}
