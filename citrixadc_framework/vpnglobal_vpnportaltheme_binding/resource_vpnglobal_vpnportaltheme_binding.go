...
func (r *VpnglobalVpnportalthemeBindingResource) readVpnglobalVpnportalthemeBindingFromApi(ctx context.Context, data *VpnglobalVpnportalthemeBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnglobal_vpnportaltheme_binding returned empty array")
		return
	}
	...
	// Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnglobal_vpnportaltheme_binding not found with the provided ID attributes"))
		return
	}
