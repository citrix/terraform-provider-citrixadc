...
func (r *AppfwprofileXmlxssBindingResource) readAppfwprofileXmlxssBindingFromApi(ctx context.Context, data *AppfwprofileXmlxssBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "appfwprofile_xmlxss_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("appfwprofile_xmlxss_binding not found with the provided ID attributes"))
		return
	}
