...
func (r *SystemuserNspartitionBindingResource) readSystemuserNspartitionBindingFromApi(ctx context.Context, data *SystemuserNspartitionBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "systemuser_nspartition_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("systemuser_nspartition_binding not found with the provided ID attributes"))
		return
	}
