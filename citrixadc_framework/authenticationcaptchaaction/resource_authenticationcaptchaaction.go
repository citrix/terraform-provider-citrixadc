...
var _ resource.ResourceWithUpgradeState = &AuthenticationcaptchaactionResource{}
...
func (r *AuthenticationcaptchaactionResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	...
	return utils.WoVersionUpgradeState(schemaResp.Schema, func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
		...
		if data.SecretkeyWoVersion.IsNull() {
			data.SecretkeyWoVersion = types.Int64Value(1)
		}
		if data.SitekeyWoVersion.IsNull() {
			data.SitekeyWoVersion = types.Int64Value(1)
		}
		resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
	})
}
