...
func (r *VpnvserverAuthenticationtacacspolicyBindingResource) readVpnvserverAuthenticationtacacspolicyBindingFromApi(ctx context.Context, data *VpnvserverAuthenticationtacacspolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnvserver_authenticationtacacspolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnvserver_authenticationtacacspolicy_binding not found with the provided ID attributes"))
		return
	}
