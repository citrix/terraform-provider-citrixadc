...
func (d *ClusternodegroupCrvserverBindingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Resource is missing
	if foundIndex == -1 {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("clusternodegroup_crvserver_binding with name %s not found", name_Name))
		return
	}

	clusternodegroup_crvserver_bindingSetAttrFromGetForDatasource(ctx, &data, dataArr[foundIndex])
