...
func (r *ClusternodegroupCrvserverBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// Update is a no-op for clusternodegroup_crvserver_binding: NITRO exposes only add (PUT)
	...
	tflog.Debug(ctx, "Update is a no-op for clusternodegroup_crvserver_binding; all attributes are RequiresReplace")
...
func (r *ClusternodegroupCrvserverBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgs(service.Clusternodegroup_crvserver_binding.Type(), name, args)
	if err != nil {
...
func (r *ClusternodegroupCrvserverBindingResource) readClusternodegroupCrvserverBindingFromApi(ctx context.Context, data *ClusternodegroupCrvserverBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "clusternodegroup_crvserver_binding returned empty array")
		return
	}
	...
	// Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("clusternodegroup_crvserver_binding not found with the provided ID attributes"))
		return
	}
