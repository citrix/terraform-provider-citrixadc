...
func (r *LbvserverFeopolicyBindingResource) readLbvserverFeopolicyBindingFromApi(ctx context.Context, data *LbvserverFeopolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "lbvserver_feopolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("lbvserver_feopolicy_binding not found with the provided ID attributes"))
		return
	}
