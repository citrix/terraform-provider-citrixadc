...
var _ resource.ResourceWithUpgradeState = &AppflowparamResource{}
...
func (r *AppflowparamResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	...
	return utils.WoVersionUpgradeState(schemaResp.Schema, func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
		...
		if data.AnalyticsauthtokenWoVersion.IsNull() {
			data.AnalyticsauthtokenWoVersion = types.Int64Value(1)
		}
		// SDKv2 echoed the secret into state; the Framework provider does not. Drop the stale echo so
		// the upgrade plans no spurious (hidden, sensitive) secret diff. Safe: the
		// create/update payload only sends it when non-null, so nulling never
