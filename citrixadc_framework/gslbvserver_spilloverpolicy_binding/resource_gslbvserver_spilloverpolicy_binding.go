...
func (r *GslbvserverSpilloverpolicyBindingResource) readGslbvserverSpilloverpolicyBindingFromApi(ctx context.Context, data *GslbvserverSpilloverpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "gslbvserver_spilloverpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("gslbvserver_spilloverpolicy_binding not found with the provided ID attributes"))
		return
	}
