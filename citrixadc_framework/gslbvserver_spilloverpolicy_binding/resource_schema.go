...
// GslbvserverSpilloverpolicyBindingResourceModel describes the resource data model.
type GslbvserverSpilloverpolicyBindingResourceModel struct {
...
func (r *GslbvserverSpilloverpolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Version: 1,
		Attributes: map[string]schema.Attribute{
			"id": schema.StringAttribute{
				Computed:    true,
				Description: "The ID of the gslbvserver_spilloverpolicy_binding resource.",
			},
...
func gslbvserver_spilloverpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *GslbvserverSpilloverpolicyBindingResourceModel) gslb.Gslbvserverspilloverpolicybinding {
	tflog.Debug(ctx, "In gslbvserver_spilloverpolicy_bindingGetThePayloadFromtheConfig Function")

	// Create API request body from the model
	gslbvserver_spilloverpolicy_binding := gslb.Gslbvserverspilloverpolicybinding{}
	if !data.Gotopriorityexpression.IsNull() {
		gslbvserver_spilloverpolicy_binding.Gotopriorityexpression = data.Gotopriorityexpression.ValueString()
	}
	if !data.Name.IsNull() {
		gslbvserver_spilloverpolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Order.IsNull() {
		gslbvserver_spilloverpolicy_binding.Order = utils.IntPtr(int(data.Order.ValueInt64()))
	}
	if !data.Policyname.IsNull() {
		gslbvserver_spilloverpolicy_binding.Policyname = data.Policyname.ValueString()
	}
	if !data.Priority.IsNull() {
		gslbvserver_spilloverpolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
	if !data.Type.IsNull() {
		gslbvserver_spilloverpolicy_binding.Type = data.Type.ValueString()
	}

	return gslbvserver_spilloverpolicy_binding
}

func gslbvserver_spilloverpolicy_bindingSetAttrFromGet(ctx context.Context, data *GslbvserverSpilloverpolicyBindingResourceModel, getResponseData map[string]interface{}) *GslbvserverSpilloverpolicyBindingResourceModel {
	tflog.Debug(ctx, "In gslbvserver_spilloverpolicy_bindingSetAttrFromGet Function")
