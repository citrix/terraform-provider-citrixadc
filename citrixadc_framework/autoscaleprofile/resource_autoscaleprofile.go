...
var _ resource.ResourceWithUpgradeState = &AutoscaleprofileResource{}
...
func (r *AutoscaleprofileResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	...
	return utils.WoVersionUpgradeState(schemaResp.Schema, func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
		...
		if data.ApikeyWoVersion.IsNull() {
			data.ApikeyWoVersion = types.Int64Value(1)
		}
		if data.SharedsecretWoVersion.IsNull() {
			data.SharedsecretWoVersion = types.Int64Value(1)
		}
		resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
	})
}
