...
func (r *CsvserverAuthorizationpolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
...
func csvserver_authorizationpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *CsvserverAuthorizationpolicyBindingResourceModel) cs.Csvserverauthorizationpolicybinding {
	tflog.Debug(ctx, "In csvserver_authorizationpolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Invoke.IsNull() {
		csvserver_authorizationpolicy_binding.Invoke = data.Invoke.ValueBool()
	}
	if !data.Labelname.IsNull() {
		csvserver_authorizationpolicy_binding.Labelname = data.Labelname.ValueString()
	}
	if !data.Labeltype.IsNull() {
		csvserver_authorizationpolicy_binding.Labeltype = data.Labeltype.ValueString()
	}
	if !data.Name.IsNull() {
		csvserver_authorizationpolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Policyname.IsNull() {
		csvserver_authorizationpolicy_binding.Policyname = data.Policyname.ValueString()
	}
	if !data.Priority.IsNull() {
		csvserver_authorizationpolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
	if !data.Targetlbvserver.IsNull() {
		csvserver_authorizationpolicy_binding.Targetlbvserver = data.Targetlbvserver.ValueString()
	}
