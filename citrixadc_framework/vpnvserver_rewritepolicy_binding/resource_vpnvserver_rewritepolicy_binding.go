...
func (r *VpnvserverRewritepolicyBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	vpnvserver_rewritepolicy_binding := vpnvserver_rewritepolicy_bindingGetThePayloadFromthePlan(ctx, &data)
...
func (r *VpnvserverRewritepolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state VpnvserverRewritepolicyBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for vpnvserver_rewritepolicy_binding resource, skipping update")
	}
...
func (r *VpnvserverRewritepolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Vpnvserver_rewritepolicy_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete vpnvserver_rewritepolicy_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted vpnvserver_rewritepolicy_binding binding")
}
