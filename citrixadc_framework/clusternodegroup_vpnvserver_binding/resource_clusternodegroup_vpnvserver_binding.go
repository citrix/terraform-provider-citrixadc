...
func (r *ClusternodegroupVpnvserverBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "clusternodegroup_vpnvserver_binding not found on the ADC immediately after create")
		return
	}
...
func (r *ClusternodegroupVpnvserverBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	...
	// The SDK v2 resource had no Update: name and vserver are both ForceNew, so
	// there are no updateable attributes. This is a no-op update that simply
	// refreshes state from the API.
	tflog.Debug(ctx, "No updateable attributes for clusternodegroup_vpnvserver_binding resource, skipping update")
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "clusternodegroup_vpnvserver_binding not found on the ADC immediately after update")
		return
	}
...
func (r *ClusternodegroupVpnvserverBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Clusternodegroup_vpnvserver_binding.Type(), name_value, argsMap)
	if err != nil {
