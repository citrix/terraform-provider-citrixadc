...
func (r *NetbridgeVlanBindingResource) readNetbridgeVlanBindingFromApi(ctx context.Context, data *NetbridgeVlanBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "netbridge_vlan_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("netbridge_vlan_binding not found with the provided ID attributes"))
		return
	}
