...
func (r *AuthenticationvserverAuthenticationoauthidppolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Authenticationvserver_authenticationoauthidppolicy_binding.Type(), name_value, argsMap)
	if err != nil {
...
func (r *AuthenticationvserverAuthenticationoauthidppolicyBindingResource) readAuthenticationvserverAuthenticationoauthidppolicyBindingFromApi(ctx context.Context, data *AuthenticationvserverAuthenticationoauthidppolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "authenticationvserver_authenticationoauthidppolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("authenticationvserver_authenticationoauthidppolicy_binding not found with the provided ID attributes"))
		return
	}
