...
func (r *LbvserverTransformpolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
...
func lbvserver_transformpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *LbvserverTransformpolicyBindingResourceModel) lb.Lbvservertransformpolicybinding {
	tflog.Debug(ctx, "In lbvserver_transformpolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Invoke.IsNull() {
		lbvserver_transformpolicy_binding.Invoke = data.Invoke.ValueBool()
	}
	if !data.Labelname.IsNull() {
		lbvserver_transformpolicy_binding.Labelname = data.Labelname.ValueString()
	}
	if !data.Labeltype.IsNull() {
		lbvserver_transformpolicy_binding.Labeltype = data.Labeltype.ValueString()
	}
	if !data.Name.IsNull() {
		lbvserver_transformpolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Order.IsNull() {
		lbvserver_transformpolicy_binding.Order = utils.IntPtr(int(data.Order.ValueInt64()))
	}
	if !data.Policyname.IsNull() {
		lbvserver_transformpolicy_binding.Policyname = data.Policyname.ValueString()
	}
	if !data.Priority.IsNull() {
		lbvserver_transformpolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
