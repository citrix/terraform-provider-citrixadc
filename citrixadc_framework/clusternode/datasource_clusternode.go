...
var _ datasource.DataSource = (*ClusternodeDataSource)(nil)

func CLusternodeDataSource() datasource.DataSource {
	return &ClusternodeDataSource{}
}
...
func (d *ClusternodeDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	nodeid_Name := types.StringValue(fmt.Sprintf("%d", data.Nodeid.ValueInt64()))

	var getResponseData map[string]interface{}
	var err error

	getResponseData, err = d.client.FindResource(service.Clusternode.Type(), nodeid_Name.ValueString())
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read clusternode, got error: %s", err))
		return
	}

	clusternodeDataSourceSetAttrFromGet(ctx, &data, getResponseData)
