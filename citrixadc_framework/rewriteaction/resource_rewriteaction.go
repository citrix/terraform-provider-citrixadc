...
func (r *RewriteactionResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// rewriteaction := rewriteactionGetThePayloadFromtheConfig(ctx, &data)

	// Make API call
	// err := r.client.UpdateUnnamedResource(service.Rewriteaction.Type(), &rewriteaction)
	// if err != nil {
	...
	data.Id = types.StringValue("rewriteaction-config")
...
func (r *RewriteactionResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// For rewriteaction, we don't actually delete the resource as it's a global configuration
	// We just remove it from state
	tflog.Trace(ctx, "Deleted rewriteaction resource from state")
}

// Helper function to read rewriteaction data from API
func (r *RewriteactionResource) readRewriteactionFromApi(ctx context.Context, data *RewriteactionResourceModel, diags *diag.Diagnostics) {
	getResponseData, err := r.client.FindResource(service.Rewriteaction.Type(), "")
	if err != nil {
