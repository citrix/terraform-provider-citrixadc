...
func (r *VpnvserverVpnintranetapplicationBindingResource) readVpnvserverVpnintranetapplicationBindingFromApi(ctx context.Context, data *VpnvserverVpnintranetapplicationBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnvserver_vpnintranetapplication_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnvserver_vpnintranetapplication_binding not found with the provided ID attributes"))
		return
	}
