...
func (r *VpnvserverVpnintranetapplicationBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	vpnvserver_vpnintranetapplication_binding := vpnvserver_vpnintranetapplication_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "vpnvserver_vpnintranetapplication_binding not found on the ADC immediately after create")
		return
	}
...
func (r *VpnvserverVpnintranetapplicationBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state VpnvserverVpnintranetapplicationBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for vpnvserver_vpnintranetapplication_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "vpnvserver_vpnintranetapplication_binding not found on the ADC immediately after update")
		return
	}
...
func (r *VpnvserverVpnintranetapplicationBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	idMap, _, err := utils.ParseIdString(data.Id.ValueString(), []string{"name", "intranetapplication"}, nil)
	if err != nil {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Vpnvserver_vpnintranetapplication_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete vpnvserver_vpnintranetapplication_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted vpnvserver_vpnintranetapplication_binding binding")
}
