...
func (r *SystemgroupSystemcmdpolicyBindingResource) readSystemgroupSystemcmdpolicyBindingFromApi(ctx context.Context, data *SystemgroupSystemcmdpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "systemgroup_systemcmdpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("systemgroup_systemcmdpolicy_binding not found with the provided ID attributes"))
		return
	}
