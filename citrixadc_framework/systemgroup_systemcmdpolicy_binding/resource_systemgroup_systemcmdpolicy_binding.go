...
func (r *SystemgroupSystemcmdpolicyBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	systemgroup_systemcmdpolicy_binding := systemgroup_systemcmdpolicy_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "systemgroup_systemcmdpolicy_binding not found on the ADC immediately after create")
		return
	}
...
func (r *SystemgroupSystemcmdpolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state SystemgroupSystemcmdpolicyBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for systemgroup_systemcmdpolicy_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "systemgroup_systemcmdpolicy_binding not found on the ADC immediately after update")
		return
	}
...
func (r *SystemgroupSystemcmdpolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	idMap, _, err := utils.ParseIdString(data.Id.ValueString(), []string{"groupname", "policyname"}, nil)
	if err != nil {
	...
	err = r.client.DeleteResourceWithArgsMap(service.Systemgroup_systemcmdpolicy_binding.Type(), groupname_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete systemgroup_systemcmdpolicy_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted systemgroup_systemcmdpolicy_binding binding")
}
