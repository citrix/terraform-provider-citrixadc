...
func (d *AuthenticationvserverResponderpolicyBindingDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	// Resource is missing
	if foundIndex == -1 {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("authenticationvserver_responderpolicy_binding with groupextraction %s not found", groupextraction_Name))
		return
	}
