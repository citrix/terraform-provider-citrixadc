...
func (r *CsvserverFeopolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
...
func csvserver_feopolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *CsvserverFeopolicyBindingResourceModel) cs.Csvserverfeopolicybinding {
	tflog.Debug(ctx, "In csvserver_feopolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Invoke.IsNull() {
		csvserver_feopolicy_binding.Invoke = data.Invoke.ValueBool()
	}
	if !data.Labelname.IsNull() {
		csvserver_feopolicy_binding.Labelname = data.Labelname.ValueString()
	}
	if !data.Labeltype.IsNull() {
		csvserver_feopolicy_binding.Labeltype = data.Labeltype.ValueString()
	}
	if !data.Name.IsNull() {
		csvserver_feopolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Policyname.IsNull() {
		csvserver_feopolicy_binding.Policyname = data.Policyname.ValueString()
	}
	if !data.Priority.IsNull() {
		csvserver_feopolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
	if !data.Targetlbvserver.IsNull() {
		csvserver_feopolicy_binding.Targetlbvserver = data.Targetlbvserver.ValueString()
	}
