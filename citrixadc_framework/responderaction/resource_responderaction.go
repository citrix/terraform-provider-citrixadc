...
func (r *ResponderactionResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	// responderaction := responderactionGetThePayloadFromtheConfig(ctx, &data)

	// Make API call
	// err := r.client.UpdateUnnamedResource(service.Responderaction.Type(), &responderaction)
	// if err != nil {
	...
	data.Id = types.StringValue("responderaction-config")
...
func (r *ResponderactionResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	// For responderaction, we don't actually delete the resource as it's a global configuration
	// We just remove it from state
	tflog.Trace(ctx, "Deleted responderaction resource from state")
}

// Helper function to read responderaction data from API
func (r *ResponderactionResource) readResponderactionFromApi(ctx context.Context, data *ResponderactionResourceModel, diags *diag.Diagnostics) {
	getResponseData, err := r.client.FindResource(service.Responderaction.Type(), "")
	if err != nil {
