...
func (r *LbvserverAppflowpolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
...
func lbvserver_appflowpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *LbvserverAppflowpolicyBindingResourceModel) lb.Lbvserverappflowpolicybinding {
	tflog.Debug(ctx, "In lbvserver_appflowpolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Invoke.IsNull() {
		lbvserver_appflowpolicy_binding.Invoke = data.Invoke.ValueBool()
	}
	if !data.Labelname.IsNull() {
		lbvserver_appflowpolicy_binding.Labelname = data.Labelname.ValueString()
	}
	if !data.Labeltype.IsNull() {
		lbvserver_appflowpolicy_binding.Labeltype = data.Labeltype.ValueString()
	}
	if !data.Name.IsNull() {
		lbvserver_appflowpolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Order.IsNull() {
		lbvserver_appflowpolicy_binding.Order = utils.IntPtr(int(data.Order.ValueInt64()))
	}
	if !data.Policyname.IsNull() {
		lbvserver_appflowpolicy_binding.Policyname = data.Policyname.ValueString()
	}
	if !data.Priority.IsNull() {
		lbvserver_appflowpolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
