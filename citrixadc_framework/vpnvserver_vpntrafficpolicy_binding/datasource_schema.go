...
// VpnvserverVpntrafficpolicyBindingDataSourceModel is the data-source-specific
// model, decoupled from VpnvserverVpntrafficpolicyBindingResourceModel.
//
// A data source is a pure read surface (Read only; no plan/apply lifecycle), so
// it can expose the FULL GET projection: the read/write attributes (as Computed
// outputs) AND the read-only attributes that the resource deliberately omits.
// Every non-key attribute is Computed.
type VpnvserverVpntrafficpolicyBindingDataSourceModel struct {
	...
	// (zion73x_readonly/vpnvserver_vpntrafficpolicy_binding.json). Never
	// settable; populated from GET, null when the appliance omits them.
	Acttype types.Int64 `tfsdk:"acttype"`
}

func VpnvserverVpntrafficpolicyBindingDataSourceSchema() schema.Schema {
...
// vpnvserver_vpntrafficpolicy_bindingDataSourceSetAttrFromGet projects a NITRO
// vpnvserver_vpntrafficpolicy_binding GET response onto the data-source model.
...
func vpnvserver_vpntrafficpolicy_bindingDataSourceSetAttrFromGet(ctx context.Context, data *VpnvserverVpntrafficpolicyBindingDataSourceModel, g map[string]interface{}) {
	tflog.Debug(ctx, "In vpnvserver_vpntrafficpolicy_bindingDataSourceSetAttrFromGet Function")
