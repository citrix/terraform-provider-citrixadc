...
func (r *AuthenticationvserverAuthenticationcertpolicyBindingResource) Schema(ctx context.Context, req resource.SchemaRequest, resp *resource.SchemaResponse) {
...
func authenticationvserver_authenticationcertpolicy_bindingGetThePayloadFromtheConfig(ctx context.Context, data *AuthenticationvserverAuthenticationcertpolicyBindingResourceModel) authentication.Authenticationvserverauthenticationcertpolicybinding {
	tflog.Debug(ctx, "In authenticationvserver_authenticationcertpolicy_bindingGetThePayloadFromtheConfig Function")
	...
	if !data.Groupextraction.IsNull() {
		authenticationvserver_authenticationcertpolicy_binding.Groupextraction = data.Groupextraction.ValueBool()
	}
	if !data.Name.IsNull() {
		authenticationvserver_authenticationcertpolicy_binding.Name = data.Name.ValueString()
	}
	if !data.Nextfactor.IsNull() {
		authenticationvserver_authenticationcertpolicy_binding.Nextfactor = data.Nextfactor.ValueString()
	}
	if !data.Policy.IsNull() {
		authenticationvserver_authenticationcertpolicy_binding.Policy = data.Policy.ValueString()
	}
	if !data.Priority.IsNull() {
		authenticationvserver_authenticationcertpolicy_binding.Priority = utils.IntPtr(int(data.Priority.ValueInt64()))
	}
	if !data.Secondary.IsNull() {
		authenticationvserver_authenticationcertpolicy_binding.Secondary = data.Secondary.ValueBool()
	}
