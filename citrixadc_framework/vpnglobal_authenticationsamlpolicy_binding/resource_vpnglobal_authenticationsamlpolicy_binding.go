...
func (r *VpnglobalAuthenticationsamlpolicyBindingResource) readVpnglobalAuthenticationsamlpolicyBindingFromApi(ctx context.Context, data *VpnglobalAuthenticationsamlpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnglobal_authenticationsamlpolicy_binding returned empty array")
		return
	}
	...
	// Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnglobal_authenticationsamlpolicy_binding not found with the provided ID attributes"))
		return
	}
