...
func (r *RnatparamResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, config, state RnatparamResourceModel
	...
	if err := utils.ExecuteUnset(r.client, service.Rnatparam.Type(), unsetIdPayload, attributesToUnset); err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to unset rnatparam attributes, got error: %s", err))
		return
	}
