...
func (r *VpnglobalVpnurlpolicyBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	vpnglobal_vpnurlpolicy_binding := vpnglobal_vpnurlpolicy_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "vpnglobal_vpnurlpolicy_binding not found on the ADC immediately after create")
		return
	}
...
func (r *VpnglobalVpnurlpolicyBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state VpnglobalVpnurlpolicyBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for vpnglobal_vpnurlpolicy_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "vpnglobal_vpnurlpolicy_binding not found on the ADC immediately after update")
		return
	}
...
func (r *VpnglobalVpnurlpolicyBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	err := r.client.DeleteResourceWithArgs(service.Vpnglobal_vpnurlpolicy_binding.Type(), "", args)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete vpnglobal_vpnurlpolicy_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted vpnglobal_vpnurlpolicy_binding binding")
}
