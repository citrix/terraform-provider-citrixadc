...
func (r *VpnglobalVpnurlpolicyBindingResource) readVpnglobalVpnurlpolicyBindingFromApi(ctx context.Context, data *VpnglobalVpnurlpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnglobal_vpnurlpolicy_binding returned empty array")
		return
	}
	...
	// Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnglobal_vpnurlpolicy_binding not found with the provided ID attributes"))
		return
	}
