...
func (r *AaauserVpnintranetapplicationBindingResource) readAaauserVpnintranetapplicationBindingFromApi(ctx context.Context, data *AaauserVpnintranetapplicationBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "aaauser_vpnintranetapplication_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("aaauser_vpnintranetapplication_binding not found with the provided ID attributes"))
		return
	}
