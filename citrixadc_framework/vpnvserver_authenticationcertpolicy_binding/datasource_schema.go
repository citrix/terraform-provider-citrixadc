...
// VpnvserverAuthenticationcertpolicyBindingDataSourceModel is the
// data-source-specific model, decoupled from the resource model. It exposes the
// read/write attributes (as Computed outputs) AND the read-only attributes the
// appliance returns on a GET
// (zion73x_readonly/vpnvserver_authenticationcertpolicy_binding.json) that the
// resource intentionally omits.
type VpnvserverAuthenticationcertpolicyBindingDataSourceModel struct {
...
func VpnvserverAuthenticationcertpolicyBindingDataSourceSchema() schema.Schema {
...
// vpnvserver_authenticationcertpolicy_bindingDataSourceSetAttrFromGet projects a
// NITRO GET response onto the data-source model via the shared utils.MapGet*
// helpers. A data source has no plan/apply reconciliation, so attributes are simply
// filled from the GET (or left Null when the GET omits them).
func vpnvserver_authenticationcertpolicy_bindingDataSourceSetAttrFromGet(ctx context.Context, data *VpnvserverAuthenticationcertpolicyBindingDataSourceModel, g map[string]interface{}) {
	tflog.Debug(ctx, "In vpnvserver_authenticationcertpolicy_bindingDataSourceSetAttrFromGet Function")
