...
func (r *CsvserverCspolicyBindingResource) readCsvserverCspolicyBindingFromApi(ctx context.Context, data *CsvserverCspolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "csvserver_cspolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("csvserver_cspolicy_binding not found with the provided ID attributes"))
		return
	}
