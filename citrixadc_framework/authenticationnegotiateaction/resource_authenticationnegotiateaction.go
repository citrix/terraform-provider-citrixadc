...
var _ resource.ResourceWithUpgradeState = &AuthenticationnegotiateactionResource{}
...
func (r *AuthenticationnegotiateactionResource) UpgradeState(ctx context.Context) map[int64]resource.StateUpgrader {
	...
	return utils.WoVersionUpgradeState(schemaResp.Schema, func(ctx context.Context, req resource.UpgradeStateRequest, resp *resource.UpgradeStateResponse) {
		...
		if data.DomainuserpasswdWoVersion.IsNull() {
			data.DomainuserpasswdWoVersion = types.Int64Value(1)
		}
		resp.Diagnostics.Append(resp.State.Set(ctx, &data)...)
	})
}
