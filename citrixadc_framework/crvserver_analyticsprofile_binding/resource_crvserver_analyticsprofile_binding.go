...
func (r *CrvserverAnalyticsprofileBindingResource) readCrvserverAnalyticsprofileBindingFromApi(ctx context.Context, data *CrvserverAnalyticsprofileBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "crvserver_analyticsprofile_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("crvserver_analyticsprofile_binding not found with the provided ID attributes"))
		return
	}
