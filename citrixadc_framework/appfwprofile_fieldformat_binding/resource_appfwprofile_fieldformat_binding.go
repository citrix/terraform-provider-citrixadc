...
func (r *AppfwprofileFieldformatBindingResource) readAppfwprofileFieldformatBindingFromApi(ctx context.Context, data *AppfwprofileFieldformatBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "appfwprofile_fieldformat_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("appfwprofile_fieldformat_binding not found with the provided ID attributes"))
		return
	}
