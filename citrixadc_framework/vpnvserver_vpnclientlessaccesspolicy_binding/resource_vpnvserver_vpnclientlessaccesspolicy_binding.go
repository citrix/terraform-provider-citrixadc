...
func (r *VpnvserverVpnclientlessaccesspolicyBindingResource) readVpnvserverVpnclientlessaccesspolicyBindingFromApi(ctx context.Context, data *VpnvserverVpnclientlessaccesspolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnvserver_vpnclientlessaccesspolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnvserver_vpnclientlessaccesspolicy_binding not found with the provided ID attributes"))
		return
	}
