...
func (r *NspartitionVlanBindingResource) readNspartitionVlanBindingFromApi(ctx context.Context, data *NspartitionVlanBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "nspartition_vlan_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("nspartition_vlan_binding not found with the provided ID attributes"))
		return
	}
