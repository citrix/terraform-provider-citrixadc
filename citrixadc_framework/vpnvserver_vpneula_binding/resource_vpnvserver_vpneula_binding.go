...
func (r *VpnvserverVpneulaBindingResource) readVpnvserverVpneulaBindingFromApi(ctx context.Context, data *VpnvserverVpneulaBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnvserver_vpneula_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnvserver_vpneula_binding not found with the provided ID attributes"))
		return
	}
