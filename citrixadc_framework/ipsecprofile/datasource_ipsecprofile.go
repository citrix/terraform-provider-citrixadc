...
var _ datasource.DataSource = (*IpsecprofileDataSource)(nil)

func IPsecprofileDataSource() datasource.DataSource {
...
func (d *IpsecprofileDataSource) Schema(ctx context.Context, req datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = IpsecprofileDataSourceSchema()
}
...
func (d *IpsecprofileDataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	...
	getResponseData, err = d.client.FindResource(service.Ipsecprofile.Type(), name_Name)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to read ipsecprofile, got error: %s", err))
		return
	}

	ipsecprofileDataSourceSetAttrFromGet(ctx, &data, getResponseData)
