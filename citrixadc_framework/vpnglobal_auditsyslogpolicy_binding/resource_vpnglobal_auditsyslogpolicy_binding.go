...
func (r *VpnglobalAuditsyslogpolicyBindingResource) readVpnglobalAuditsyslogpolicyBindingFromApi(ctx context.Context, data *VpnglobalAuditsyslogpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "vpnglobal_auditsyslogpolicy_binding returned empty array")
		return
	}
	...
	// Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("vpnglobal_auditsyslogpolicy_binding not found with the provided ID attributes"))
		return
	}
