...
func (r *NetbridgeIptunnelBindingResource) Create(ctx context.Context, req resource.CreateRequest, resp *resource.CreateResponse) {
	...
	netbridge_iptunnel_binding := netbridge_iptunnel_bindingGetThePayloadFromthePlan(ctx, &data)
	...
	idParts = append(idParts, fmt.Sprintf("tunnel:%s", utils.UrlEncode(fmt.Sprintf("%v", data.Tunnel.ValueString()))))
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "netbridge_iptunnel_binding not found on the ADC immediately after create")
		return
	}
...
func (r *NetbridgeIptunnelBindingResource) Update(ctx context.Context, req resource.UpdateRequest, resp *resource.UpdateResponse) {
	var data, state NetbridgeIptunnelBindingResourceModel
	...
	if hasChange {
		...
		tflog.Debug(ctx, "No changes detected for netbridge_iptunnel_binding resource, skipping update")
	}
	...
	if data.Id.IsNull() {
		resp.Diagnostics.AddError("Client Error", "netbridge_iptunnel_binding not found on the ADC immediately after update")
		return
	}
...
func (r *NetbridgeIptunnelBindingResource) Delete(ctx context.Context, req resource.DeleteRequest, resp *resource.DeleteResponse) {
	...
	idMap, _, err := utils.ParseIdString(data.Id.ValueString(), []string{"name", "tunnel"}, nil)
	if err != nil {
	...
	if val, ok := idMap["tunnel"]; ok && val != "" {
		argsMap["tunnel"] = val
	}

	err = r.client.DeleteResourceWithArgsMap(service.Netbridge_iptunnel_binding.Type(), name_value, argsMap)
	if err != nil {
		resp.Diagnostics.AddError("Client Error", fmt.Sprintf("Unable to delete netbridge_iptunnel_binding, got error: %s", err))
		return
	}

	tflog.Trace(ctx, "Deleted netbridge_iptunnel_binding binding")
}
...
func (r *NetbridgeIptunnelBindingResource) readNetbridgeIptunnelBindingFromApi(ctx context.Context, data *NetbridgeIptunnelBindingResourceModel, diags *diag.Diagnostics) {
	...
	for i, v := range dataArr {
		...
		} else if _, ok := v["tunnel"].(string); ok {
			match = false
			continue
		}
		if match {
