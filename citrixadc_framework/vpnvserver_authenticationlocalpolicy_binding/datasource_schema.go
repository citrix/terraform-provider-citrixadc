...
// VpnvserverAuthenticationlocalpolicyBindingDataSourceModel is the
...
// outputs) AND the read-only attributes the resource deliberately omits (acttype).
type VpnvserverAuthenticationlocalpolicyBindingDataSourceModel struct {
	...
	// (zion73x_readonly/vpnvserver_authenticationlocalpolicy_binding.json).
	Acttype types.Int64 `tfsdk:"acttype"`
}

func VpnvserverAuthenticationlocalpolicyBindingDataSourceSchema() schema.Schema {
	return schema.Schema{
		Attributes: map[string]schema.Attribute{
			...
			"bindpoint": schema.StringAttribute{
				// Optional (not Required): the datasource is looked up by name+policy;
				// bindpoint is not echoed by NITRO GET and is not a lookup key.
				Optional:    true,
				Computed:    true,
			...
			"acttype": schema.Int64Attribute{
				Computed:    true,
				Description: "The type of the bound authentication policy (returned by the appliance on GET).",
			},
		},
	}
}

// vpnvserver_authenticationlocalpolicy_bindingDataSourceSetAttrFromGet projects a
...
func vpnvserver_authenticationlocalpolicy_bindingDataSourceSetAttrFromGet(ctx context.Context, data *VpnvserverAuthenticationlocalpolicyBindingDataSourceModel, g map[string]interface{}) {
	tflog.Debug(ctx, "In vpnvserver_authenticationlocalpolicy_bindingDataSourceSetAttrFromGet Function")
	...
	// Composite key id (legacy format: name:<name>,policy:<policy>).
	idParts := []string{}
	idParts = append(idParts, fmt.Sprintf("name:%s", utils.UrlEncode(utils.AnyToString(g["name"]))))
	idParts = append(idParts, fmt.Sprintf("policy:%s", utils.UrlEncode(utils.AnyToString(g["policy"]))))
