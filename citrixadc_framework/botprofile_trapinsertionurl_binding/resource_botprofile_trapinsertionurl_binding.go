...
func (r *BotprofileTrapinsertionurlBindingResource) readBotprofileTrapinsertionurlBindingFromApi(ctx context.Context, data *BotprofileTrapinsertionurlBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "botprofile_trapinsertionurl_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("botprofile_trapinsertionurl_binding not found with the provided ID attributes"))
		return
	}
