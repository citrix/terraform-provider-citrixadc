...
func (r *CsvserverAppflowpolicyBindingResource) readCsvserverAppflowpolicyBindingFromApi(ctx context.Context, data *CsvserverAppflowpolicyBindingResourceModel, diags *diag.Diagnostics) {
	...
	// Resource is missing
	if len(dataArr) == 0 {
		diags.AddError("Client Error", "csvserver_appflowpolicy_binding returned empty array.")
		return
	}
	...
	//  Resource is missing
	if foundIndex == -1 {
		diags.AddError("Client Error", fmt.Sprintf("csvserver_appflowpolicy_binding not found with the provided ID attributes"))
		return
	}
