...
func createSpilloverpolicyFunc(d *schema.ResourceData, meta interface{}) error {
	...
	var spilloverpolicyName string
	if v, ok := d.GetOk("name"); ok {
		spilloverpolicyName = v.(string)
	} else {
		spilloverpolicyName= resource.PrefixedUniqueId("tf-spilloverpolicy-")
		d.Set("name", spilloverpolicyName)
	}
...
func updateSpilloverpolicyFunc(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("name") {
		log.Printf("[DEBUG]  citrixadc-provider: Name has changed for spilloverpolicy %s, starting update", spilloverpolicyName)
		spilloverpolicy.Name = d.Get("name").(string)
		hasChange = true
	}
