...
const testAccVpnvserver_authenticationsamlidppolicy_binding_basic_step2 = `
...
func TestAccVpnvserver_authenticationsamlidppolicy_binding_basic(t *testing.T) {
	resource.Test(t, resource.TestCase{
		...
		Steps: []resource.TestStep{
			...
			{
				Config: testAccVpnvserver_authenticationsamlidppolicy_binding_basic_step2,
				Check: resource.ComposeTestCheckFunc(
					testAccCheckVpnvserver_authenticationsamlidppolicy_bindingNotExist("citrixadc_vpnvserver_authenticationsamlidppolicy_binding.tf_binding", "tf_vserver,tf_samlidppolicy"),
				),
			},
		},
	})
...
func testAccCheckVpnvserver_authenticationsamlidppolicy_bindingExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No vpnvserver_authenticationsamlidppolicy_binding id is set")
		}
		...
		if !found {
			return fmt.Errorf("vpnvserver_authenticationsamlidppolicy_binding %s not found", n)
		}
...
func testAccCheckVpnvserver_authenticationsamlidppolicy_bindingNotExist(n string, id string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		client := testAccProvider.Meta().(*NetScalerNitroClient).client
		...
		if found {
			return fmt.Errorf("vpnvserver_authenticationsamlidppolicy_binding %s was found, but it should have been destroyed", n)
		}
...
func testAccCheckVpnvserver_authenticationsamlidppolicy_bindingDestroy(s *terraform.State) error {
	nsClient := testAccProvider.Meta().(*NetScalerNitroClient).client

	for _, rs := range s.RootModule().Resources {
		if rs.Type != "citrixadc_vpnvserver_authenticationsamlidppolicy_binding" {
			continue
		}
		...
		_, err := nsClient.FindResource(service.Vpnvserver_authenticationsamlidppolicy_binding.Type(), rs.Primary.ID)
		if err == nil {
			return fmt.Errorf("vpnvserver_authenticationsamlidppolicy_binding %s still exists", rs.Primary.ID)
		}
