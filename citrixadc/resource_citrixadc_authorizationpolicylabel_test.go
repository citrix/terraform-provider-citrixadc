...
func testAccCheckAuthorizationpolicylabelExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		data, err := nsClient.FindResource(service.Authorizationpolicylabel.Type(), rs.Primary.ID)
...
func testAccCheckAuthorizationpolicylabelDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := nsClient.FindResource(service.Authorizationpolicylabel.Type(), rs.Primary.ID)
		if err == nil {
