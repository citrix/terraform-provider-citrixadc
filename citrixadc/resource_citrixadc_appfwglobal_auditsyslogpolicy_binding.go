...
func readAppfwglobal_auditsyslogpolicy_bindingFunc(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] citrixadc-provider: Reading appfwglobal_auditsyslogpolicy_binding state %s", policyname)
	...
	if len(dataArr) == 0 {
		...
		log.Printf("[WARN] citrixadc-provider: Clearing appfwglobal_auditsyslogpolicy_binding state %s", policyname)
		d.SetId("")
		return nil
	}
