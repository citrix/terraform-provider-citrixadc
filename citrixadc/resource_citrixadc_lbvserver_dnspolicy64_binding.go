...
func readLbvserver_dnspolicy64_bindingFunc(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] citrixadc-provider: Reading lbvserver_dnspolicy64_binding state %s", bindingId)
	...
	if len(dataArr) == 0 {
		...
		log.Printf("[WARN] citrixadc-provider: Clearing lbvserver_dnspolicy64_binding state %s", bindingId)
		d.SetId("")
		return nil
	}
