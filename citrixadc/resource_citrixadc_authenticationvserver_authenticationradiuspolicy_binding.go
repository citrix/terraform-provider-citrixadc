...
func readAuthenticationvserver_authenticationradiuspolicy_bindingFunc(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] citrixadc-provider: Reading authenticationvserver_authenticationradiuspolicy_binding state %s", bindingId)
	...
	if len(dataArr) == 0 {
		...
		log.Printf("[WARN] citrixadc-provider: Clearing authenticationvserver_authenticationradiuspolicy_binding state %s", bindingId)
		d.SetId("")
		return nil
	}
