...
func readLbvserver_spilloverpolicy_bindingFunc(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] citrixadc-provider: Reading lbvserver_spilloverpolicy_binding state %s", bindingId)
	...
	if len(dataArr) == 0 {
		...
		log.Printf("[WARN] citrixadc-provider: Clearing lbvserver_spilloverpolicy_binding state %s", bindingId)
		d.SetId("")
		return nil
	}
