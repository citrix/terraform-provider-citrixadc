...
func createAuditnslogactionFunc(d *schema.ResourceData, meta interface{}) error {
	...
	var auditnslogactionName string
	if v, ok := d.GetOk("name"); ok {
		auditnslogactionName = v.(string)
	} else {
		auditnslogactionName = resource.PrefixedUniqueId("tf-auditnslogaction-")
		d.Set("name", auditnslogactionName)
	}
...
func updateAuditnslogactionFunc(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("name") {
		log.Printf("[DEBUG]  citrixadc-provider: Name has changed for auditnslogaction %s, starting update", auditnslogactionName)
		auditnslogaction.Name = d.Get("name").(string)
		hasChange = true
	}
	...
	if hasChange {
		_, err := client.UpdateResource(service.Auditnslogaction.Type(), auditnslogactionName, &auditnslogaction)
		if err != nil {
