...
func testAccCheckLocationExist(n string, id *string) resource.TestCheckFunc {
	return func(s *terraform.State) error {
		...
		if rs.Primary.ID == "" {
			return fmt.Errorf("No location name is set")
		}
		...
		data, err := nsClient.FindResource(service.Location.Type(), rs.Primary.ID)
		...
		if data == nil {
			return fmt.Errorf("location %s not found", n)
		}
...
func testAccCheckLocationDestroy(s *terraform.State) error {
	...
	for _, rs := range s.RootModule().Resources {
		...
		_, err := nsClient.FindResource(service.Location.Type(), rs.Primary.ID)
		if err == nil {
