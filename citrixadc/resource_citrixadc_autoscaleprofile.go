...
func createAutoscaleprofileFunc(d *schema.ResourceData, meta interface{}) error {
	...
	var autoscaleprofileName string
	if v, ok := d.GetOk("name"); ok {
		autoscaleprofileName = v.(string)
	} else {
		autoscaleprofileName = resource.PrefixedUniqueId("tf-autoscaleprofile-")
		d.Set("name", autoscaleprofileName)
	}
...
func readAutoscaleprofileFunc(d *schema.ResourceData, meta interface{}) error {
	...
	d.Set("apikey", data["apikey"])
	d.Set("name", data["name"])
	d.Set("sharedsecret", data["sharedsecret"])
...
func updateAutoscaleprofileFunc(d *schema.ResourceData, meta interface{}) error {
	...
	if d.HasChange("name") {
		log.Printf("[DEBUG]  citrixadc-provider: Name has changed for autoscaleprofile %s, starting update", autoscaleprofileName)
		autoscaleprofile.Name = d.Get("name").(string)
		hasChange = true
	}
	...
	if d.HasChange("type") {
		log.Printf("[DEBUG]  citrixadc-provider: Type has changed for autoscaleprofile %s, starting update", autoscaleprofileName)
		autoscaleprofile.Type = d.Get("type").(string)
		hasChange = true
	}
	if d.HasChange("url") {
	...
	if hasChange {
		_, err := client.UpdateResource(service.Autoscaleprofile.Type(), autoscaleprofileName, &autoscaleprofile)
		if err != nil {
