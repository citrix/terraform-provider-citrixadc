...
func readHanode_routemonitor6_bindingFunc(d *schema.ResourceData, meta interface{}) error {
	...
	log.Printf("[DEBUG] citrixadc-provider: Reading hanode_routemonitor6_binding state %s", bindingId)
	...
	if len(dataArr) == 0 {
		...
		log.Printf("[WARN] citrixadc-provider: Clearing hanode_routemonitor6_binding state %s", bindingId)
		d.SetId("")
		return nil
	}
...
func deleteHanode_routemonitor6_bindingFunc(d *schema.ResourceData, meta interface{}) error {
	...
	args := make([]string, 0)
	args = append(args, fmt.Sprintf("routemonitor:%s", url.PathEscape(routemonitor)))
