...
The sslkeyfile resource imports a private key file onto the Citrix ADC so it can be paired with a certificate to terminate SSL traffic. The key file is fetched from a remote source at creation time and stored under the given name on the appliance. If the key is encrypted, the passphrase used to decrypt it can be supplied via the `password` attributes.
...
resource "citrixadc_sslkeyfile" "tf_sslkeyfile" {
  ...
  password = var.sslkeyfile_password
}
```
...
resource "citrixadc_sslkeyfile" "tf_sslkeyfile" {
  name                = "servercert1key"
  src                 = "http://www.example.com/key_file.pem"
  password_wo         = var.sslkeyfile_password
...
An sslkeyfile can be imported using its name, e.g.

```shell
terraform import citrixadc_sslkeyfile.tf_sslkeyfile servercert1key
```
