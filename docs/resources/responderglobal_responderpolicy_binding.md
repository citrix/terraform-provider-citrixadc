...
The responderglobal_responderpolicy_binding resource is used to bind a responder policy to the responder global bind point.
...
* `globalbindtype` - (Optional) Applicable global bind point. Possible values: [ SYSTEM_GLOBAL, VPN_GLOBAL, RNAT_GLOBAL ] Defaults to `"SYSTEM_GLOBAL"`.
* `type` - (Optional) Specifies the bind point whose policies you want to display. Available settings function as follows: REQ_OVERRIDE binds the policy to the priority request queue; REQ_DEFAULT binds the policy to the default request queue; OTHERTCP_REQ_OVERRIDE and OTHERTCP_REQ_DEFAULT bind to the non-HTTP TCP priority and default request queues; SIPUDP_REQ_OVERRIDE and SIPUDP_REQ_DEFAULT bind to the SIP UDP queues; RADIUS_REQ_OVERRIDE and RADIUS_REQ_DEFAULT bind to the RADIUS queues; MSSQL_REQ_OVERRIDE and MSSQL_REQ_DEFAULT bind to the Microsoft SQL queues; MYSQL_REQ_OVERRIDE and MYSQL_REQ_DEFAULT bind to the MySQL queues; HTTPQUIC_REQ_OVERRIDE and HTTPQUIC_REQ_DEFAULT bind to the HTTP_QUIC queues.
...
* `labeltype` - (Optional) Type of invocation. Available settings function as follows: vserver forwards the request to the specified virtual server; policylabel invokes the specified policy label. Possible values: [ vserver, policylabel ]
