...
* `clientsecret_wo_version` - (Optional) A user-managed integer version tracker for `clientsecret_wo`. Because write-only values are not stored in state, Terraform cannot detect when the value changes. Increment this version number to signal that the client secret has changed and re-send it; for this resource that forces the application to be replaced. There is no default value, and the provider never sets it automatically.
