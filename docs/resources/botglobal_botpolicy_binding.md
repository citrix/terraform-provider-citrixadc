...
* `type` - (Optional) Specifies the bind point whose policies you want to display. Available settings function as follows: * REQ_OVERRIDE - Request override. Binds the policy to the priority request queue. * REQ_DEFAULT - Binds the policy to the default request queue. Possible values: [ REQ_OVERRIDE, REQ_DEFAULT ]
* `globalbindtype` - (Optional) Global bind type for the binding. Defaults to `"SYSTEM_GLOBAL"`.
...
A botglobal_botpolicy_binding can be imported using its policyname and type, e.g.

```shell
terraform import citrixadc_botglobal_botpolicy_binding.tf_binding tf_botpolicy,REQ_DEFAULT
```
