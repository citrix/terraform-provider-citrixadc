...
* **Read** is a no-op. The cleared entries are not a persistent managed object, so there is no NITRO query endpoint for clear state; the provider preserves state rather than re-fetching (which would otherwise cause perpetual drift). Because of this, the separate `citrixadc_gslbldnsentries` data source handles the read/count side of the entries.
...
* `nodeid` - (Optional) Unique number that identifies the cluster node. This is a GET-only cluster filter on the appliance and is not sent in the `clear` action payload; the provider tracks it so that changing it forces the `clear` action to re-run (it forces resource replacement).
...
* `id` - A synthetic identifier with the constant value `gslbldnsentries_clear`. The cleared LDNS entries are not a queryable managed object, so this ID is purely a Terraform state handle and does not map to any server-assigned key.
