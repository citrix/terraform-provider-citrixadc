...
* `id` - The id of the appfwglobal_auditnslogpolicy_binding. It is the concatenation of the `policyname` and `type` attributes separated by a comma.


## Import

A appfwglobal_auditnslogpolicy_binding can be imported using the concatenation of its `policyname` and `type` separated by a comma, e.g.

```shell
terraform import citrixadc_appfwglobal_auditnslogpolicy_binding.tf_binding tf_auditnslogpolicy,NONE
```
