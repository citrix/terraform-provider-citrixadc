...
* `password` - (Optional, Sensitive) Password for the system user. Can include any ASCII character. The value is persisted in Terraform state (encrypted). See also `password_wo` for an ephemeral alternative. At least one of `password` or `password_wo` must be set.
