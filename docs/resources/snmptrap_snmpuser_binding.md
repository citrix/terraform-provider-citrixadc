...
The snmptrap_snmpuser_binding resource is used to bind an SNMP user to an SNMP trap.
...
* `securitylevel` - (Optional) Security level of the SNMPv3 trap. Possible values: [ noAuthNoPriv, authNoPriv, authPriv ]. Defaults to `"authNoPriv"`.
* `td` - (Optional) Integer value that uniquely identifies the traffic domain in which you want to configure the entity. If you do not specify an ID, the entity becomes part of the default traffic domain, which has an ID of 0. Defaults to `0`.
* `version` - (Optional) SNMP version, which determines the format of trap messages sent to the trap listener. This setting must match the setting on the trap listener. Otherwise, the listener drops the trap messages. Possible values: [ V1, V2, V3 ]. Defaults to `"V3"`.
...
* `id` - The id of the snmptrap_snmpuser_binding. It is the concatenation of the `trapclass`, `trapdestination` and `username` attributes separated by a comma.
