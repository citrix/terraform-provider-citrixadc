...
The sslwrapkey resource creates a named SSL wrap key on the Citrix ADC. A wrap key is a password- and salt-derived key used by the appliance's crypto subsystem to wrap (encrypt) and unwrap other key material — for example, when exporting or importing protected keys. The key is immutable: any change to its attributes forces a new wrap key to be created.
...
```hcl
variable "sslwrapkey_password" {
  type      = string
  sensitive = true
}

variable "sslwrapkey_salt" {
  type      = string
  sensitive = true
}
...
resource "citrixadc_sslwrapkey" "tf_sslwrapkey" {
  ...
  password    = var.sslwrapkey_password
  salt        = var.sslwrapkey_salt
}
```
...
resource "citrixadc_sslwrapkey" "tf_sslwrapkey" {
  wrapkeyname         = "mywrapkey"
  password_wo         = var.sslwrapkey_password
  password_wo_version = 1
  salt_wo             = var.sslwrapkey_salt
  salt_wo_version     = 1
}
```
...
* `id` - The id of the sslwrapkey. It has the same value as the `wrapkeyname` attribute.

## Import

An sslwrapkey can be imported using its wrapkeyname, e.g.

```shell
terraform import citrixadc_sslwrapkey.tf_sslwrapkey mywrapkey
```
