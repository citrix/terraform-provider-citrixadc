...
* `pempassphrase_wo_version` - (Optional) A user-managed integer version tracker for `pempassphrase_wo`. Because write-only values are not stored in state, Terraform cannot detect when the value changes. Increment this version number to signal that the value has changed and force the resource to re-send the write-only secret. Changing this value forces the resource to be replaced. There is no default; set it explicitly when using `pempassphrase_wo`.
...
A sslcert can be imported using its id, e.g.

```shell
terraform import citrixadc_sslcert.tf_sslcert sslcert-config
```
