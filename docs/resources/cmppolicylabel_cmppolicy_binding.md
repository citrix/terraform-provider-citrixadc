...
The cmppolicylabel_cmppolicy_binding resource is used to bind an HTTP compression policy to a compression policy label.
...
A cmppolicylabel_cmppolicy_binding can be imported using the concatenation of the `labelname` and `policyname` attributes separated by a comma, e.g.
