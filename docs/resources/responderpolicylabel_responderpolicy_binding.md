...
The responderpolicylabel_responderpolicy_binding resource is used to bind a responder policy to a responder policy label.
...
* `invokelabelname` - (Optional) If labelType is policylabel, name of the policy label to invoke. If labelType is reqvserver or resvserver, name of the virtual server.
...
* `id` - The id of the responderpolicylabel_responderpolicy_binding. It is the concatenation of `labelname` and `policyname` attributes separated by a comma.


## Import

A responderpolicylabel_responderpolicy_binding can be imported using the concatenation of the `labelname` and `policyname` attributes separated by a comma, e.g.
