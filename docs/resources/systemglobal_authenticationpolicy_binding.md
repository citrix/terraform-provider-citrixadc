...
The systemglobal_authenticationpolicy_binding resource is used to bind an authentication policy to the system global configuration.
...
* `builtin` - (Optional) Indicates that a variable is a built-in (SYSTEM INTERNAL) type. This is a list of strings. Possible values: [ MODIFIABLE, DELETABLE, IMMUTABLE, PARTITION_ALL ]
...
* `globalbindtype` - (Optional) The global bind point at which the policy is bound. Defaults to `"SYSTEM_GLOBAL"`. Possible values: [ SYSTEM_GLOBAL, VPN_GLOBAL, RNAT_GLOBAL ]
...
A systemglobal_authenticationpolicy_binding can be imported using its policyname, e.g.

```shell
terraform import citrixadc_systemglobal_authenticationpolicy_binding.tf_bind tf_authenticationpolicy
```
