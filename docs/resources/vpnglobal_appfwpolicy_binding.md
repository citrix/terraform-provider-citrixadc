...
```hcl
resource "citrixadc_appfwprofile" "tf_profile" {
  name = "tf_appfwprofile"
  type = ["HTML"]
}

resource "citrixadc_appfwpolicy" "tf_policy" {
  name        = "tf_appfwpolicy"
  rule        = "true"
  profilename = citrixadc_appfwprofile.tf_profile.name
}

resource "citrixadc_vpnglobal_appfwpolicy_binding" "tf_bind" {
  policyname             = citrixadc_appfwpolicy.tf_policy.name
  priority               = 100
...
* `policyname` - (Required) The name of the application firewall policy to bind to the global VPN bind point. Changing this forces a new resource to be created.
...
* `id` - The id of the vpnglobal_appfwpolicy_binding. It has the same value as the `policyname` attribute.


## Import

A vpnglobal_appfwpolicy_binding can be imported using its policyname, e.g.

```shell
terraform import citrixadc_vpnglobal_appfwpolicy_binding.tf_bind tf_appfwpolicy
```
