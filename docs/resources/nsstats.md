...
```hcl
resource "citrixadc_nsstats" "clear_global" {
  cleanuplevel = "global"
}
```
...
* `id` - A synthetic identifier with the fixed value `nsstats-config`. Because `nsstats` is an action-only resource with no NITRO GET endpoint, the ID does not correspond to a readable ADC object.
