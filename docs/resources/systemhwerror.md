...
* `id` - The id of the systemhwerror resource. It is a synthetic constant string `"systemhwerror-config"`. The ID is purely a Terraform state handle, not a NITRO lookup key (there is no GET endpoint for this resource).
