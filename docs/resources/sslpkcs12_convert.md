...
The secret attributes can be supplied either as sensitive attributes (persisted in state) or as write-only attributes (not persisted in state). At least one variant of `password`/`password_wo` is required. The `pempassphrase`/`pempassphrase_wo` pair is optional and only needed to unlock an encrypted input PEM key when exporting.

### Using the sensitive attributes (persisted in state)

```hcl
variable "sslpkcs12_convert_password" {
  type      = string
  sensitive = true
}

variable "sslpkcs12_convert_pempassphrase" {
  type      = string
  sensitive = true
}

# Export an existing PEM certificate and key into a PKCS#12 bundle
resource "citrixadc_sslpkcs12_convert" "tf_pkcs12" {
  ...
  export        = true
  aes256        = true
  password      = var.sslpkcs12_convert_password
  pempassphrase = var.sslpkcs12_convert_pempassphrase
}
```
...
resource "citrixadc_sslpkcs12_convert" "tf_pkcs12" {
  ...
  export                   = true
  aes256                   = true
  password_wo              = var.sslpkcs12_convert_password
  password_wo_version      = 1
  pempassphrase_wo         = var.sslpkcs12_convert_pempassphrase
...
* `pempassphrase` - (Optional, Sensitive) Pass phrase used to encrypt the PEM private key. Required only when the input PEM key is encrypted (typically during an export operation); exporting an unencrypted key needs no pass phrase. The value is persisted in Terraform state (encrypted). See also `pempassphrase_wo` for an ephemeral alternative. Changing this attribute forces a new resource to be created.
...
* `id` - A synthetic identifier for this action-only resource. It is a fixed string with the value `sslpkcs12_convert`. It does not correspond to any object on the Citrix ADC, because this action-only resource has no NITRO GET endpoint.
