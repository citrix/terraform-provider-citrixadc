...
# Resource: nslaslicense

The nslaslicense resource applies a local-area-services (LAS) / fixed-bandwidth license file that has already been staged on the Citrix ADC, activating the licensed capacity it grants. Use it to bring a previously uploaded license file into effect on the appliance.
...
* `id` - The id of the nslaslicense resource. It is a synthetic identifier derived from the `filename` attribute.
