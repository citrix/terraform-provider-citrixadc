...
* `pempassphrase_wo_version` - (Optional) A user-managed integer version tracker for `pempassphrase_wo`. Because write-only values are not stored in state, Terraform cannot detect when the value changes. Increment this version number to signal that the value has changed and force the resource to re-send the write-only secret; because the underlying secret cannot be updated in place, this triggers a replacement of the resource.
...
* `challengepassword_wo_version` - (Optional) A user-managed integer version tracker for `challengepassword_wo`. Because write-only values are not stored in state, Terraform cannot detect when the value changes. Increment this version number to signal that the value has changed and force the resource to re-send the write-only secret; because the underlying secret cannot be updated in place, this triggers a replacement of the resource.
...
A sslcertreq can be imported using its id, e.g.

```shell
terraform import citrixadc_sslcertreq.tf_sslcertreq sslcertreq-config
```
