...
* `customtrap` - (Optional) By default, Customtrap will be disabled, set to enabled when using the feature.
...
A snmpoption can be imported using its id, e.g.

```shell
terraform import citrixadc_snmpoption.tf_snmpoption snmpoption-config
```
