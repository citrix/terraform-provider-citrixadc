...
# Resource: csvserver_cmppolicy_binding

The csvserver_cmppolicy_binding resource is used to create bindings between a content switching virtual server and a compression policy.
...
* `id` - The id of the csvserver_cmppolicy_binding. It is the concatenation of the `name` and `policyname` attributes separated by a comma.


## Import

A csvserver_cmppolicy_binding can be imported using its id, e.g.
