...
```hcl
resource "citrixadc_sslcrlfile_import" "tf_sslcrlfile_import" {
  name = "crl1"
...
* `id` - The id of the sslcrlfile_import resource. It has the same value as the `name` attribute.


## Import

An sslcrlfile_import resource can be imported using its name, e.g.

```shell
terraform import citrixadc_sslcrlfile_import.tf_sslcrlfile_import crl1
```
