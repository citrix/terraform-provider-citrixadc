...
The sslpkcs8 resource converts a private key file to PKCS#8 format on the Citrix ADC, reading the input key in PEM or DER format and writing the converted PKCS#8 key to an output file on the appliance filesystem.
...
resource "citrixadc_sslpkcs8" "tf_pkcs8" {
...
resource "citrixadc_sslpkcs8" "tf_pkcs8" {
  ...
  pkcs8file           = "servercert1_pkcs8.key"
  keyform             = "PEM"
  password_wo         = var.sslpkcs8_password
...
* `id` - The id of the sslpkcs8. It is a static string `"sslpkcs8-config"`, because this action-only resource has no NITRO GET endpoint.
