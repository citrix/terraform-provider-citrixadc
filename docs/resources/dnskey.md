...
* `password_wo_version` - (Optional) A user-managed integer version tracker for `password_wo`. Because write-only values are not stored in state, Terraform cannot detect when the value changes. This value is fully user-controlled: the provider does not set or default it. Increment this version number to signal that the value has changed and cause the write-only secret to be re-sent, which forces the resource to be replaced.
