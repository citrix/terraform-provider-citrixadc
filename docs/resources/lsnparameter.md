...
* `memlimit` - (Optional) Amount of Citrix ADC memory to reserve for the LSN feature, in multiples of 2MB. Note: If you later reduce the value of this parameter, the amount of active memory is not reduced. Changing the configured memory limit can only increase the amount of active memory. This command is deprecated, use 'set extendedmemoryparam -memlimit' instead.
...
A lsnparameter can be imported using its id, e.g.

```shell
terraform import citrixadc_lsnparameter.tf_lsnparameter lsnparameter-config
```
