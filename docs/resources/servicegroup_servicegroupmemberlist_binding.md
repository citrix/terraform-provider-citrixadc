...
Manages the complete set of members of a Citrix ADC service group as a single atomic unit. Applying this resource issues a NITRO `PUT` that replaces the entire member set of the named service group with the `members` you declare: any existing member that is not present in the list is removed (or disabled, depending on the graceful setting on the service group).
...
~> **No drift detection.** The NITRO API exposes no GET endpoint for this binding, so the provider cannot read the current members back from the ADC. The `Read` operation is a no-op that preserves prior Terraform state. Changes made outside Terraform (for example, members added through the CLI) are not detected. Because every attribute forces replacement, any change to `servicegroupname` or `members` recreates the binding, re-issuing the `PUT` that replaces the full member set.
