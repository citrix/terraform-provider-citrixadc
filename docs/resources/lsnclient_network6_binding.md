...
* `network` - (Optional) IPv4 address(es) of the LSN subscriber(s) or subscriber network(s) on whose traffic you want the Citrix ADC to perform Large Scale NAT.
...
```shell
terraform import citrixadc_lsnclient_network6_binding.tf_lsnclient_network6_binding my_lsn_client,2001:DB8:5001::/96
```
