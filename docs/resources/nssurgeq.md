...
```hcl
resource "citrixadc_nssurgeq" "flush_all" {}
```

Flush the surge queue of a specific service group member:

```hcl
resource "citrixadc_nssurgeq" "flush_member" {
  name       = "svcgrp1"
  servername = "websrv1"
  port       = 80
}
...
* `id` - A synthetic identifier with the fixed value `nssurgeq-config`. Because `nssurgeq` is an action-only resource with no NITRO GET endpoint, the ID does not correspond to a readable ADC object.
