...
* `id` - A synthetic identifier whose value equals the configured `sessionid`. It is purely a Terraform state handle for this action; the ADC does not assign an ID to the flush action, so this value is not a server-assigned key and cannot be used to look the resource up on the ADC.
