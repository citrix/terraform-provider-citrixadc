...
This resource does not create, read, or manage a persistent object on the appliance. The underlying NITRO endpoint exposes no add, set, or delete operation for the kill action, so there is no server-side object to reconcile:

* **Create** (`terraform apply`) fires the `kill` action.
* **Read**, **Update**, and **Delete** are no-ops.
* The `id` is a synthetic value; it does not correspond to a persistent object on the Citrix ADC.
...
* `id` - A synthetic identifier for this action-only resource. It is a fixed string with the value `rdpconnections_kill`. It does not correspond to any object on the Citrix ADC.
