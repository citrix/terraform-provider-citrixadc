...
* `sitekey` - (Optional, Sensitive) Sitekey to identify gateway fqdn while loading captcha. The value is persisted in Terraform state (encrypted). See also `sitekey_wo` for an ephemeral alternative. At least one of `sitekey` or `sitekey_wo` must be set.
* `sitekey_wo` - (Optional, Sensitive, WriteOnly) Same as `sitekey`, but the value is **not persisted in Terraform state**. Use this for improved secret hygiene. Must be used together with `sitekey_wo_version`. If both `sitekey` and `sitekey_wo` are set, `sitekey_wo` takes precedence. At least one of `sitekey` or `sitekey_wo` must be set.
