...
```hcl
resource "citrixadc_systemsshkey" "private_key" {
  name       = "id_rsa_admin"
...
```hcl
resource "citrixadc_systemsshkey" "public_key" {
...
A systemsshkey can be imported using its composite ID, in the form `name:<name>,sshkeytype:<sshkeytype>`:

```shell
terraform import citrixadc_systemsshkey.private_key name:id_rsa_admin,sshkeytype:PRIVATE
```
