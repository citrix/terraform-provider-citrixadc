...
### Read-only aaapreauthenticationaction metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_aaapreauthenticationaction` resource) and are Computed/GET-only. Any attribute the appliance does not return is `null`.
