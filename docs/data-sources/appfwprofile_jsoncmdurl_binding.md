...
* `keyname_json_cmd` - (Required) An expression that designates a keyname on the JSON CMD URL for which Command injection violations are relaxed.
* `as_value_type_json_cmd` - (Required) Type of the relaxed JSON CMD key value.
* `as_value_expr_json_cmd` - (Required) The JSON CMD key value expression.
