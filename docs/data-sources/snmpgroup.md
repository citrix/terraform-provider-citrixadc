...
### Read-only snmpgroup metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_snmpgroup` resource). They are Computed / GET-only and are `null` when the appliance does not return them.

* `storagetype` - The storage type for this group. Possible values: `volatile`, `nonVolatile`.
* `status` - The status of this group. Possible values: `active`.
