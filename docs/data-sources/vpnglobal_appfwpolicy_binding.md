...
# Data Source: vpnglobal_appfwpolicy_binding

The vpnglobal_appfwpolicy_binding data source allows you to retrieve information about an application firewall (appfw) policy bound to the global VPN bind point.


## Example usage

```terraform
data "citrixadc_vpnglobal_appfwpolicy_binding" "tf_bind" {
  policyname = "tf_appfwpolicy"
}

output "policyname" {
  value = data.citrixadc_vpnglobal_appfwpolicy_binding.tf_bind.policyname
}

output "priority" {
  value = data.citrixadc_vpnglobal_appfwpolicy_binding.tf_bind.priority
}
```
...
* `policyname` - (Required) The name of the application firewall policy bound to the global VPN bind point.
