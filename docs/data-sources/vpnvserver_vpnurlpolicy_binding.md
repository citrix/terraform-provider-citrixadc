...
# Data Source: vpnvserver_vpnurlpolicy_binding

The vpnvserver_vpnurlpolicy_binding data source allows you to retrieve information about a binding between a VPN virtual server and a VPN URL policy.
...
output "priority" {
  value = data.citrixadc_vpnvserver_vpnurlpolicy_binding.tf_bind.priority
}

output "gotopriorityexpression" {
  value = data.citrixadc_vpnvserver_vpnurlpolicy_binding.tf_bind.gotopriorityexpression
}
```
...
### Read-only vpnvserver_vpnurlpolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_vpnurlpolicy_binding` resource) and are therefore GET-only / Computed. Any attribute the appliance does not return is `null`.
