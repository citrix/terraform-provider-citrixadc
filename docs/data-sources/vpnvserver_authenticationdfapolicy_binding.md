...
# Data Source: vpnvserver_authenticationdfapolicy_binding

The vpnvserver_authenticationdfapolicy_binding data source allows you to retrieve information about a binding between a VPN virtual server and an authentication DFA policy.

## Example Usage

```terraform
data "citrixadc_vpnvserver_authenticationdfapolicy_binding" "tf_binding" {
  name   = "tf_vpnvserver"
  policy = "tf_dfapolicy"
}

output "priority" {
  value = data.citrixadc_vpnvserver_authenticationdfapolicy_binding.tf_binding.priority
}

output "groupextraction" {
  value = data.citrixadc_vpnvserver_authenticationdfapolicy_binding.tf_binding.groupextraction
}
```
...
* `id` - The id of the vpnvserver_authenticationdfapolicy_binding. It is a system-generated identifier.

### Read-only vpnvserver_authenticationdfapolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_authenticationdfapolicy_binding` resource). They are GET-only/Computed, and any attribute the appliance does not return is `null`.
