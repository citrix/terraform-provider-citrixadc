...
# Data Source: vpnvserver_authenticationldappolicy_binding

The vpnvserver_authenticationldappolicy_binding data source allows you to retrieve information about a binding between a VPN virtual server and an authentication LDAP policy.

## Example Usage

```terraform
data "citrixadc_vpnvserver_authenticationldappolicy_binding" "tf_binding" {
  name   = "vpn_vserver"
  policy = "tf_ldappolicy"
}

output "priority" {
  value = data.citrixadc_vpnvserver_authenticationldappolicy_binding.tf_binding.priority
}

output "groupextraction" {
  value = data.citrixadc_vpnvserver_authenticationldappolicy_binding.tf_binding.groupextraction
}
```
...
* `id` - The id of the vpnvserver_authenticationldappolicy_binding. It is the concatenation of the `name` and `policy` attributes separated by a comma.
...
### Read-only vpnvserver_authenticationldappolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_authenticationldappolicy_binding` resource). They are GET-only/Computed, and any attribute the appliance does not return is `null`.
