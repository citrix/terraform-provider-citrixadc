...
### Read-only dnsaaaarec metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_dnsaaaarec` resource). They are GET-only/Computed, and any attribute the appliance does not return is `null`.

* `vservername` - Virtual server name.
* `authtype` - Authentication type. Possible values: [ ALL, ADNS, PROXY ].
