...
# Data Source: vpnvserver_authenticationtacacspolicy_binding

The vpnvserver_authenticationtacacspolicy_binding data source allows you to retrieve information about a vpnvserver_authenticationtacacspolicy_binding configuration.
...
output "priority" {
  value = data.citrixadc_vpnvserver_authenticationtacacspolicy_binding.tf_bind.priority
}

output "gotopriorityexpression" {
  value = data.citrixadc_vpnvserver_authenticationtacacspolicy_binding.tf_bind.gotopriorityexpression
}
```
...
* `id` - The id of the vpnvserver_authenticationtacacspolicy_binding. It is the concatenation of the `name` and `policy` attributes separated by a comma.
...
### Read-only vpnvserver_authenticationtacacspolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_authenticationtacacspolicy_binding` resource). They are GET-only / Computed. Any attribute the appliance does not return is `null`.
