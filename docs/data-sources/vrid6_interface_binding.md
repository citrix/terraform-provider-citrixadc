...
### Read-only vrid6_interface_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vrid6_interface_binding` resource) and are therefore GET-only / Computed. Any attribute the appliance does not return is `null`.
