...
# Data Source: vpnvserver_vpnsessionpolicy_binding

The vpnvserver_vpnsessionpolicy_binding data source allows you to retrieve information about a binding between a VPN virtual server and a VPN session policy.
...
output "priority" {
  value = data.citrixadc_vpnvserver_vpnsessionpolicy_binding.tf_bind.priority
}

output "gotopriorityexpression" {
  value = data.citrixadc_vpnvserver_vpnsessionpolicy_binding.tf_bind.gotopriorityexpression
}
```
...
### Read-only vpnvserver_vpnsessionpolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_vpnsessionpolicy_binding` resource). They are Computed/GET-only and are `null` when the appliance does not return them.
