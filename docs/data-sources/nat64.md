---
subcategory: "Network"
---

# Data Source `nat64`

The nat64 data source allows you to retrieve information about NAT64 rule configurations.


## Example usage

```terraform
data "citrixadc_nat64" "tf_nat64" {
  name = "nat64_rule_1"
}

output "name" {
  value = data.citrixadc_nat64.tf_nat64.name
}

output "acl6name" {
  value = data.citrixadc_nat64.tf_nat64.acl6name
}

output "netprofile" {
  value = data.citrixadc_nat64.tf_nat64.netprofile
}
```


## Argument Reference

* `name` - (Required) Name for the NAT64 rule. Must begin with a letter, number, or the underscore character (_), and can consist of letters, numbers, and the hyphen (-), period (.) pound (#), space ( ), at sign (@), equals (=), colon (:), and underscore characters. Cannot be changed after the rule is created. Choose a name that helps identify the NAT64 rule.

## Attribute Reference

In addition to the arguments, the following attributes are available:

* `acl6name` - Name of any configured ACL6 whose action is ALLOW.  IPv6 Packets matching the condition of this ACL6 rule and destination IP address of these packets matching the NAT64 IPv6 prefix are considered for NAT64 translation.
* `netprofile` - Name of the configured netprofile. The Citrix ADC selects one of the IP address in the netprofile as the source IP address of the translated IPv4 packet to be sent to the IPv4 server.

## Attribute Reference

* `id` - The id of the nat64. It has the same value as the `name` attribute.


## Import

A nat64 can be imported using its name, e.g.

```shell
terraform import citrixadc_nat64.tf_nat64 nat64_rule_1
```
