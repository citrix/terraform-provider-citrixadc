...
# Data Source: vpnglobal_auditnslogpolicy_binding

The vpnglobal_auditnslogpolicy_binding data source allows you to retrieve information about an audit nslog policy bound to the global VPN bind point.


## Example usage

```terraform
data "citrixadc_vpnglobal_auditnslogpolicy_binding" "tf_bind" {
  policyname = "tf_nslogpolicy"
}

output "policyname" {
  value = data.citrixadc_vpnglobal_auditnslogpolicy_binding.tf_bind.policyname
}

output "priority" {
  value = data.citrixadc_vpnglobal_auditnslogpolicy_binding.tf_bind.priority
}
```
...
* `policyname` - (Required) The name of the audit nslog policy bound to the global VPN bind point.
