...
# Data Source: vpnvserver_authenticationlocalpolicy_binding

The vpnvserver_authenticationlocalpolicy_binding data source allows you to retrieve information about the binding between a VPN virtual server and an authentication local policy.
...
data "citrixadc_vpnvserver_authenticationlocalpolicy_binding" "tf_bind" {
  ...
  policy = "tf_localpolicy"
}

output "priority" {
  value = data.citrixadc_vpnvserver_authenticationlocalpolicy_binding.tf_bind.priority
}

output "bindpoint" {
  value = data.citrixadc_vpnvserver_authenticationlocalpolicy_binding.tf_bind.bindpoint
}
```
...
* `id` - The id of the vpnvserver_authenticationlocalpolicy_binding. It is the concatenation of `name` and `policy` attributes separated by comma.

### Read-only vpnvserver_authenticationlocalpolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_authenticationlocalpolicy_binding` resource). They are GET-only/Computed, and any attribute the appliance does not return is `null`.
