...
### Read-only vrid6 metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vrid6` resource). They are Computed / GET-only, and any attribute the appliance does not return is `null`.
...
* `type` - Type (static or dynamic) of this VRID. Possible values: `STATIC`, `DYNAMIC`.
* `state` - State of this VRID.
* `flags` - Flags.
* `ipaddress` - The IP address bound to the VRID6.
