...
# Data Source: vpnvserver_vpntrafficpolicy_binding

The vpnvserver_vpntrafficpolicy_binding data source allows you to retrieve information about a binding between a VPN virtual server and a VPN traffic policy.
...
output "priority" {
  value = data.citrixadc_vpnvserver_vpntrafficpolicy_binding.tf_bind.priority
}

output "gotopriorityexpression" {
  value = data.citrixadc_vpnvserver_vpntrafficpolicy_binding.tf_bind.gotopriorityexpression
}
```
...
* `id` - The id of the vpnvserver_vpntrafficpolicy_binding. It is the concatenation of the `name` and `policy` attributes separated by a comma.

### Read-only vpnvserver_vpntrafficpolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_vpntrafficpolicy_binding` resource) and are therefore GET-only / Computed. Any attribute the appliance does not return is `null`.

* `acttype` - Type of action associated with the bound policy.
