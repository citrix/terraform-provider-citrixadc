...
# Data Source: vpnvserver_authenticationloginschemapolicy_binding

The vpnvserver_authenticationloginschemapolicy_binding data source allows you to retrieve information about the authentication login schema policy bound to a VPN virtual server.

## Example usage

```terraform
data "citrixadc_vpnvserver_authenticationloginschemapolicy_binding" "example" {
  name   = "tf_vserver_example"
  policy = "tf_loginschemapolicy"
}

output "priority" {
  value = data.citrixadc_vpnvserver_authenticationloginschemapolicy_binding.example.priority
}

output "groupextraction" {
  value = data.citrixadc_vpnvserver_authenticationloginschemapolicy_binding.example.groupextraction
}
```
...
* `id` - The id of the vpnvserver_authenticationloginschemapolicy_binding. It is the concatenation of `name` and `policy` attributes separated by a comma.
