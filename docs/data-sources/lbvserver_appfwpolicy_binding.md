...
The lbvserver_appfwpolicy_binding data source allows you to retrieve information about the binding between a load balancing virtual server and an application firewall (AppFw) policy.


## Example Usage

```terraform
data "citrixadc_lbvserver_appfwpolicy_binding" "tf_binding" {
  name       = "demo_lb"
  policyname = "demo_appfwpolicy"
}

output "bindpoint" {
  value = data.citrixadc_lbvserver_appfwpolicy_binding.tf_binding.bindpoint
}

output "priority" {
  value = data.citrixadc_lbvserver_appfwpolicy_binding.tf_binding.priority
}
```
