...
# Data Source: authenticationvserver_authenticationldappolicy_binding

The authenticationvserver_authenticationldappolicy_binding data source allows you to retrieve information about the binding between an authentication virtual server and an authentication LDAP policy.
...
data "citrixadc_authenticationvserver_authenticationldappolicy_binding" "tf_bind" {
  ...
  policy = "tf_authenticationldappolicy"
}

output "priority" {
  value = data.citrixadc_authenticationvserver_authenticationldappolicy_binding.tf_bind.priority
}

output "bindpoint" {
  value = data.citrixadc_authenticationvserver_authenticationldappolicy_binding.tf_bind.bindpoint
}
```
...
### Read-only authenticationvserver_authenticationldappolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_authenticationvserver_authenticationldappolicy_binding` resource). They are GET-only / Computed. Any attribute the appliance does not return is `null`.
