...
# Data Source: vpnvserver_authenticationnegotiatepolicy_binding

The vpnvserver_authenticationnegotiatepolicy_binding data source allows you to retrieve information about an authentication negotiate policy binding to a VPN virtual server.

## Example Usage

```terraform
data "citrixadc_vpnvserver_authenticationnegotiatepolicy_binding" "tf_binding" {
  name   = "tf_vpnvserver"
  policy = "tf_negotiatepolicy"
}

output "priority" {
  value = data.citrixadc_vpnvserver_authenticationnegotiatepolicy_binding.tf_binding.priority
}

output "secondary" {
  value = data.citrixadc_vpnvserver_authenticationnegotiatepolicy_binding.tf_binding.secondary
}

output "groupextraction" {
  value = data.citrixadc_vpnvserver_authenticationnegotiatepolicy_binding.tf_binding.groupextraction
}
```
...
* `id` - The id of the vpnvserver_authenticationnegotiatepolicy_binding. It is the concatenation of the `name` and `policy` attributes separated by a comma.
...
### Read-only vpnvserver_authenticationnegotiatepolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_authenticationnegotiatepolicy_binding` resource). They are GET-only/Computed, and any attribute the appliance does not return is `null`.

* `acttype` - The type of the bound authentication policy (returned by the appliance on GET).
