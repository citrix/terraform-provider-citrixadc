...
* `id` - The id of the aaagroup_authorizationpolicy_binding. It is the concatenation of `groupname` and `policy` attributes separated by a comma.
* `type` - Bindpoint to which the policy is bound.

### Read-only aaagroup_authorizationpolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_aaagroup_authorizationpolicy_binding` resource). They are Computed/GET-only, and any attribute the appliance does not return is `null`.
