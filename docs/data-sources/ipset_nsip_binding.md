---
subcategory: "Network"
---

# Data Source: ipset_nsip_binding

The ipset_nsip_binding data source allows you to retrieve information about the binding between an IP set and IP addresses.


## Example Usage

```terraform
data "citrixadc_ipset_nsip_binding" "tf_ipset_nsip_binding" {
  name      = "tf_test_ipset"
  ipaddress = "10.1.1.1"
}

output "name" {
  value = data.citrixadc_ipset_nsip_binding.tf_ipset_nsip_binding.name
}

output "ipaddress" {
  value = data.citrixadc_ipset_nsip_binding.tf_ipset_nsip_binding.ipaddress
}
```


## Argument Reference

* `name` - (Required) Name of the IP set to which to bind IP addresses.
* `ipaddress` - (Required) One or more IP addresses bound to the IP set.


## Attribute Reference

In addition to the arguments, the following attributes are available:

* `id` - The id of the ipset_nsip_binding. It is a system-generated identifier.
