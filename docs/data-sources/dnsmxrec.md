...
### Read-only dnsmxrec metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_dnsmxrec` resource) and are `Computed`. Any attribute the appliance does not return is `null`.
