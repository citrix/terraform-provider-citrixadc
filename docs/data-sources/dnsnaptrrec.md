...
### Read-only dnsnaptrrec metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_dnsnaptrrec` resource). Any attribute the appliance does not return is `null`.
