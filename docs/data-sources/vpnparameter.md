...
output "splittunnel" {
  value = data.citrixadc_vpnparameter.tf_vpnparameter.splittunnel
}

output "locallanaccess" {
  value = data.citrixadc_vpnparameter.tf_vpnparameter.locallanaccess
}
```
...
* `backenddtls12` - Enables DTLS 1.2 for backend server handshakes
...
* `clientdebug` - Set the trace level on Citrix Gateway. Technical support technicians use these debug logs for in-depth debugging and troubleshooting purposes. Available settings function as follows: * DEBUG - Detailed debug messages are collected and written into the specified file. * STATS - Application audit level error messages and debug statistic counters are written into the specified file. * EVENTS - Application audit-level error messages are written into the specified file. * OFF - Only critical events are logged into the Windows Application Log.
...
* `clientoptions` - Display only the configured menu options when you select the "Configure Citrix Gateway" option in the Citrix Gateway Plug-in's system tray icon for Windows.
...
* `httpport` - Destination port numbers other than port 80, added as a comma-separated list. Traffic to these ports is processed as HTTP traffic, which allows functionality, such as HTTP authorization and single sign-on to a web application to work.
...
* `httptrackconnproxy` - Enable or disable HTTP tracking for packets proxied via vpn vserver using GSLB connection proxy feature.
...
* `secureprivateaccess` - Enables or disables the secure private access configuration.
...
* `spoofiip` - Indicate whether or not the application requires IP spoofing, which routes the connection to the intranet application through the virtual adapter.
...
* `windowsclienttype` - The Windows client type. Choose between two types of Windows Client a) Application Agent - which always runs in the task bar as a standalone application and also has a supporting service which runs permanently when installed b) Activex Control - ActiveX control run by Microsoft Internet Explorer.
