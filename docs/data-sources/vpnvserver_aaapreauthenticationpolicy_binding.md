...
# Data Source: vpnvserver_aaapreauthenticationpolicy_binding

The vpnvserver_aaapreauthenticationpolicy_binding data source allows you to retrieve information about a binding between a VPN virtual server and an AAA preauthentication policy.
...
output "priority" {
  value = data.citrixadc_vpnvserver_aaapreauthenticationpolicy_binding.tf_binding.priority
}

output "gotopriorityexpression" {
  value = data.citrixadc_vpnvserver_aaapreauthenticationpolicy_binding.tf_binding.gotopriorityexpression
}
```
...
### Read-only vpnvserver_aaapreauthenticationpolicy_binding metadata

These attributes are returned by the appliance on a GET (they are not configurable on the `citrixadc_vpnvserver_aaapreauthenticationpolicy_binding` resource). They are GET-only/Computed, and any attribute the appliance does not return is `null`.
